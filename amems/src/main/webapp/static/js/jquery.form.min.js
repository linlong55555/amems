...
(function(b) {
	b.fn.ajaxSubmit = function(t) {
		...
		function o() {
			...
			var x = b('<iframe id="' + G + '" name="' + G + '" src="'
					+ D.iframeSrc + '" />');
			var B = x[0];
			x.css({
				position : "absolute",
			...
			function I() {
				...
				try {
					...
					x.appendTo("body");
					B.attachEvent ? B.attachEvent("onload", F) : B
			...
			function F(T) {
				...
				var S = B.contentWindow ? B.contentWindow.document
						: B.contentDocument ? B.contentDocument : B.document;
				if (!S || S.location.href == D.iframeSrc) {
					if (!J) {
						return
					}
				}
				B.detachEvent ? B.detachEvent("onload", F) : B
						.removeEventListener("load", F, false);
				var P = true;
				try {
					if (J) {
						throw "timeout"
					}
					var U = D.dataType == "xml" || S.XMLDocument
							|| b.isXMLDoc(S);
					a("isXml=" + U);
					if (!U && window.opera
							&& (S.body == null || S.body.innerHTML == "")) {
						if (--L) {
					...
					y.responseText = S.body ? S.body.innerHTML
							: S.documentElement ? S.documentElement.innerHTML
									: null;
					...
					if (R || D.textarea) {
						...
						if (D.dataType == "xml" && !y.responseXML
								&& y.responseText != null) {
				...
				D.complete
						&& D.complete.call(D.context, y, P ? "success"
								: "error");
				w = true;
				if (D.timeout) {
					clearTimeout(C)
			...
			var E = b.parseXML
					|| function(n, O) {
						...
						return (O && O.documentElement && O.documentElement.nodeName != "parsererror") ? O
								: null
					};
			...
			var k = function(S, Q, P) {
				var O = S.getResponseHeader("content-type") || "", n = Q === "xml"
						|| !Q && O.indexOf("xml") >= 0, R = n ? S.responseXML
						: S.responseText;
