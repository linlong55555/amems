stockout_update_modal = {
	...
	setDate : function(data){
		...
		$("#id").val(data.id);					
		$("#shdh").val(data.shdh);		
		$("#shdh").removeClass("readonly-style");  //去掉文本框为白的的样式
		$("#shlx").val(data.shlx);					
...
$(document).ready(function(){
	Navigation(menuCode, '查看出库单信息', 'Select Technical Assessment', 'GC-2-4 ', '林龙', '2017-08-01', '林龙', '2017-08-01');//加载导航栏
	
	$("#form input").attr("readonly",true);
	$("#form select").attr("readonly",true);
	$("#form textarea").attr("readonly",true);
	stockout_update_modal.open($("#viewid").val());
