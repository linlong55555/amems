...
//		 $("#modalEname").html("Update Defect Deferred Log");
	...
	initInfo : function(param){
		...
		toolcheck_open_modal.getAttachments(param,false,false);				//加载附件
	...
	setDate : function(data){
		...
		console.info(data.paramsMap.zzwms);
		$("#bjmc").val(data.paramsMap.zzwms+" "+data.paramsMap.zywms);			//部件号
		...
		$("#kcsl").val(data.paramsMap.kcsl+" "+data.paramsMap.jldw);		//数量
 ...
 */
$(function(){
	toolcheck_find_modal.open();
	Navigation(menuCode, '查看计量工具', 'Select View', 'SC12-1 ', '林龙', '2017-09-27', '林龙', '2017-10-10');//加载导航栏
});