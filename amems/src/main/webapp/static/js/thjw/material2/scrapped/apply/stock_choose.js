...
stock_open_alert = {
	...
	inithcly: function(){
		...
		DicAndEnumUtil.registerDic('85','stock_open_alert_hcly', $.trim($("#zzjg").val()));
	},
	//初始化器材状态
	initqczt: function(){
		var this_ = this;
	...
	appendContentHtmlStock: function(list){
		...
		$.each(list,function(index,row){
			...
			htmlContent += "<input type='checkbox' onclick='"+this_.id+".rowonclick(event);'  name='"+this_.id+"_list_checkbox' value='"+index+"'  >"; 
			htmlContent += "</td>";
			...
			if(row.djsl && row.djsl > 0){
				htmlContent += "<td class='text-right' >"+StringUtil.escapeStr(row.kcsl-row.djsl)+"/"+StringUtil.escapeStr(row.kcsl)+" "+StringUtil.escapeStr(row.jldw)+"</td>"; 
			}else{
				htmlContent += "<td class='text-right' >"+StringUtil.escapeStr(row.kcsl)+" "+StringUtil.escapeStr(row.jldw)+"</td>"; 
			}
...
//		    htmlContent += "<td class='text-left'>"+DicAndEnumUtil.getEnumName('stockStatusEnum',row.zt)+"</td>"; 
		...
		TableUtil.addTitle("#"+this_.id+"_stocklist tr td"); //加载td title
		 SelectUtil.selectNode('selectAllId',this_.id+"_stocklist");
	},
	/**
	 * 搜索条件重置
	 */
	searchreset: function(){
		var this_=this;
		 $("#stock_open_alert_divSearch").find("input").each(function() {
				$(this).attr("value", "");
		 });
		 $("#stock_open_alert_divSearch").find("textarea").each(function(){
			 $(this).val("");
		 });
		 $("#stock_open_alert_divSearch").find("select").each(function(){
				$(this).val("");				
		 });
	...
	setData: function(){
		if(this.param.callback && typeof(this.param.callback) == "function"){
			var this_ = this;
			var data = [];
			$("#"+this_.id+"_stocklist", $("#"+this_.id)).find("tr input:checked").each(function(){
	...
	more: function() {//查询条件更多 展开/收缩
		var this_=this;
		if ($("#stock_open_alert_divSearch", $("#"+this.id)).css("display") == "none") {
			$("#stock_open_alert_divSearch", $("#"+this.id)).css("display", "block");
			this_.settingHeight();
			$("#icon", $("#"+this.id)).removeClass("icon-caret-down");
			$("#icon", $("#"+this.id)).addClass("icon-caret-up");
		} else {
			$("#stock_open_alert_divSearch", $("#"+this.id)).css("display", "none");
			this_.settingHeight();
	...
	rowonclick: function(e){
		SelectUtil.checkRow($(e.target).parent().find("input[type='checkbox']"),'selectAllId',this.id+"_stocklist");
	},
	settingHeight:function(){
		var modalBodyHeight=parseInt($("#stock_open_alert .modal-body").css("max-height"));
		var searchHeight=$("#stock_open_alert .row-height").outerHeight();
		var paginationHeight=$("#stock_open_alert .page-height").outerHeight();
		var tableHeight=modalBodyHeight-searchHeight-paginationHeight-10;
		$("#stock_open_alert").find("table").parent("div").css({"max-height":tableHeight+"px","overflow":"auto"})
	}
}
