...
var rentAddData={
		...
		initInfo : function(){
			...
			if(this.param.changeCss){
				$("#rent_table_div", $("#"+this.id)).removeClass("padding-right-8").addClass("padding-right-0");
			}
			this.initModelHead();
			this.initViewOrHide();
			this.initDic();
		...
		initModelHead : function(){
			var cn = "租出";
			var eng = "Rent";
			if(this.param.htlx == 31){
				cn = "租进";
				eng = "In";
			}
			$("#zcmodalHeadCN", $("#"+this.id)).html(cn);
			$("#zcmodalHeadENG", $("#"+this.id)).html(eng);
			$("#zccolHeadCN", $("#"+this.id)).html(cn);
			$("#zccolHeadENG", $("#"+this.id)).html(eng);
		},
		...
		addRow : function(obj){
			...
			if(StringUtil.escapeStr(obj.bjid) != ''){
				display = '';
			}
			var tr = "";
			tr += "<tr>";
			...
			/*序列号  */
			tr += '<td><input type="text" class="form-control" name="xlh" value="'+StringUtil.escapeStr(obj.xlh)+'" onBlur="'+this_.id+'.loadXlhRel(this)" onkeyup="'+this_.id+'.changeBjh(this)" /></td>';
			/*数量*/
			if(this_.param.colOptionhead){
				
				tr += '<td>';
				tr += '<div class="input-group col-xs-12">';
				tr += '<input type="text" class="form-control" name="sl" value="'+StringUtil.escapeStr(obj.sl)+'" onkeyup="'+this_.id+'.changeSl(this)" />';
				tr += '<span name="vdw" class="input-group-addon addon-newstyle" style="'+display+'">'+StringUtil.escapeStr(obj.dw)+'</span>';
				tr += '</div>';
				tr += '</td>';
				
//				tr += '<td><input type="text" class="form-control" name="sl" value="'+StringUtil.escapeStr(obj.sl)+'" onkeyup="'+this_.id+'.changeSl(this)" /></td>';
			}else{
				tr += "<td title='"+StringUtil.escapeStr(obj.sl)+"' style='text-align:right;vertical-align:middle;'>"+StringUtil.escapeStr(obj.sl)+"</td>";
			}
			
			var dj = formatUndefine(obj.dj);
			if(dj != '' || dj === 0){
				dj = dj.toFixed(2);
			}
			...
			/*单价*/
			if(this_.param.colOptionhead){
				tr += '<td><input type="text" class="form-control" name="dj" value="'+dj+'" onkeyup="'+this_.id+'.changeDj(this)" /></td>';
			}else{
				tr += "<td title='"+dj+"' style='text-align:right;vertical-align:middle;'>"+dj+"</td>";
			}
			/*状态*/
			if(this_.param.colOptionhead){
				tr += "<td><select name='wlzt' class='form-control'>"+this_.formatSelect(obj.wlzt)+"</select></td>";
			}else{
				tr += "<td title='"+StringUtil.escapeStr(obj.wlzt)+"' style='text-align:left;vertical-align:middle;'>"+StringUtil.escapeStr(obj.wlzt)+"</td>";
			}
			/*产权*/
			if(this_.param.colOptionhead){
				tr += '<td title="'+StringUtil.escapeStr(obj.cqbh)+'" name="zccq" style="text-align:left;vertical-align:middle;">';
//				if(this.param.htlx == 32){
//					tr += StringUtil.escapeStr(obj.cqbh);
//				}else{
//					tr += '<div class="input-group col-xs-12">';
...
//				}
			...
			var xj = 0;
			xj = formatUndefine(obj.sl) * formatUndefine(obj.dj);
			if(xj == ''){
				xj = 0;
			}
			xj = xj.toFixed(2);
			/*小计*/
			tr += "<td title='"+xj+"'  name='xj' style='text-align:right;vertical-align:middle;'>"+xj+"</td>";
...
//				indexOfTime(row.htrq)
		...
		setNoData : function(){
			$("#"+this.tbodyId, $("#"+this.id)).empty();
			if(this.param.colOptionhead){
				$("#"+this.tbodyId, $("#"+this.id)).append("<tr><td colspan='11' style='text-align:center;vertical-align:middle;' >暂无数据 No data.</td></tr>");
			}else{
				$("#"+this.tbodyId, $("#"+this.id)).append("<tr><td colspan='10' style='text-align:center;vertical-align:middle;' >暂无数据 No data.</td></tr>");
			}
		},
		/**
		 * 拼装交货期
		...
		loadXlhRel : function(obj){
			$(obj).parent().parent().find("input[name='sl']").removeAttr("disabled");
			if(obj.value != null && obj.value !=''){
				$(obj).parent().parent().find("input[name='sl']").val(1);
				$(obj).parent().parent().find("input[name='sl']").attr("disabled", "disabled");
			}
		},
		/**
		 * 加载部件相关信息
...
//					if(this_.param.htlx == 32){
//						$(this).find("td[name='zccq']").html(cqbh);
//						$(this).find("td[name='zccq']").attr("title", cqbh);
//					}else{
...
//					}
		...
		openCqWin : function(obj){
			var this_ = this;
			var dprtcode = this_.param.dprtcode;
			var cqId = $(obj).parent().parent().parent().find("input[name='cqid']").val();
			cqModal.show({
				...
				callback: function(data){//回调函数
					...
					$(obj).parent().parent().parent().find("input[name='cqid']").val(cqid);
				}
			});
		},
		/**
		...
		clearMaterialData : function($obj){
			...
			$obj.find("span[name='vdw']").html('');
			$obj.find("span[name='vdw']").css("display", "none");
		},
		setMaterialData : function($obj, obj){
			...
			$obj.find("span[name='vdw']").html(StringUtil.escapeStr(obj.jldw));
			$obj.find("span[name='vdw']").css("display", "");
		},
		//计算小计
		sumxj : function($obj){
			var this_ = this;
			var xj = 0;
			var sl = Number($.trim($obj.find("input[name='sl']").val()));
			var dj = Number($.trim($obj.find("input[name='dj']").val()));
			xj = sl * dj;
			if(xj == ''){
				xj = 0;
			}
...
//			}
			xj = xj.toFixed(2);
			$obj.find("td[name='xj']").html(xj);
			$obj.find("td[name='xj']").attr("title", xj);
			this.sumTotal();
		},
		//计算总计
		sumTotal : function(){
...
//			}
		...
		getData : function(){
			...
			if (len > 0){
				...
				$("#"+this.tbodyId,$("#"+this.id)).find("tr").each(function(){
					...
					var sl =$(tr_this).find("input[name='sl']").val();
					var dj =$(tr_this).find("input[name='dj']").val();
					var wlzt =$(tr_this).find("select[name='wlzt']").val();
					...
					if(null == sl || "" == sl || sl <= 0){
						...
						$(tr_this).find("input[name='sl']",$("#"+this_.id)).focus();
					...
					if(!this_.numberValidator.reg.test(sl)){  
						AlertUtil.showModalFooterMessage("数量"+this_.numberValidator.message);
					...
					if(null == dj || "" == dj){
						var message = "请输入单价!";
						AlertUtil.showModalFooterMessage(message);
						$(tr_this).find("input[name='dj']",$("#"+this_.id)).focus();
						$(tr_this).find("input[name='dj']").addClass("border-color-red");
						this_.isValid = false; 
						return false;
					}
					
					if(!this_.numberValidator.reg.test(dj)){  
						AlertUtil.showModalFooterMessage("单价"+this_.numberValidator.message);
						$(tr_this).find("input[name='dj']").focus();
						$(tr_this).find("input[name='dj']").addClass("border-color-red");
						this_.isValid = false; 
						return false;
					}
		...
		changeSl : function(obj){
			this.onkeyup4Num(obj);
			$(obj).removeClass("border-color-red");
			this.sumxj($(obj).parent().parent().parent());
		},
		changeDj : function(obj){
			this.onkeyup4Num(obj);
			$(obj).removeClass("border-color-red");
			this.sumxj($(obj).parent().parent());
		},
