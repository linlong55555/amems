...
function initMuliSelect(){
	
	var option="<option value='1'>收货</option>";
	    option+="<option value='2'>正常</option>";
	    option+="<option value='11'>长期封存</option>";
	    option+= "<option value='12'>校验中</option>";
	    option+="<option value='13'>修理中</option>";
	    option+="<option value='14'>外借中</option>"; 
	    $('#zt_search').empty();
	    $('#zt_search').append(option);	
	   $('#zt_search').multiselect({
...
function search(){
 TableUtil.resetTableSorting("insideut_list_table"); //重置排序图标
	var pgd = $("th[name='pgd']");
				...
				success : function(data) {
					...
					if (data.total > 0) {
						 ...
						 appendContentHtml(data.rows);
						  new Pagination({
							renderTo : "insideut_search_pagination",
							data : data,
							sortColumn : sortType,
							orderType : sequence,
							extParams : {},
						...
						// 标记关键字
					    signByKeyword($("#keyword").val(),[5,6,7,8,14,18,19,28,29,30,32],"#insideutList tr td");

					} else {
						$("#insideutList").empty();
						$("#insideut_search_pagination").empty();
						$("#insideutList").append("<tr class='text-center'><td colspan=\"32\">暂无数据 No data.</td></tr>");
					}
					new Sticky({
						tableId : 'insideut_list_table'
					});
				}
			});
...
function appendContentHtml(list) {
	...
	$.each(list,function(index, row) {
		...
		htmlContent += "<td style='text-align:center;vertical-align:middle;'><input type='checkbox' name='insideut_list_table_row' kcid='"+row.id+"' zt='"+row.zt+"' index="+index+"  onclick=SelectUtil.selectCheckbox(event,"+index+",'insideut_list_table','insideut_list_table_row',this) /></td>";
...
//		htmlContent = htmlContent + "<td class='text-left' title='" + StringUtil.escapeStr((row.materialBatchInfo?(((row.materialBatchInfo.jz||row.materialBatchInfo.jz==0)?row.materialBatchInfo.jz.toFixed(2):"")||""):"")+" "+(row.materialBatchInfo?(row.materialBatchInfo.jzbz||""):"")) + "'>"+ StringUtil.escapeStr((row.materialBatchInfo?(((row.materialBatchInfo.jz||row.materialBatchInfo.jz==0)?row.materialBatchInfo.jz.toFixed(2):"")||""):"")+" "+(row.materialBatchInfo?(row.materialBatchInfo.jzbz||""):"")) + "</td>";
		...
		htmlContent = htmlContent + "<td class='text-center' title='" + StringUtil.escapeStr((row.rksj||'').substring(0,10))+ "'>" + StringUtil.escapeStr((row.rksj||'').substring(0,10)) + "</td>";
		...
		htmlContent = htmlContent + "<td class='text-center' title='" + StringUtil.escapeStr((row.scrq||'').substring(0,10)) + "'>" + StringUtil.escapeStr((row.scrq||'').substring(0,10)) + "</td>";
...
//		htmlContent = htmlContent + "<td class='text-center' title='" + StringUtil.escapeStr(row.qczt) + "'>"+ StringUtil.escapeStr(row.qczt) + "</td>";
	...
	$("#insideutList").empty();
	$("#insideutList").append(htmlContent);
	 refreshPermission(); 
...
//searchParams.qczt=$.trim($("#qczt_search").val());	//器材状态
...
function updateStatus(zt){
	...
	var $checkedObjs = $("#insideutList").find(":checkbox:checked");
	...
	if (flag) {
		AlertUtil.showMessage("请选择非收货状态数据");
		return;
	}

	AjaxUtil.ajax({
		url : basePath + "/material/stock/tool/updateStatus",
		type : "post",
		contentType : "application/json;charset=utf-8",
		dataType : "json",
...
function exportExcel(){	
	...
	window.open(basePath+"/material/stock/material/2/StockInfo.xls?json="+JSON.stringify(param));
}

function showStyle(obj){
	$(obj).addClass("bg_tr");
	$("#insideutList tr").not(obj).each(function(){
