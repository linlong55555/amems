FilesDownModal = {
	...
	setEffectorDisabled : function(){
		this.getAttachments(this.param.djid,true,true);
...
//							}
						...
						this.uploadObj = $("#fileuploader", $("#"+this_.id)).uploadFile(
								{
									...
									onSuccess : function(files, data, xhr, pd) // 上传成功事件，data为后台返回数据
									{
										...
										if(this_.param.colOptionhead){
											trHtml += '<td><div>';
											trHtml += '<i class="deleterow icon-trash color-blue cursor-pointer" id="'+data.attachments[0].uuid+'"  title="删除 Delete">  ';
											trHtml += '</div></td>';
										}
									...
									if (len > 0) {
										...
										for (var i = 0; i < len; i++) {
											...
											if(this_.param.colOptionhead){
												...
												trHtml += '<i class="fjbtn deleterow icon-trash color-blue cursor-pointer" id="'+attachments[i].uuid+'" title="删除 Delete">  ';
												trHtml += '</div></td>';
											}
											trHtml += '<td style="text-align:center;vertical-align:middle;">';
											trHtml += 	  "<span name='orderNumber'>"+this_.orderNumber+"</span>";
											...
											trHtml += "<a href='javascript:void(0);' class='downloadfile' id='"+attachments[i].id+"' >"+(StringUtil.escapeStr(attachments[i].wbwjm)+"."+StringUtil.escapeStr(attachments[i].hzm))+"</a>";
											trHtml += '</td>';
					...
					loadAttachementsWithData : function(){
						...
						var attachments = [];
						var delList = [];
						$.each(this_.param.attachDatas, function(i, obj){
							if(obj.operate != "DEL"){
						...
						for(var i = 0; i < attachments.length; i++){
							...
							for(var j = 0; j < delList.length; j++){
								if(attachments[i].uuid == delList[j].uuid){
									isdelete = true;
								}
							}
							if(isdelete){
						...
						if (len > 0) {
							...
							for (var i = 0; i < len; i++) {
								...
								if(this_.param.colOptionhead){
									trHtml += '<td><div>';
									trHtml += '<i class="deleterow icon-trash color-blue cursor-pointer" id="'+attachments[i].uuid+'" title="删除 Delete">  ';
									trHtml += '</div></td>';
								}
								trHtml += '<td style="text-align:center;vertical-align:middle;">';
								trHtml += 	  "<span name='orderNumber'>"+this_.orderNumber+"</span>";
								...
								if(attachments[i].id){
									...
									trHtml += (StringUtil.escapeStr(attachments[i].wbwjm)+"."+StringUtil.escapeStr(attachments[i].hzm));
								}
								trHtml += '</td>';
