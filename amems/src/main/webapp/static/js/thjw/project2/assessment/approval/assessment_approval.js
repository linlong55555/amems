...
//		$("#glpgdid").html("<option value=''>暂无关联技术评估单No data</option>");
		...
		ReferenceUtil.init({
		 ...
		 Assessment_Open_Modal.button(4);			   //技术评估单新增权限
		 ...
		 $("#modalName").html("批准技术评估单");
		 $("#modalEname").html("Approval Evaluation");
		 $("#banDobule").hide();//隐藏高版本
	...
	EmptiedData : function(){
		...
		$("#Assessment_Open_Modal input[name='']").get(0).checked=true;
	 ...
	 */
	passed : function(){
		var url="/project2/assessment/approval/passed";//批准通过
		var str="批准通过";//审核通过
		Assessment_Approval_Modal.subjectFrom(str,url);
	},
	/**
	 * 批准驳回
	 */
	turnDown : function(){
		var url="/project2/assessment/approval/turnDown";//批准驳回
		var str="批准驳回";
		Assessment_Approval_Modal.subjectFrom(str,url);
	},
	/**
