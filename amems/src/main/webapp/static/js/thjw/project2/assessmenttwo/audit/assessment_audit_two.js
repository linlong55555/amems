...
//		$("#glpgdid").html("<option value=''>暂无关联技术评估单No data</option>");
		...
		ReferenceUtil.init({
		 ...
		 Assessment_Open_Modal.button(3);			    //技术评估单新增权限
		 ...
		 $("#bbstate").hide();//隐藏版本标识		 		//版本从1开始-显示
		 $("input[name='lylx']").attr("disabled",true); //来源radio按钮不可编辑，默认为其他
		 $("#lysm").show();							    //来源为其他时，显示来源说明
		 $("#modalName").html("审核技术评估单");
		 $("#modalEname").html("Audit Evaluation");
	...
	passed : function(){
		...
		var str="审核通过";//审核通过
		Assessment_Audit_Modal.subjectFrom(str,url);
	},
	/**
	 * 审核驳回
	 */
