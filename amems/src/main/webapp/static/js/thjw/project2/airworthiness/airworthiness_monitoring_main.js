	...
	// 带条件搜索的翻页
	 function AjaxGetDatasWithSearch(pageNumber,sortType,sequence){
		  ...
		  AjaxUtil.ajax({
				url : basePath + "/project2/airworthiness/monitoring/queryList",
				type : "post",
				...
				success : function(data) {
					...
					if (data.total > 0) {
						...
						// 标记关键字
						signByKeyword($("#keyword_search").val(), [ 3, 4,12 ]);
					} else {
						$("#airworthiness_list").empty();
						$("#pagination").empty();
						$("#airworthiness_list").append("<tr class='text-center'><td colspan=\"30\">暂无数据 No data.</td></tr>");
					}
					new Sticky({
	...
	/* 表格拼接*/
	 function appendContentHtml(list){
			...
			$.each(list,function(index,row){	
				htmlContent += "<tr onClick='show(this)'>";	
			    if(row.JSWJJKZT==0){
			    ...
			    if(row.SCFJID !=null&&row.SCFJID !=''){//文件
	...
	initMultiselect($("#dprtcode").val());
	 AjaxUtil.ajax({
			...
			success : function(data) {
				$("#fjzch").empty();
				if(data.length>0){
					var htmlcontent="<option value=''>选择全部 All</option>";
					$.each(data,function(index,row){
						htmlcontent +="<option value='"+row.fjzch+"'>"+row.fjzch+"</option>";
						$("#fjzch").html(htmlcontent);
					})
				}else{
					var htmlcontent="<option value=''>暂无数据</option>";
					$("#fjzch").html(htmlcontent);
				}
			}
	 });
...
function exportExcel(){
	...
	window.open(basePath+"/project2/airworthiness/CADMonitor.xls?params="+JSON.stringify(searchParams));
}