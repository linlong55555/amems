...
technical_order_history_alert_Modal = {
	id : 'technical_order_history_alert_Modal',
	getHistoryBbList : function(id){
		var this_ = this;
		startWait();
		// 提交数据
		AjaxUtil.ajax({
			url:basePath + "/project2/instruction/historylist",
			type:"post",
			async : false,
			data:{
				id:id
	...
	appendContentHtml: function(list){
		...
		$.each(list,function(index,row){
			...
			htmlContent += '<td title="'+StringUtil.escapeStr(row.jszlh)+'" class="text-left" style="padding-left:8px;">';
			htmlContent += "<a href='javascript:void(0);' onclick="+this_.id+".view('"+row.id+"') >"+StringUtil.escapeStr(row.jszlh)+"</a>";
			htmlContent += '</td>';
			...
			htmlContent += "<td class='text-center'>"+DicAndEnumUtil.getEnumName('technicalInstructionStatusEnum',row.zt)+"</td>";
