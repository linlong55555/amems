...
function initJkxszWin(way,mainId,syxlb) {
	...
	showHideDashedBorder(way);//显示隐藏虚线框
...
function reviewDatas(way,data){
	...
	if(way == 3){
		$.each(data,function(index,row){
			...
			$("#div"+(row.xc+2)+' input[jklbh="'+row.jklbh+'"]').click();
			$("#fddiv"+(row.xc+2)+row.jklbh).find("input[type=text]").attr("value",scz);//首次值
		});
	}
...
function initZxfsHead(){
	$("#"+param.id+" .formula_div").html([
				'<div class="pull-left padding-left-0 padding-right-0 form-group" style="width:230px;">',
					'<div class="padding-leftright-8 label-input-div" >',
					...
					'</div>',
				'</div>',
				'<div id="radioDivs"  class="pull-left"></div>',
				
				'<div class="pull-left padding-left-0 padding-right-0 form-group" >',
				'<div class="padding-leftright-8 label-input-div">',
				'<div class="input-group input-group-new" style="width:670px;">',
				...
				'<div class="input-group-addon"><label class="margin-right-5 " style="color:#333;"><input id="zztjbsradio" type="checkbox" style="vertical-align: -4px;" onchange=eo_main.setRedonly("zztj")  maxlength="7" >&nbsp;终止条件&nbsp;</label></div>',
				'<input class="form-control" id="zztj"  maxlength="100" type="text" style="visibility:hidden;">',
				'</div>',
					/*'<label class="margin-right-5 label-input"><input id="hdwzradio" type="checkbox">&nbsp;后到为准</label>',
					'<label class="margin-right-5 label-input"><input id="zztjbsradio" type="checkbox" onchange=eo_main.setRedonly("zztj")  maxlength="10" >&nbsp;终止条件</label>',
...
function initMonitorHead(data){
	var monitorHtml = '<div class="monitor_setting_css" style="padding-left:10px;padding-right:10px;">';
	monitorHtml += '<div class="col-lg-8 col-sm-8 col-xs-8 padding-left-0 padding-right-0 form-group settingFd" id="div'+param.countNum+'" name="danciChongfuDiv">';
	monitorHtml += '<p class="settingFd-p" name="danciChongfuP" >第<span>1</span>次</p>';
	...
	$.each(data,function(index,row){
		...
		monitorHtml += '<input class="monitorItem" name="jkxItems" type="checkbox" style="vertical-align: -4px;" appendToId="div'+param.countNum+'" jkflbh="'+row.jkflbh+'" jklbh="'+row.jklbh+'" jklms="'+row.ms+'" dcSortNumber="'+index+'" >&nbsp;';
		monitorHtml += row.ms;
		monitorHtml += '</label>';
	});
	monitorHtml += '</div>';
...
function initFdMonitorHead(data){
	...
	var monitorHtml = '<div class="monitor_setting_css" style="padding-left:10px;padding-right:10px;">';
	monitorHtml += '<div class="col-lg-8 col-sm-8 col-xs-8 padding-left-0 padding-right-0 form-group settingFd" id="div'+param.countNum+'" >';
	monitorHtml += '<p class="settingFd-p">第<span name="jkxOrderNumber">1</span>次</p>';
	monitorHtml += '<div class="col-lg-11 col-md-11 col-sm-10 col-xs-9 padding-leftright-8">';
	...
	$.each(data,function(index,row){
		monitorHtml += '<label style="margin-right:10px;" name="monitor">';
		monitorHtml += '<input class="monitorItem" name="fdjkxItems'+param.countNum+'" type="checkbox" style="vertical-align: -4px;" appendToId="div'+param.countNum+'" jkflbh="'+row.jkflbh+'" jklbh="'+row.jklbh+'" jklms="'+row.ms+'" dcSortNumber="'+index+'" >&nbsp;';
		monitorHtml += row.ms;
		monitorHtml += '</label>';
	});
...
function initDCMonitorBody(){		
	...
	monitorBody +=' <input class="form-control input-sm" type="text" maxlength="10" name="zq">';
...
function initCFMonitorBody(data){
	...
	$.each(data,function(index,row){
		...
		monitorBody +=buildUnit(row,'sc',3);
		monitorBody += '</div>';
		...
		monitorBody +='<div class="input-group padding-left-8">';
		if(MonitorUnitUtil.isTime(row.jklbh)){
			monitorBody +=' <input class="form-control input-sm" type="text" maxlength="7" name="zq" jklbhrow="'+row.jklbh+'_row"  onkeyup="validateInputTime2(this)">';
		}else{
			monitorBody +=' <input class="form-control input-sm" type="text" maxlength="7" name="zq" jklbhrow="'+row.jklbh+'_row"  onkeyup="validateInputNumber2(this)">';
		}
		...
		if(MonitorUnitUtil.isTime(row.jklbh)){
			monitorBody +=' <input class="form-control input-sm" type="text" maxlength="7" name="rcq" onkeyup="validateInputTime(this)" value="0" >';
		}else{
			monitorBody +=' <input class="form-control input-sm" type="text" maxlength="7" name="rcq" onkeyup="validateInputNumber(this)" value="0" >';
		}
		monitorBody +='<span class="input-group-addon padding-left-5 padding-right-3" style="border:none;background-color:white;font-size: 12px;">/</span>';
		monitorBody +='<span class="input-group-addon padding-left-5 padding-right-3" style="border:none;background-color:white;font-size: 12px;">+</span>';
		if(MonitorUnitUtil.isTime(row.jklbh)){
			monitorBody +=' <input class="form-control input-sm" type="text" maxlength="7" name="rch" onkeyup="validateInputTime(this)" value="0" >';
		}else{
			monitorBody +=' <input class="form-control input-sm" type="text" maxlength="7" name="rch" onkeyup="validateInputNumber(this)" value="0" >';
		}
...
function initFormula(){
	$("#radioDivs").empty();
	$("#radioDivs",$("#"+param.id)).append([
					'<div class=" pull-left padding-left-0 padding-right-0 form-group">',
					'<label class="pull-left text-right padding-left-0 padding-right-0" style="width:60px;text-align:right;">',
						'<div class="font-size-12 margin-topnew-2">计算公式</div>',
						'<div class="font-size-9">Formula</div>',
					'</label>',
					'<div class="pull-left padding-leftright-8" style="width:200px;">',
...
function changeWay(chkVal){
	...
	showHideDashedBorder(chkVal);//显示隐藏虚线框
...
function showOrHideDcDiv(chkObj){
	...
	var chkSortNumber = chkObj.attr("dcSortNumber");//选择的监控项序号
	...
	if(chkJklbh == "1_10"){
		...
		if(MonitorUnitUtil.isTime(chkJklbh)){
			monitorBody +=' <input class="form-control input-sm" type="text" maxlength="7" name="zq" onkeyup="validateInputTime(this)" >'; //加监听(时间格式)
		}else{
			monitorBody +=' <input class="form-control input-sm" type="text" maxlength="7" name="zq" onkeyup="validateInputNumber(this)" >';//加监听(数字)
		}
		monitorBody +=buildUnit(row,'zq',1);
	}
	monitorBody +='</div>';
	...
	//选中的个数
	if(getSelectMonitorNum() == 1){
		$("#dcDiv").append(monitorBody); //添加监控项
	}else{
		var isAppend = false;
		$.each(getSelectMonitorItems(),function(index,item){
			var jklbh_ = $(item).attr("jklbh");
			var dcSortNumber_ = $(item).attr("dcSortNumber");
			if(chkJklbh != jklbh_ && chkSortNumber < dcSortNumber_ && !isAppend){
				$("#dcDivContent"+jklbh_).before(monitorBody); //添加监控项
				isAppend = true;
			}
		});
		if(!isAppend){ //如果没有append就直接加到dcDiv
			$("#dcDiv").append(monitorBody); //添加监控项
		}
	}
...
function showOrHideFdDiv(chkObj){
	...
	//选中的个数
	var fdChkItem = $("#"+parentDiv+' input.monitorItem:checkbox:checked');
	
	if(fdChkItem.length == 1){
		$("#"+parentDiv).append(monitorBody); //添加监控项
	}else{
		var isAppend = false;
		$.each(fdChkItem,function(index,fdItem){
			var jklbh_ = $(fdItem).attr("jklbh");
			var dcSortNumber_ = $(fdItem).attr("dcSortNumber");
			if(chkJklbh != jklbh_ && chkSortNumber < dcSortNumber_ && !isAppend){
				$("#fd"+parentDiv+jklbh_).before(monitorBody); //添加监控项
				isAppend = true;
			}
		});
		if(!isAppend){ //如果没有append就直接加到dcDiv
...
function buildUnit(item, type,way){
	...
	if(type == "sc"){
		if(jklbh == "1_10"){
			if(way == 2){
				...
				if(MonitorUnitUtil.isTime(jklbh)){
					html +=' <input class="form-control input-sm" name="'+type+'" type="text" maxlength="7" jklbhrow="'+jklbh+'_row"  onkeyup="validateInputTime2(this)">';
				}else{
					html +=' <input class="form-control input-sm" name="'+type+'" type="text" maxlength="7" jklbhrow="'+jklbh+'_row"  onkeyup="validateInputNumber2(this)">';
				}
				html +='<div class="input-group-btn" style="min-width:45px;">';
				html +='<button value="12" name="'+type+'_dw" type="button" class="btn btn-default" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="min-width:45px;height:34px;padding:0 0 0 0;color: #555 !important;background:white;">'+MonitorUnitUtil.getMonitorUnit(jklbh,"12")+'&nbsp;<span class="caret"></span></button>';
				html +='<ul class="dropdown-menu dropdown-menu-right" style="min-width:45px;">';
				...
				html +='<div class="input-group padding-left-8" style="width: 85%;">';
				html +=' <input class="form-control input-sm datepicker" name="'+type+'" type="text" data-date-format="yyyy-mm-dd" maxlength="10"  onchange="changeInput(this)">';
				html +='</div>';
			}else{
				html +=' <input class="form-control input-sm datepicker" name="'+type+'" type="text" data-date-format="yyyy-mm-dd" maxlength="10"  onchange="changeInput(this)">';
			...
			if(way == 2){
				...
				if(MonitorUnitUtil.isTime(jklbh)){
					html +=' <input class="form-control input-sm" name="'+type+'" type="text" maxlength="7"  onkeyup="validateInputTime(this)">';
				}else{
					html +=' <input class="form-control input-sm" name="'+type+'" type="text" maxlength="7"  onkeyup="validateInputNumber(this)">';
				}
			}
		...
		if(jklbh == "1_10"){
			html +='<div class="input-group-btn" style="min-width:45px;">';
			if(type=="rc"){
				html +='<button value="11" name="'+type+'_dw" type="button" class="btn btn-default" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="min-width:45px;height:34px;padding:0 0 0 0;color: #555 !important;background:white;">'+MonitorUnitUtil.getMonitorUnit(jklbh,"11")+'&nbsp;<span class="caret"></span></button>';
			}else{
				html +='<button value="12" name="'+type+'_dw" type="button" class="btn btn-default" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="min-width:45px;height:34px;padding:0 0 0 0;color: #555 !important;background:white;">'+MonitorUnitUtil.getMonitorUnit(jklbh,"12")+'&nbsp;<span class="caret"></span></button>';
			}
			html +='<ul class="dropdown-menu dropdown-menu-right" style="min-width:45px;">';
	...
	if(zxway == 1) {
		...
		$("#"+parentId).find("input[type=checkbox]:checked").each(function(index,chkObj){//div下选中的监控项input
			...
			if(null == info.scz || "" == info.scz){
				...
				submitValidate(info.jklbh,info.scz,jklms,inputObj,zxway);
			}
...
//			}
			...
			if(!(null == info.scz || "" == info.scz)){
				submitValidate(info.jklbh,info.scz,jklms+",首次",sczInputObj,zxway);
			}
			if(!(null == info.zqz || "" == info.zqz)){
				submitValidate(info.jklbh,info.zqz,jklms+",周期",zqzInputObj,zxway);
			}
			...
			if(null == info.ydzQ || "" == info.ydzQ){
				...
				submitValidate(info.jklbh,info.ydzQ,jklms+",第一个容差值",ydzqInputObj,zxway);
			}
			...
			if(null == info.ydzH || "" == info.ydzH){
				...
				submitValidate(info.jklbh,info.ydzH,jklms+",第二个容差值",ydzHInputObj,zxway);
			}
		...
		for (var c = 3; c <= param.countNum; c++) {
			if($("#div"+c).length > 0) { //若果div存在
				...
				$("#"+parentId).find("input[type=checkbox]:checked").each(function(index,chkObj){//div下选中的监控项input
					...
					info.scz = inputObj.attr("value");//首次值
					if(null == info.scz || "" == info.scz){
						AlertUtil.showModalFooterMessage("请输入,第"+xcNum+"次,"+jklms+"!");
						if(info.jklbh != "1_10"){
							inputObj.focus();
						}
						inputObj.addClass("border-color-red");
						...
						submitValidate(info.jklbh,info.scz,"第"+xcNum+"次,"+jklms,inputObj,zxway);
					}
	...
	if((null == paramData || paramData.length <= 0) && this_.eoJkxSettingisValid){
		AlertUtil.showModalFooterMessage("请至少选择一个监控项!");
		this_.eoJkxSettingisValid = false; 
		return false;
	}
 ...
 * @returns
 */
function resRowNumber(){
	var len = $("span[name='jkxOrderNumber']").length;
	if (len >= 1){
		$("span[name='jkxOrderNumber']").each(function(index){
			$(this).html(index+2);
		});
	}
}
...
function autoCheckSetting(){
	$("input[name='jkxItems']:checked").each(function(index){
		var classVal = "monitorItem";
		var name = "fdjkxItems"+param.countNum;
		var jklbh = $(this).attr("jklbh");
		
		$("input."+classVal+"[name='"+name+"'][jklbh='"+jklbh+"']").click();
	});
}
 ...
 * 3：分段显示
 */
function showHideDashedBorder(way){
	if(way==3){
		$("div[name='danciChongfuDiv']").addClass("settingFd");
		$("p[name='danciChongfuP']").css("display", "block");
	}else{
		$("div[name='danciChongfuDiv']").removeClass("settingFd");
		$("p[name='danciChongfuP']").css("display", "none");
	}
}
 ...
 * @param value 值
 * @param jklms 描述
 * @param inputObj 当前文本框
 * @param zxway 1：单次 2：重复 3：分段显示
 */
function submitValidate(jklbh,value,jklms,inputObj,zxway){
	if("1_10" == jklbh){//日历
		if(zxway==1 || zxway==3){//校验是否是日期
			
		}else if(zxway==2){ //校验是否是数字
//			if(!validateNumber(value)){
//				AlertUtil.showModalFooterMessage(jklms+"必须为数字");
//				inputObj.addClass("border-color-red");
//				this_.eoJkxSettingisValid = false; 
//				return false;
//			}
		}
	}else if("2_10_FH" == jklbh){//飞行时间
		//校验是否是hh:mm格式
		if(!validateFH(value)){
			AlertUtil.showModalFooterMessage(jklms+"格式错误，小时:分钟");
			inputObj.addClass("border-color-red");
			this_.eoJkxSettingisValid = false; 
			return false;
		}
	}else if("3_10_FC" == jklbh){//飞行循环
		//校验是否是数字
		if(!validateNumber(value)){
			AlertUtil.showModalFooterMessage(jklms+"必须为数字");
			inputObj.addClass("border-color-red");
...
/**
 * FH:飞行时间校验，要满足hh:mm格式
 */
function validateFH(value){
	var this_ = this;
	var reg = /^(0|[1-9]\d*)(\:[0-5]?[0-9]?)?$/;
