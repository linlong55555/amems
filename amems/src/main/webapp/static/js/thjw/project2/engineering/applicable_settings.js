applicable_settings ={
	...
	appendHtml:function(list){
		...
		$.each(list,function(index,row){
		    ...
		    htmlContent += "<td name='szwz' style='text-align:left;vertical-align:middle;display:none'>"+StringUtil.escapeStr(row.szwz)+"</td>";
		    htmlContent += "</tr>";	
		});
	...
	getData : function(){
		...
		if (len > 0){
			...
			$("#applicable_settings_list").find("tr[id='contentTr']").each(function(){
				...
				infos.szwz = szwz;
				infos.xc = xc;
				infos.qrzt = "1";//确认状态：0未确认、1已确认
				infos.sgbs = "1";//手工标识：0否、1是
				param.push(infos);
			});
		}
		return param;
	...
	loadAllFj : function(){
		...
		if($("#qbsyInput").attr('checked')){ //全部适用加载所有的
			this_.clearData();
			var planData = choosePlaneModal.loadAllDatas(this_.param.dprtcode);
			var fjdata=[];
