...
var yjtsJb2 = '';
var yjtsJb3 = '';
var bulletinData = []; //批量审核数据

/**
 *加载js
 */
$(function(){
	Navigation(menuCode, '', '', 'GC-2-2 ', '林龙', '2017-08-02', '林龙', '2017-08-02');//加载导航栏
    initFjjxOption();				//初始化机型
    
    //执行待办
	if(todo_ywid) {
		Assessment_Audit_Modal.open(todo_ywid);
		todo_ywid = null;
	}
...
function decodePageParam(){
	TableUtil.resetTableSorting("auditTable");
	try{
...
function gatherSearchParam (){
	...
	if($("#bbList").prop("checked")==true){
		searchParam.bBbid="bBbid"; //该版本下一个版本id
	}
	var wjlx = $("#wjlx").val();
	var wjlxList=[];
	 if(wjlx != null){
		...
		AjaxUtil.ajax({
		   ...
		   success:function(data){
				...
				signByKeyword($("#keyword_search").val(), [4,7, 17,18, 20]);
	 			...
	 			hideBottom();
	 			new Sticky({tableId:'auditTable'}); //初始化表头浮动
	    }
  }); 	
}
...
function appendContentHtml(list){
		 ...
		 $.each(list,function(index,row){
			  choseAllIds.push(index);
				...
				htmlContent += "<td class='text-center fixed-column' ><input type='checkbox' index='"+index+"' name='checkrow'  onclick=\"checkRow("+index+",this)\"/></td>";
			    ...
			    if( row.zt ==2 ){
	    			...
	    			+ " permissioncode='project2:assessment:audit:main:01' onClick=\"Assessment_Audit_Modal.open('"+row.id+"')\" title='审核 Audit'></i>";//修改
			    }
...
function showHiddenContent(index,this_){
	var $checkbox1 = $("#auditTable :checkbox[name='checkrow']:eq("+index+")");
...
function batchApproval(idList,yj){
	...
	// 提交数据
	AjaxUtil.ajax({
		url:basePath+"/project2/assessment/audit/updateBatchAudit",
		type:"post",
		data:{
