	$(function(){
		 ...
		 $("#import_btn").remove();
		 // 加载维修方案版本
		 maintenancePlanApproval.reload();
		 // 初始化时间控件
		 maintenancePlanApproval.initDateWidget();
		 // 初始化技术评估单
		 maintenancePlanApproval.initEvaluation();
		 // 绑定事件
		 maintenancePlanApproval.addEvent();
		 
		//执行待办
			if(todo_ywid) {
				maintenancePlanApproval.showApprovalAgreeModal(todo_ywid);
				todo_ywid = null;
			}
		 
	 });
	
	var maintenancePlanApproval={
			data:[],
			wxfa:{},
			...
			AjaxGetDatasWithSearch:function(pageNumber,sortType,sequence){
				 ...
				 AjaxUtil.ajax({
					type: "post",
			 		url:basePath+"/project2/maintenancescheme/approvelist",
			...
			appendContentPlanHtml:function(list){
				...
				if(list.length > 0){
					$.each(list,function(index,row){
						htmlContent +="<tr id='"+row.id+"' class='cursor-pointer' onclick='maintenancePlanApproval.showBottomRightContent(this,\""+row.bb+"\")'>"
						htmlContent += "<td>";
						htmlContent += "<a href='#' style='text-decoration:none;' class='checkPermission' permissioncode='project2:maintenanceproject:approval:main:01' onclick='maintenancePlanApproval.showApprovalAgreeModal(\""+row.id+"\")' title='批准 Approve'><i class='icon-check'></i></a>&nbsp;";
						if(row.fBbid){
			...
			// 显示审批模态框
			showApprovalAgreeModal:function(id){
				...
				AjaxUtil.ajax({
					...
					success:function(wxfa){
						...
						$("#pfyj").val("");
						ModalUtil.showModal("approvalModal");
					}
				});
			...
			agree:function(){
				...
				AlertUtil.showConfirmMessage("确定要批准通过吗？", {callback: function(){
					
					startWait($("#approvalModal"));
					// 提交数据
					AjaxUtil.ajax({
						url:basePath+"/project2/maintenancescheme/approveagree",
						type:"post",
						data:JSON.stringify({
							id : wxfa.id,
							jx : wxfa.jx,
							pfyj : introduce_process_info_class.getData().shenpi,
						...
						contentType: "application/json;charset=utf-8",
						success:function(data){
							finishWait($("#approvalModal"));
							AlertUtil.showMessage("批准通过成功!");
			...
			reject:function(){
				...
				AlertUtil.showConfirmMessage("确定要批准驳回吗？", {callback: function(){
					
					startWait($("#approvalModal"));
					// 提交数据
					AjaxUtil.ajax({
						url:basePath+"/project2/maintenancescheme/approvereject",
						type:"post",
						data:JSON.stringify({
							id : wxfa.id,
							jx : wxfa.jx,
