	...
	var MaintenanceSchemeHistory={
			// 获取历史版本列表
			getHistoryData : function(id, func){
				var this_ = this;
				AjaxUtil.ajax({
					url:basePath+"/project2/maintenancescheme/versionlist",
			...
			buildHistoryHtml : function(data){
				...
				html += ['<thead>',
								'<tr>',
									'<th class="colwidth-15">',
									   '<div class="font-size-12 line-height-12">维修方案编号</div>',
							           '<div class="font-size-9 line-height-12">MP No.</div>',
								   	'</th>',
								   	'<th class="colwidth-5">',
				...
				if(data && data.length > 0){
					$.each(data, function(i, row){
						html += ['<tr>',
									'<td>',
										'<a href="javascript:void(0);" onclick="MaintenanceSchemeHistory.viewScheme(\''+row.id+'\')">'+StringUtil.escapeStr(row.wxfabh)+'</a>',
									'</td>',
									'<td><a href="javascript:void(0);" onclick="MaintenanceSchemeHistory.viewScheme(\''+row.id+'\')">R'+StringUtil.escapeStr(row.bb)+'</a></td>',
									'<td>'+(row.sjSxrq||"未生效").substr(0,10)+'</td>',
								'</tr>'].join('');
					});
				}
				html += '</tbody>';
