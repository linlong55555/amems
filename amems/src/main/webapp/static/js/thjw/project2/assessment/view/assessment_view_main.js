 ...
 */
$(function(){
	Navigation(menuCode, '', '', 'GC-2-5 ', '林龙', '2017-08-05', '林龙', '2017-08-05');//加载导航栏
...
function decodePageParam(){
	TableUtil.resetTableSorting("viewTable");
	try{
		...
		AjaxUtil.ajax({
		   ...
		   success:function(data){
				...
				signByKeyword($("#keyword_search").val(), [3,6,16, 17, 19]);
	 			...
	 			hideBottom();
	 			new Sticky({tableId:'viewTable'}); //初始化表头浮动
	    }
  }); 	
}
			  ...
			  choseAllIds.push(index);
				...
				htmlContent += "<td class='fixed-column text-center'>";
				   if(row.zt == 2 || row.zt == 3 || row.zt == 4 || row.zt == 10){ //状态为提交后，审核后和批准,完成后才可以关闭
				    	htmlContent += "<i class='icon-power-off color-blue cursor-pointer checkPermission' permissioncode='project2:assessment:view:main:01' "
...
function exportExcel(){
	...
	window.open(basePath+"/project2/assessment/TechnicalAssessment.xls?paramjson="+JSON.stringify(param));
