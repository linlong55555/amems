	...
	$.each(list,function(index,row){
		if (index % 2 == 0) {
		 ...
		 htmlContent = htmlContent + "<td class='text-center fixed-column'>" +
		   ...
		   "  onClick=edit('"+ row.id +"') title='修改 Edit' permissioncode='baseinfo:customer:main:edit'></i>&nbsp;&nbsp;" +
			"<i name='remove' class='icon-trash  color-blue cursor-pointer checkPermission' " +
			"permissioncode='baseinfo:customer:main:del' onClick=del('"+ row.id +"') title='删除Delete'></i></td>"; 
...
function edit(id) {
	...
	//	object.dprtcode = dprtcode;
	 ...
	 AjaxUtil.ajax({
			url: basePath+"/baseinfo/customer/getcustomerbyid",
			type: "post",
			contentType:"application/json;charset=utf-8",
...
function del(id) {
	AlertUtil.showConfirmMessage("确定要删除吗？", {callback: function(){
		 AjaxUtil.ajax({
			 url:basePath+"/baseinfo/customer/del/"+id,
			 type:"post",
			 contentType:"application/json;charset=utf-8",
			 dataType:"json",
			 modal:$("#AddEditAlertModal"),
...
function editObject() {
    ...
    obj.id = $("#customerid").val();
	  ...
	  startWait($("#AddEditAlertModal"));
	  AjaxUtil.ajax({
			url: basePath+"/baseinfo/customer/updatecustomer",
			type: "post",
			contentType:"application/json;charset=utf-8",
...
function addObject() {
	...
	startWait($("#AddEditAlertModal"));
	AjaxUtil.ajax({
		url: basePath + "/baseinfo/customer/addcustomer",
		type: "post",
		contentType:"application/json;charset=utf-8",
		data:JSON.stringify(obj),
		dataType:"json",
...
function exportExcel(){
	...
	var dprtcode = $("#dprtcode").val();
	window.open(basePath+"/baseinfo/customer/CustomerInfo.xls?dprtcode="
