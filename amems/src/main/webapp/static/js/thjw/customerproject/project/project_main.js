...
function AjaxGetDatasWithSearch(pageNumber,sortType,sequence) {
	 var searchParam = gatherSearchParam();
	    ...
	    startWait();
	    AjaxUtil.ajax({
			   url: basePath+"/customerproject/project/getprojectList",
			   type: "post",
	...
	$.each(list,function(index,row){
		if (index % 2 == 0) {
		 ...
		 htmlContent = htmlContent + "<td class='text-center fixed-column'>" +
		   "<i name='edit' class='icon-edit color-blue cursor-pointer checkPermission' permissioncode='customerproject:project:main:edit' " +
		   "  onClick=edit('"+ row.id +"') title='修改 Edit'></i>&nbsp;&nbsp;" +
			"<i  name='remove' class='icon-trash  color-blue cursor-pointer checkPermission' " +
			"permissioncode='customerproject:project:main:del'  onClick=del('"+ row.id +"') title='删除Delete'></i>"
		//   	"<i  name='detail' class='icon-trash  color-blue cursor-pointer checkPermission' " +
		//   	"permissioncode='customerproject:project:main:show'  onClick=showProject('"+ row.id +"') title='详细Detail'></i>"
		  +"</td>"; 
...
//				 trHtml = trHtml+'<i class="icon-edit color-blue cursor-pointer" onclick="#(1)" title="修改 "></i>&nbsp;&nbsp;';
			...
			trHtml = trHtml+"<td class='text-left'>"+StringUtil.escapeStr(data.attachments[0].wbwjm)
...
function editObject() {
	 ...
	 if (!oxt.f) {
	  ...
	  startWait($("#AddEditAlertModal"));
	  AjaxUtil.ajax({
			url: basePath+"/customerproject/project/updateproject",
			type: "post",
			contentType:"application/json;charset=utf-8",
...
function addProjectObject() {
	...
	startWait($("#AddEditAlertModal"));
	AjaxUtil.ajax({
		url: basePath + "/customerproject/project/addproject",
		type: "post",
		contentType:"application/json;charset=utf-8",
		data:JSON.stringify(obj),
		dataType:"json",
...
function del(id) {
	AlertUtil.showConfirmMessage("确定要删除吗？", {callback: function(){
		 AjaxUtil.ajax({
			 url:basePath+"/customerproject/project/del/"+id,
			 type:"post",
			 contentType:"application/json;charset=utf-8",
			 dataType:"json",
			 modal:$("#AddEditAlertModal"),
...
function showProject(id) {
	...
	window.open(basePath + "/customerproject/project/showProject?id=" + id);
...
function getProjectAircraftInfo() {
	...
	startWait($("#AddEditAlertModal"));
	AjaxUtil.ajax({
		url: basePath + "/customerproject/project/getprojectaircraft",
		type: "post",
		contentType:"application/json;charset=utf-8",
		data:JSON.stringify(obj),
		dataType:"json",
...
function exportExcel(){
	...
	window.open(basePath+"/customerproject/project/ProjectInfo.xls?dprtcode="
