/*var alertModalAdd='AddEditAlertModal';*/
//页面初始化
$(function(){

	//导航
	Navigation(menuCode, '', '', 'ZONE', '李高升', '2017-08-19', '李高升', '2017-08-19');
...
// 隐藏Modal时验证销毁重构
 ...
 function AjaxGetDatasWithSearch(pageNumber,sortType,sequence){
	    ...
	    var searchParam = gatherSearchParam();
		    ...
		    startWait();
		AjaxUtil.ajax({
		   url:basePath+"/basic/zone/getZoneStationList",
		   type: "post",
...
/* 表格拼接 */
 function appendContentHtml(list){
		...
		$.each(list,function(index,row){
			if (index % 2 == 0) {
			 ...
			 htmlContent = htmlContent + "<td class='text-center'><i name='edit' class='icon-edit color-blue cursor-pointer checkPermission' permissioncode='basic:zone:main:edit' jx='"+StringUtil.escapeStr(row.jx)+"' ms='"+StringUtil.escapeStr(row.ms)+"' zone='"+StringUtil.escapeStr(row.sz)+"'"	+ 
			   "  objId='"+StringUtil.escapeStr(row.id)+"'"+
			   "  dprtcode='"+StringUtil.escapeStr(row.dprtcode)+"'"+
			   "  dprtname='"+StringUtil.escapeStr(row.dprtname)+"'"+
			   "  onClick=edit(this) title='修改 Edit' permissioncode='basic:zone:main:edit'></i>&nbsp;&nbsp;" +
				"<i  name='remove' class='icon-trash  color-blue cursor-pointer checkPermission' permissioncode='basic:zone:main:del'  onClick=del('"+ row.id +"') title='删除Delete'></i></td>"; 
 ...
 function saveUpdate(){
	// 验证
	 ...
	 if(operation=="add"){
		 var url=basePath+"/basic/zone/add";
		 ...
		 var url=basePath+"/basic/zone/edit";
		 obj.id=objId;
		 submit(obj,url,operation);
	 }
...
/* 逻辑删除*/
 function del (id){
	 AlertUtil.showConfirmMessage("确定要删除吗？", {callback: function(){
		 AjaxUtil.ajax({
			 url:basePath+"/basic/zone/delete/"+id,
			 type:"post",
			 contentType:"application/json;charset=utf-8",
			 dataType:"json",
			 success:function(massge){
 ...
 /* 飞机机型初始化*/
	...
	function initImport(){
		// 初始化导入
	   importModal.init({
		    importUrl:"/basic/zone/excelImport",
		    downloadUrl:"/common/templetDownfile/14",
	...
	function exportExcel(){
		var searchParam = gatherSearchParam();
			window.open(basePath+"/basic/zone/zone.xls?dprtcode="
					+ encodeURIComponent(searchParam.dprtcode)+ "&lx=1" + "&keyword="
