...
function saveMainAnnunciate(obj){
	...
	// 提交数据
	AjaxUtil.ajax({
		url:basePath+"/project/annunciate/modifyAnnunciate",
		type: "post",
		contentType:"application/json;charset=utf-8",
		data:JSON.stringify(obj),
		dataType:"json",
...
function appendSelectChoosePgd(dataList){
	...
	$.each(param,function(index,data){
		...
		htmlContent = htmlContent +"<td class='text-left' title='"+data.shzltgh+"' style='vertical-align:middle'>"+StringUtil.escapeStr(data.shzltgh)+"</td>";
...
//	 trHtml = trHtml+'<i class="icon-edit color-blue cursor-pointer" onclick="#(1)" title="修改 "></i>&nbsp;&nbsp;';
		...
		/* htmlContent = htmlContent+'<td>'+row.yswjm+'</td>';
		 ...
		 htmlContent = htmlContent+"<td class='text-left'><a href=\"javascript:downloadfile('"+row.id+"')\">"+StringUtil.escapeStr(formatUndefine(row.wbwjm))+"</a></td>";
