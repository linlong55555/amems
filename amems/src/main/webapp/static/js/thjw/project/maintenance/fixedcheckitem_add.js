...
$(function() {
	Navigation(menuCode,"新增定检项目","Add");
	getFjjxByWxfabh();
	queryMaintenanceBbList();
	sumTotal();
	$("#rotatable").append("<tr  style='height:35px;'><td colspan='18' style='text-align:center;vertical-align:middle;'>暂无数据 No data.</td></tr>");
...
//获取维修方案版本,如果维修方案只有一个版本,则可以修改定检项目版本
function queryMaintenanceBbList(){
 	...
 	AjaxUtil.ajax({
 		...
 		type:"post",
 		async:false,
 		data:{'wxfabh' : wxfabh,'dprtcode':userJgdm},
 		dataType:"json",
 		success:function(data){
 			if(data != null && data.length == 1){
 				$("#bb").removeAttr("readonly");
 			}	
 		}
 	});
}
...
function save(){
	...
	if(Number(rwxfabb) < Number(bb)){
		AlertUtil.showErrorMessage("定检项目版本不能大于维修方案最大版本!");
		return false;
	}
	...
	if(pxh == "" || pxh == null){
		AlertUtil.showErrorMessage("排序号不能为空!");
		return false;
	}
	...
	//获取监控项目参数
	var monitorItemList = getMonitorItemParam();
	...
	// 提交数据
	AjaxUtil.ajax({
		url:basePath+"/project/fixedcheckitem/add",
		...
		success:function(data){
			finishWait();
			AlertUtil.showMessage('保存成功!','/project/fixedcheckitem/main?id='+data+'&wxfabh='+encodeURIComponent(wxfabh)+"&pageParam="+pageParam);
		}
	});
}
