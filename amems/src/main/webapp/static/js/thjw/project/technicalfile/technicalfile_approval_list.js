...
$(document).ready(function() {
	decodePageParam();
	// 菜单设置 自己设置
//	MenuUtil.activeMenu("project:technicalfile:mainapproval");
...
function AjaxGetDatasWithSearch(pageNumber, sortType, sequence) {
	...
	$
			.ajax({
				url : basePath
						+ "/project/technicalfile/queryTechnicalFileListApproval",
				type : "post",
	...
	$.each(list,function(index, row) {
					...
					if (index % 2 == 0) {
						...
						htmlContent = htmlContent
								...
								+ "<i class='icon-check color-blue cursor-pointer' onClick=\"edit('"
								+ row.id + "')\" title='批准 Approval'></i>" 
...
function edit(id) {
	if (checkUpdMt(id)) {
		window.location = basePath
				+ "/project/technicalfile/initApprovalFileUpload?id=" + $.trim(id)+"&pageParam="+encodePageParam();
	}
}
...
function checkUpdMt(id) {
	var flag = false;
	AjaxUtil.ajax({
		url : basePath + "/project/technicalfile/checkUpdMtApproval",
		type : "post",
		async : false,
		data : {
			'id' : id
...
function find(id,dprtcode) {
	window.open( basePath
			+ "/project/technicalfile/findApprovalFileUpload/"+$.trim(id)+"/"+$.trim(dprtcode)+"");
}
