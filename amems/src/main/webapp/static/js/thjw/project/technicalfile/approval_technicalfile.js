...
/*	for(var j=0;j<document.getElementById("ly").options.length;j++) 
	...
	//初始化日志功能
	if($('#technicalFileId').val()!=null && $('#technicalFileId').val()!=""){
...
function save(fixedCheckItem){
	// 提交数据
	AjaxUtil.ajax({
		url : basePath+"/project/technicalfile/modifyModifyFileApproval",
		...
		success:function(data){
			...
			if (data.state == "Success") {
				...
				//.close();
				AlertUtil.showMessage('批准通过成功!','/project/technicalfile/mainapproval?id='+fixedCheckItem.id+'&pageParam='+pageParam);
			} else {
...
function btnSave1(){
	...
	if(fixedCheckItem.pfyj=="" || fixedCheckItem.pfyj==null){
		AlertUtil.showErrorMessage("生产经理或授权委托人意见不能为空");
		return
	}
	startWait();
	
	AlertUtil.showConfirmMessage("您确定要批准驳回吗？",{callback: function(){
		submit(fixedCheckItem);
	}});
}
function submit(fixedCheckItem){
	// 提交数据
	AjaxUtil.ajax({
		url : basePath+"/project/technicalfile/modifyModifyFileApprovalBh",
		contentType:"application/json;charset=utf-8",
		type:"post",
		data:JSON.stringify(fixedCheckItem),
		dataType:"json",
		success:function(data){
			finishWait();//结束Loading
			if (data.state == "Success") {
				AlertUtil.showMessage('批准驳回成功!','/project/technicalfile/mainapproval?id='+fixedCheckItem.id+'&pageParam='+pageParam);
			} else {
				AlertUtil.showErrorMessage(data.text);
			}
		}
...
function btnSave2(){
	...
	AlertUtil.showConfirmMessage("您确定要中止(关闭)吗？",{callback: function(){
		submit1(fixedCheckItem);
	}});
}

function submit1(fixedCheckItem){
	// 提交数据
	AjaxUtil.ajax({
		url : basePath+"/project/technicalfile/modifyModifyFileApprovalStop",
		contentType:"application/json;charset=utf-8",
		type:"post",
		data:JSON.stringify(fixedCheckItem),
		dataType:"json",
		success:function(data){
			finishWait();//结束Loading
			if (data.state == "Success") {
				AlertUtil.showMessage('中止成功!','/project/technicalfile/mainapproval?id='+fixedCheckItem.id+'&pageParam='+pageParam);
			} else {
				AlertUtil.showErrorMessage(data.text);
			}
		}
...
function back(){
	window.location.href =basePath+"/project/technicalfile/mainapproval?pageParam="+pageParam;
}