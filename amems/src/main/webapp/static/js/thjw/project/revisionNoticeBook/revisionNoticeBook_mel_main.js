	...
	$(document).ready(function(){
			 ...
			 */
			 $('#add').on('click',function(){
				 window.location.href =  basePath+"/project/revisionNoticeBook/intoAddRevisionNoticeBook_mel?pageParam="+encodePageParam();
...
/*	function end(id,zlbh,zt) {
	 ...
	 function appendContentHtml(list,monitorsettings){
		 choseAllIds=[];
		   ...
		   $.each(list,function(index,row){
			   ...
			   if(row.zt==3){
				   htmlContent = htmlContent +"<i class='icon-remove-sign color-blue cursor-pointer checkPermission' permissioncode='project:revisionnoticebook:end' jszlh='"+row.jszlh+"' zdjsyy='' onClick=\"shutDown('"+row.id+"', this,'',true)\" title='指定结束 End'></i>&nbsp;&nbsp;";
			   }
			   if(row.zt==1){
	...
	/*		function clickRow(index,this_){
			...
			function add(){
				window.location = basePath+"/project/revisionNoticeBook/intoAddRevisionNoticeBook_mel?pageParam="+encodePageParam();
			}
			/**
			 * 打印
			 * @param id
