...
$(document).ready(function(){
	Navigation(menuCode,"审核MEL更改单","Audit");
	//加载已选择的技术评估单
	selectChoosePgd();
	initPlaneModel();
...
function agreedAudit(){
	...
	AlertUtil.showConfirmMessage("您确定要审核通过吗？", {callback: function(){
		...
		// 提交数据
		AjaxUtil.ajax({
			url:basePath+"/project/mel/agreedAudit",
			...
			success:function(data){
				finishWait();
				AlertUtil.showMessage('审核通过!','/project/mel/main?id='+id+'&pageParam='+pageParam);
			}
		});
...
function rejectedAudit(){
	...
	AlertUtil.showConfirmMessage("您确定要审核驳回吗？", {callback: function(){
		...
		// 提交数据
		AjaxUtil.ajax({
			url:basePath+"/project/mel/rejectedAudit",
			contentType:"application/json;charset=utf-8",
			type:"post",
			data:JSON.stringify(fixedCheckItem),
			dataType:"json",
			success:function(data){
				finishWait();
				AlertUtil.showMessage('审核驳回!','/project/mel/main?id='+id+'&pageParam='+pageParam);
			}
		});
