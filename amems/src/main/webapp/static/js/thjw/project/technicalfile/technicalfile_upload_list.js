...
function fjjxjz(){
	var dprtcode = $.trim($("#zzjg").val());
		   ...
		   $.each(list,function(index,row){
			   ...
			   if (index % 2 == 0) {
				   ...
				   htmlContent += "<tr bgcolor=\""+getWarningColor("#fefefe",row.syts,row.zt)+"\" >";
			   		...
			   		htmlContent = htmlContent + "<td class='text-center fixed-column'><i class='icon-print color-blue cursor-pointer '  onClick=\"printTechnicalApproval('"
				+ row.id + "')\" title='打印 Print'></i>&nbsp;&nbsp;";
			   if(row.zt==0|| row.zt==5 || row.zt==6){
				   htmlContent = htmlContent + "<i class='icon-edit color-blue cursor-pointer' onClick=\"edit('"
					+ row.id + "')\" title='修改 Edit'></i>&nbsp;&nbsp;";
			   }
			   if(row.zt==0|| row.zt==5 || row.zt==6){
				   htmlContent = htmlContent + "<i class='fa glyphicon glyphicon-trash color-blue cursor-pointer' onClick=\"Delete('"
					+ row.id + "')\" title='作废 Invalid'></i>&nbsp;&nbsp;";
			   }
			   if(row.zt==3){
				   htmlContent = htmlContent + "<i class='icon-remove-sign color-blue cursor-pointer' pgdh='"+row.pgdh+"' zdjsyy='' onClick=\"shutDown('"
				   +row.id+"',this,'',true)\" title='指定结束 End'></i>&nbsp;&nbsp;"; 
			   }
			   ...
			   if(row.zt==9){
				   ...
				   " onclick=\"shutDown('"+row.id+"',this,'"+row.zdjsrq+"',false)\">"+formatUndefine(zts[row.zt])+"</a></td>";			   }else{
	 ...
	 function edit(id){
		 if(checkUpdMt1(id)){
		 window.location =basePath+"/project/technicalfile/initModifyFileUpload?id="+id+"&pageParam="+encodePageParam();
		 }
	 }
...
function add(){
	window.location = basePath+"/project/technicalfile/intoAddTechnicalFileUpload?pageParam="+encodePageParam();
}
