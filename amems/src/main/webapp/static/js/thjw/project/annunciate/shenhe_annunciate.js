...
function appendPgd(){
	...
	$('input[name=pgd]:checked').each(function(){   
		...
		htmlContent = htmlContent + "<td><input type='hidden' name='PgdIdAndPgdh' value="+pgdId+","+StringUtil.escapeStr(pgdh)+"><input type='hidden' name='Ckzl' value="+StringUtil.escapeStr(ckzl)+"><a href=\"javascript:selectPgdBypgdh('"+p.pgdId+"')\">"+StringUtil.escapeStr(pgdh)+"</a></td>";
...
function submitMainAnnunciate(obj){
	...
	AjaxUtil.ajax({
		...
		success:function(data){
			if(obj.zt==2){
				AlertUtil.showMessage('审核成功!','/project/annunciate/main?id='+data+'&pageParam='+pageParam);
			}else if(obj.zt==5){
...
//跳转查看评估单
function selectPgdBypgdh(id){
	 window.location =basePath+"/project/technicalfile/findApprovalFileUpload/"+$.trim(id)+"";
