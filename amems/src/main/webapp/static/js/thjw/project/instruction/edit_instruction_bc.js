...
function appendPgd(){
	...
	$('input[name=pgd]:checked').each(function(){   
		...
		htmlContent = htmlContent + "<td class='text-center' style='vertical-align:middle'><input type='hidden' name='PgdIdAndPgdh' value="+pgdId+","+StringUtil.escapeStr(pgdh)+"><input type='hidden' name='Ckzl' value='"+StringUtil.escapeStr(ckzl)+"'><input type='hidden' name='Ckzl1' value='"+StringUtil.escapeStr(ckzl)+"'><a href=\"javascript:viewMainTechnicalfile('"+pgdId+"','"+dprtcode+"')\">"+StringUtil.escapeStr(pgdh)+"</a></td>";
...
function saveMaininstruction(obj){
	...
	// 提交数据
	AjaxUtil.ajax({
		url:basePath+"/project/instruction/updateMainInstruction",
		type: "post",
		contentType:"application/json;charset=utf-8",
		data:JSON.stringify(obj),
		dataType:"json",
		success:function(data){
			if(obj.zt==0){
				finishWait();
				AlertUtil.showMessage('保存成功!','/project/instruction/main?id='+data+'&pageParam='+pageParam);
			}else if(obj.zt==1){
				finishWait();
...
function appendSelectChoosePgd(dataList){
	...
	$.each(param,function(index,data){
		...
		htmlContent += "<td class='text-center' style='vertical-align:middle'>" +
				"<input type='hidden' name='PgdIdAndPgdh' value="+data.id+","+StringUtil.escapeStr(data.pgdh)+"><input type='hidden' name='Ckzl1' value='"+StringUtil.escapeStr(data.shzltgh)+"'>"+
...
function appendContent(dataList){
	...
	$.each(param,function(index,data){
		...
		htmlContent =htmlContent +"<td><textarea class='col-lg-12 col-sm-12 col-xs-12' style='height:40px' name='gznr' maxlength='300' >"+StringUtil.escapeStr(data.gznr)+"</textarea></td>"
...
function jxgcs(){
	...
	AjaxUtil.ajax({
		...
		success:function(data){
			...
			var jsrid=$("#instructionJsrid").val();
			$("#instructionJsrid").val("");
			var htmlContent="";
