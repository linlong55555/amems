...
$(function(){
	Navigation(menuCode,"复制工单","Modifly W/O");
	...
	if($("#gdlx").val()==1){
    	...
    	$("#fjzch").append("<option value='"+fjzch+"'>"+fjzch+"</option>");
		loadJkxm();
    }else{
    	$("#demo1").css("display", "none");  
    	$("#zxfl_demo1").css("display", "none");
...
function appendContentHtml(list){
	   ...
	   $.each(list,function(index,row){
		   ...
		   htmlContent = htmlContent + "<td>"+StringUtil.escapeStr(row.bb)+"</td>";  
	   ...
	   success:function(data){
	    finishWait();
		   if(data.total >0){
			   ...
			   // 标记关键字
			   signByKeyword($("#keyword_search2").val(),[2,4]);
		   } else {
			  $("#glgdCardlist2").empty();
			  $("#pagination3").empty();
...
function appendGlgd(){
	...
	$('input[name=wo_glgd]:checked').each(function(){  
		...
		htmlContent = htmlContent + "<td style='vertical-align:middle'class='text-left' name='gdbh'><input type='hidden' name='id' value="+xggdid+" />"+gdbh+"</td>";  
...
//		 trHtml = trHtml+'<i class="icon-edit color-blue cursor-pointer" onclick="#(1)" title="修改 "></i>&nbsp;&nbsp;';
		 ...
		 trHtml = trHtml+'<td class="text-left"><input type="hidden" name="fjid" value=\''+fileid+'\'/><input type="hidden" name="yswjm" value=\''+StringUtil.escapeStr(data.attachments[0].yswjm)+'\'><input type="hidden" name="wbwjm" value=\''+data.attachments[0].wbwjm+'\'>'+StringUtil.escapeStr(data.attachments[0].wbwjm)+'</td>';
