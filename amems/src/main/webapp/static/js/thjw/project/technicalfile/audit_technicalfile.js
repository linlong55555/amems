	...
	AjaxUtil.ajax({
		...
		success:function(data){
			...
			if (data.state == "Success") {
				AlertUtil.showMessage('审核通过成功!','/project/technicalfile/mainaudit?id='+fixedCheckItem.id+'&pageParam='+pageParam);
			} else {
...
function btnSave1(){
	...
	if(fixedCheckItem.shyj=="" || fixedCheckItem.shyj==null){
		AlertUtil.showErrorMessage("技术室主任意见不能为空");
		return
	}
	startWait();
...
function submit(fixedCheckItem){
	...
	AjaxUtil.ajax({
		...
		success:function(data){
			finishWait();//结束Loading
			if (data.state == "Success") {
				AlertUtil.showMessage('审核驳回成功!','/project/technicalfile/mainaudit?id='+fixedCheckItem.id+'&pageParam='+pageParam);
			} else {
				AlertUtil.showErrorMessage(data.text);
			}
		}
...
function back(){
	window.location.href =basePath+"/project/technicalfile/mainaudit?pageParam="+pageParam;
}
