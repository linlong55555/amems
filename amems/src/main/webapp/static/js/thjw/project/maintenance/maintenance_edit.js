...
$(function() {
	Navigation(menuCode,"修改维修方案","Edit");
	initPlaneModel();
	...
	var data = acAndTypeUtil.getACTypeList({DPRTCODE:$("#dprtcode").val()});
 	...
 	if(data.length >0){
		$.each(data,function(i,obj){
			if(obj.FJJX == $("#oldjx").val()){
				option += "<option value='"+StringUtil.escapeStr(obj.FJJX)+"' selected=true>"+StringUtil.escapeStr(obj.FJJX)+"</option>";
			}else{
...
function update(){
	...
	// 提交数据
	AjaxUtil.ajax({
		url:basePath+"/project/maintenance/edit",
		type:"post",
		data:{
			'id' : id,
			'wxfabh' : wxfabh,
