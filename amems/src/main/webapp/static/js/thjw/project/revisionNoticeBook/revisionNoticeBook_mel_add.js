...
$(document).ready(function(){
	Navigation(menuCode,"新增修订MEL","Add");
	...
	$('#xdqx').datepicker({
	  ...
	  $('#alertModal').on('hide.bs.modal', function () {
		window.location.href =  basePath+"/project/revisionNoticeBook/mel_main";
	  })
...
function jxhx(){
	...
	function edit(zt) {
	  ...
	  var obj = {
			  ...
			  "tzslx":2		//通知书类型1.维修方案 2.MEL 3.工单
	  };
		  ...
		  success:function(data) {							//请求成功后调用的回调函数
			  if(obj.zt==0){
				  if(data.state=="Failure"){
					  ...
					  AlertUtil.showMessage('保存成功!','/project/revisionNoticeBook/mel_main?id='+data.id+'&pageParam='+pageParam);
				  }
				}else if(obj.zt==1){
					if(data.state=="Failure"){
						  AlertUtil.showErrorMessage(data.message);
					  }else{
						  AlertUtil.showMessage('提交成功!','/project/revisionNoticeBook/mel_main?id='+data.id+'&pageParam='+pageParam);
					  }
					...
					if(data.state=="Failure"){
						  ...
						  AlertUtil.showMessage('操作成功!','/project/revisionNoticeBook/mel_main?id='+data.id+'&pageParam='+pageParam);
					  }
