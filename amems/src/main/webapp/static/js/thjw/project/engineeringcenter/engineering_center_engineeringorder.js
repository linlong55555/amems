...
Engineeringorder = {
	flag : true,
	id : "Engineeringorder",
	tbodyId : 'engineering_order_list',
	param: {
		pgdid:'' //评估单id
	},
	show : function(param){
	...
	loadData : function(pgdid){
		...
		AjaxUtil.ajax({
			async: false,
			url:basePath+"/project/engineering/queryByPgdId",
			type:"post",
			data:{pgdid:pgdid},
			dataType:"json",
			success:function(data){
				if(data != null && data.length > 0){
					...
					$("#"+this_.tbodyId).empty();
					$("#"+this_.tbodyId).append("<tr class='text-center'><td colspan=\"14\">暂无数据 No data.</td></tr>");
				}
			}
		});
	},
	appendTechnicalHtml : function(list){
		...
		$.each(list,function(index,row){
			...
			htmlContent = htmlContent + "<td title='"+StringUtil.escapeStr(row.gczlbh)+"' class='text-center fixed-column'><a href=\"javascript:"+this_.id+".viewMainEngineering('"+row.id+"','"+row.dprtcode+"')\">"+StringUtil.escapeStr(row.gczlbh)+"</a></td>";   
			   ...
			   htmlContent = htmlContent + "<td class='text-right'>"+StringUtil.escapeStr(row.bb)+"</td>";  
			   htmlContent = htmlContent + "<td title='"+StringUtil.escapeStr(row.xggzh)+"' class='text-left'>"+StringUtil.escapeStr(row.xggzh)+"</td>";  
			   htmlContent = htmlContent + "<td title='"+StringUtil.escapeStr(row.txyj)+"' class='text-left'>"+StringUtil.escapeStr(row.txyj)+"</td>";
			   if(row.isCfzxsx==1){
			   ...
			   if(row.zt==9){
				   		...
				   		htmlContent += "<td class='text-left'><a href='javascript:void(0);' djid='"+row.id+"' sqdh='"+StringUtil.escapeStr(row.gczlbh)+"' zdjsyy='"+StringUtil.escapeStr(row.zdjsyy)+"' zdjsr='"+zdjsr+"' zdjsrq='"+row.zdjsrq+"' onclick=\""+this_.id+".shutDown(this,false)\">"+formatUndefine(zts[row.zt])+"</a></td>";
				   }else{
	...
	formatPgdn : function(pgjg,dprtcode){
		...
		if(pgjg != null && pgjg != ""){
			...
			$.each(strs,function(i,s){
				...
				if(i == 1){
					htmlContent += "&nbsp;&nbsp;<i class='icon-caret-down cursor-pointer' onclick="+this_.id+".vieworhidePgdh(this)></i><div class='engineeringorderDisplayFile' style='display:none'>";
				}
	...
	vieworhideContentAll : function(){
		if (this.flag) {
			$(".engineeringorderDisplayFile").each(function(){
				$(this).fadeIn(500);
	...
	viewMainEngineering : function(id,dprtcode){
