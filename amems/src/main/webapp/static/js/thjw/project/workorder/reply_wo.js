...
$(function() {
	...
	Navigation(menuCode,"批准工单","Approved W/O");
...
//批准中止
function overdApprove(){
	var pfyj=$("#pfyj").val();
	if(pfyj==null||pfyj==""){
		AlertUtil.showErrorMessage("中止（关闭）意见不能为空！");
		return false;
	}else{
...
function rejectedApprove(){
	var pfyj=$("#pfyj").val();
	if(pfyj==null||pfyj==""){
		AlertUtil.showErrorMessage("批准驳回意见不能为空！");
		return false;
	}else{
...
function validateUpdate(zt){
	...
	// 提交数据
	AjaxUtil.ajax({
		url:basePath+"/project/workorder/Reply",
		...
		success:function(data){
			if(data>0){
				finishWait();
				if(zt==3){
					AlertUtil.showMessage('批准通过成功!','/project/workorder/main?id='+id+'&pageParam='+pageParam);
				}else if(zt==4){
					AlertUtil.showMessage('中止关闭成功!','/project/workorder/main?id='+id+'&pageParam='+pageParam);
				}else{
					AlertUtil.showMessage('批准驳回成功!','/project/workorder/main?id='+id+'&pageParam='+pageParam);
				}
			}else{
				finishWait();
