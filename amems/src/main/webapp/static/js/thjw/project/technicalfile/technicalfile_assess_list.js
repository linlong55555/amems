...
function AjaxGetDatasWithSearch(pageNumber, sortType, sequence) {
	...
	startWait();
	AjaxUtil.ajax({
				url : basePath+ "/project/technicalfile/queryTechnicalFileListAssess",
				type : "post",
	...
	$.each(
					...
					function(index, row) {
						...
						htmlContent = htmlContent
								...
								+ "<i class='icon-file-text-alt color-blue cursor-pointer' onClick=\"edit('"
								+ row.id + "')\" title='评估 Estimate'></i>&nbsp;&nbsp;<i class='icon-print color-blue cursor-pointer '  onClick=\"printTechnicalApproval('"
								+ row.id + "')\" title='打印 Print'></i>&nbsp;&nbsp;</td>";
						htmlContent = htmlContent
								+ "<td class='text-center fixed-column'><a href=\"javascript:find('"
								+ row.id + "','"+row.dprtcode+"')\">" + StringUtil.escapeStr(formatUndefine(row.pgdh))
								+ "</a></td>";
...
function edit(id) {
	if (checkUpdMt(id)) {
		window.location = basePath+ "/project/technicalfile/initAssessFileUpload?id=" + $.trim(id)+"&pageParam="+encodePageParam();
...
function checkUpdMt(id) {
	var flag = false;
	AjaxUtil.ajax({
		url : basePath + "/project/technicalfile/checkUpdMtAssess",
		type : "post",
		async : false,
		data : {
			'id' : id
...
function find(id,dprtcode) {
	window.open( basePath + "/project/technicalfile/findAssessFileUpload/"+$.trim(id)+"/"+$.trim(dprtcode)+"");
}
...
function appendChangeContent(data) {
	...
	var jxgcs = $('#uid').val();
	var i=$("option[value='"+jxgcs+"']","#jxgcs").length;
	if(i>0){
		$('#jxgcs').val(jxgcs);
	}
}
