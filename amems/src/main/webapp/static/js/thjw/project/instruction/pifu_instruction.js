...
//初始化日志功能
if(	$("#jg").val()=="1"){
	logModal.init({code:'B_G_004',id:$('#instructionId').val()});
}
});
//打开评估单列表对话框
function openPgd() {
...
function submitMainInstruction(obj){
	...
	// 提交数据
	AjaxUtil.ajax({
		url:basePath + "/project/instruction/submitpifuMainInstruction",
		type: "post",
		contentType:"application/json;charset=utf-8",
		data:JSON.stringify(obj),
		dataType:"json",
		success:function(data){
			if(obj.zt==3){
				AlertUtil.showMessage('批准成功!','/project/instruction/main?id='+data+'&pageParam='+pageParam);
			}else if(obj.zt==4){
				AlertUtil.showMessage('中止成功!','/project/instruction/main?id='+data+'&pageParam='+pageParam);
			}else if(obj.zt==6){
...
function updateJszt(obj){
	AjaxUtil.ajax({
		   url:basePath+"/project/instruction/updateJszt",
		   type: "post",
		   dataType:"json",
		   contentType:"application/json;charset=utf-8",
		   data:JSON.stringify(obj),
