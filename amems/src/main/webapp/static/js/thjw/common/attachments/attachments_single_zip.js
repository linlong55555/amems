...
attachmentsSingleZipUtil = {
		...
		initAttachment : function(id, attachment, fileType, isEdit, message){
			...
			if(isEdit){
				...
				if(attachment != null && attachment.wbwjm != null){
					t += "<a title='"+(StringUtil.escapeStr(attachment.wbwjm)+"."+attachment.hzm)+"' href='javascript:void(0);' onclick=attachmentsSingleZipUtil.downloadfile('"+attachment.id+"')>"+(StringUtil.escapeStr(attachment.wbwjm)+"."+attachment.hzm)+"</a>";
		 		}
		...
		createAttachment:function(attachmentid,wbwjm,hzm,id){
			var wbwjmStr = "";
 			...
 			wbwjmStr += "<a class='cursor-pointer' style='display:table;table-layout: fixed;' title='"+(StringUtil.escapeStr(wbwjm)+"."+hzm)+"'  href='javascript:void(0)' onclick=attachmentsSingleZipUtil.downloadfile('"+attachmentid+"')  ><div  style='display:table-cell;vertical-align: middle;word-break: break-all;word-wrap:break-word;padding-top:7px;'><label id='"+id+"_label'>"+(StringUtil.escapeStr(wbwjm)+"."+hzm)+"</label></div>";
 			wbwjmStr += "<div  style='display:none;vertical-align: middle;'><label  id='"+id+"_label_ellipsis' >...</label></div></a></div>" ;
 			wbwjmStr +=	"<div  style='display:table-cell;vertical-align: middle;padding-top:7px;'><i class='pull-left icon-remove-sign cursor-pointer color-blue cursor-pointer' style='font-size:15px;margin-left:4px;' title='删除 Delete' onclick=attachmentsSingleZipUtil.removeFile('"+id+"')></i></div>";
 			wbwjmStr += "</div></div>";
 			return wbwjmStr;
		},
...
//				uploadButtonClass: "ajax-file-upload_ext",
				...
				{
					var fileType = data.attachments[0].hzm.toUpperCase();
					if(fileType != 'ZIP'){
						AlertUtil.showModalFooterMessage("只能上传ZIP压缩包!");
						return;
					}
				...
				,onSubmit : function (files, xhr) {
					...
					if(wbwjm.length>0){
						if(/[<>\/\\|:"?*]/gi.test(wbwjm)){
			            	$('.wbwjmSingle').focus();
			            	AlertUtil.showModalFooterMessage("文件说明不能包含如下特殊字符 < > / \\ | : \" * ?");
