...
Tools_list_edit = {
	...
	initDataList : function(){
		...
		AjaxUtil.ajax({
			async: false,
			url:basePath+"/project2/materialtool/queryAllList",
	...
	addRow : function(obj){
		...
		tr += 	  "<input type='hidden' name='bjh' value='"+obj.bjh+"'/>";
		tr += 	  "<input type='hidden' name='hclx' value='"+obj.hclx+"'/>";
		tr += 	  "<input type='hidden' name='jhly' value='"+StringUtil.escapeStr(obj.jhly)+"'/>";
		tr += "</td>";
		tr += "<td title='"+DicAndEnumUtil.getEnumName('materialTypeEnum',obj.hclx)+"' style='text-align:left;vertical-align:middle;'>"+DicAndEnumUtil.getEnumName('materialTypeEnum',obj.hclx) +"</td>";
