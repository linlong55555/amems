...
function AjaxGetDatasWithSearch1(pageNumber,sortType,sequence){
	...
	AjaxUtil.ajax({
		   url:basePath+"/aerialmaterial/secondment/checkoutList",
		   type: "post",
		   ...
		   success:function(data){
			    ...
			    if(data.rows !=""){
					...
					// 标记关键字
					   signByKeyword($("#keyword_search1").val(),[2,3,4]);
					   
				   } else {
				
					  $("#list1").empty();
					  $("#pagination1").empty();
					  $("#list1").append("<tr><td class='text-center' colspan=\"10\">暂无数据 No data.</td></tr>");
				   }
			    //new Sticky({tableId:'jcwgh'});
	      }
	    }); 
...
function gatherSearchParam1(){
	 var searchParam = {};
	 searchParam.jddxlx = $("#jddxlx1").val();
...
//字段排序
function orderByloangiveback(obj, _obj) {
	$obj = $("#jcwgh th[name='column_"+obj+"']");
	var orderStyle = $obj.attr("class");
	$("#jcwgh .sorting_desc").removeClass("sorting_desc").addClass("sorting");
	$("#jcwgh .sorting_asc").removeClass("sorting_asc").addClass("sorting");
