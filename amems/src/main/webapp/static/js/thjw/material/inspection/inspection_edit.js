...
$(function(){
	...
	if($("#hclyone").val()!='' && $("#hclyone").val()!=null){
...
function validateInsert(zt){
    ...
    param.grn=$.trim($('#grn').val());  
    param.csn=$.trim($('#csn').val());  
    param.cso=$.trim($('#cso').val());  
    ...
    if(zt==2&&$("#jyjg").val()!=2){
    	...
    	materialHistory.ckid=$("#ckid").val();
    	materialHistory.cklb=$("#cklb").val();
    	materialHistory.ckh=$("#ckh").val();
    	materialHistory.kwid=$("#kwid").val();
    	...
    	materialHistory.csn=$("#csn").val();
    	materialHistory.cso=$("#cso").val();
    ...
    var paramsMap={};
    paramsMap.bjh=$("#bjh").val();
    paramsMap.pch=$("#pch").val();
    paramsMap.sn=$("#xlh").val();
	...
	if($('#gljb').val()==3){
		...
		var reg=/^[1-9]\d*$/;
		if(reg.test($("#kysl").val())==false&&$("#jyjg").val()!=2){
			AlertUtil.showErrorMessage("可用数请填写大于0整数！");
			return false;
		}
		if(reg.test($("#kysl").val())==false&&$("#jyjg").val()==2){
			AlertUtil.showErrorMessage("可用数请填写大于等于0整数！");
			return false;
		}
	}
	if($('#gljb').val()!=3){
		...
		if(reg.test($("#kysl").val())==false&&reg1.test($("#kysl").val())==false&&$("#jyjg").val()!=2){
			AlertUtil.showErrorMessage("可用数请填写大于0整数或小数！");
			return false;
		}
		if(reg.test($("#kysl").val())==false&&reg1.test($("#kysl").val())==false&&$("#jyjg").val()==2){
			AlertUtil.showErrorMessage("可用数请填写大于等于0整数或小数！");
			return false;
		}
	}
