...
$(document).ready(function(){
	...
	//初始化日志功能
	logModal.init({code:'D_01502',extparam:{GYSTYPE:'2'}});
	initImport();
});
...
function loadMonitorsettings() {
	AjaxUtil.ajax({
		url:basePath + "/aerialmaterial/outsourcingfirm/getByKeyDprtcode",
		type:"post",
		async: false,
		data:{
...
function AjaxGetDatasWithSearch(pageNumber,sortType,sequence){
	...
	startWait();
	AjaxUtil.ajax({
		url:basePath+"/aerialmaterial/outsourcingfirm/queryAllPageList",
		type: "post",
 ...
 function appendContentHtml(list){
	   ...
	   $.each(list,function(index,row){
		   ...
		   htmlContent += "<i class='icon-edit color-blue cursor-pointer checkPermission' permissioncode='aerialmaterial:outsourcingfirm:main:02' onClick=\"edit('"+ row.id + "','"+ row.dprtcode + "')\" title='修改 Edit'></i>&nbsp;&nbsp;";
		   htmlContent += "<i class='icon-trash color-blue cursor-pointer checkPermission' permissioncode='aerialmaterial:outsourcingfirm:main:03' onClick=\"del('"+ row.id + "')\" title='作废 Invalid'></i>";  
...
function selectById(id,obj){
 	AjaxUtil.ajax({
 		async: false,
 		url:basePath+"/aerialmaterial/outsourcingfirm/selectByPrimaryKey",
 		type:"post",
 		data:{id:id},
 		dataType:"json",
 		success:function(data){
...
function save(){
	...
	if(sqkssj != '' && sqjssj != '' && TimeUtil.compareDate(sqkssj,sqjssj)){
		AlertUtil.showErrorMessage("授权开始日期不能大于授权截至日期!");
		return false;
	}
	...
	var url = basePath+"/aerialmaterial/outsourcingfirm/edit";
	if($.trim($("#id").val()) == null || $.trim($("#id").val()) == ''){
		url = basePath+"/aerialmaterial/outsourcingfirm/add";
	}
...
function del(id){
	AlertUtil.showConfirmMessage("确定要作废吗？", {callback: function(){
		...
		startWait();
		AjaxUtil.ajax({
			url:basePath + "/aerialmaterial/outsourcingfirm/cancel",
			type:"post",
			async: false,
			data:{
				'id' : id
...
function initImport(){
    importModal.init({
	    importUrl:"/aerialmaterial/outsourcingfirm/excelImport",
...
function exportExcel(){
	...
	window.open(basePath+"/aerialmaterial/outsourcingfirm/OutsourcingSupplier.xls?dprtcode="
				+ dprtcode 
				+ "&gyslb="+gyslb
