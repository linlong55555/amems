...
$(function() {
	Navigation(menuCode,"修改合同","Edit");//初始化导航
...
function changeContractType(){
	...
	if (len >= 1) {
		if($("#materialTable").find("td").length !=1){
			if(!confirm("切换合同类型将清空航材列表，确定要切换吗？")){
				$("#htlx").val(type == 1?2:1);
				return;
			}
		}
...
//保存
function save(){
	var url = basePath+"/aerialmaterial/contract/editSave";
	var message = "保存成功!";
	performAction(url,message);
}
//提交
function submit(){
	var url = basePath+"/aerialmaterial/contract/editSubmit";
	var message = "提交成功!";
	performAction(url,message);
}
...
function performAction(url,message){
	...
	param.id = id;
	param.contractDetailIdList = oldDetailArr;
	...
	AjaxUtil.ajax({
		...
		success:function(data){
			finishWait();
			AlertUtil.showMessage(message,'/aerialmaterial/contract/main?id='+id+'&pageParam='+pageParam);
		}
	});
}
	...
	MaterialContractModal.show({
		...
		callback: function(data){//回调函数
			if(data != null && data.length > 0){
				...
				var bjIdList = [];
				$.each(data,function(index,row){
					if(row.BJID != null && row.BJID != ""){
						bjIdList.push(row.BJID);
					}
				});
				var procurementCatalogList = [];
				if(bjIdList.length > 0){
					procurementCatalogList = getProcurementCatalog(bjIdList);
				}
				$.each(data,function(index,row){
					var bjIdList = [];
					bjIdList.push(row.BJID);
...
function addRow(obj){
	...
	var tr = "";
		...
		if(type == 1){
			...
			tr += "<td>";
			tr += "<input name='htSl' type='text' value='"+formatUndefine(obj.HTSL)+"' class='form-control' placeholder='' onkeyup='clearNoNumTwo(this)' />";
			tr += "</td>";
			tr += "<td>";
			...
			tr += "<td title='"+formatUndefine(obj.SN)+"' style='text-align:center;vertical-align:middle;'>"+formatUndefine(obj.SN)+"</td>";
			tr += "<td>";
			tr += "<input name='bjClf' type='text' value='"+formatUndefine(obj.BJ_CLF)+"' class='form-control' placeholder='' onkeyup='clearNoNumTwo(this)' />";
			tr += "</td>";
			tr += "<td>";
...
function removeCol(obj){
	...
	var id = $(obj).next().val();
	if ('' != id && null != id) {
		var message = "确定删除该合同上的提订单吗？";
		if($("#htlx").val() != 1){
			message = "确定删除该合同上的送修单吗？";
		}
