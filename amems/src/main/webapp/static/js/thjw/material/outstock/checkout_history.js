$(document).ready(function(){
	...
	Navigation(menuCode,"借出出库","Lending Outstock");
...
function selectUser(){
	userModal.show({
		...
		callback: function(data){//回调函数
			...
			$("#inventorysqbmid").val(formatUndefine(data.bmdm));
	 ...
	 function putoutstorage(){
		...
		if($("#jddx").val()==""||$("#jddx").val()==null){
			...
			return;
				...
				if (len > 0){
					$("#detailTBody").find("tr").each(function(){
						...
						var bjid =$("input[name='bjid']").eq(index).val(); //当前行，隐藏件号的值
		 ...
		 AlertUtil.showConfirmMessage("你确定要出库吗？",{callback: function(){
				...
				// 提交数据
				AjaxUtil.ajax({
					url : basePath+"/aerialmaterial/outstock/checkoutstockRemoval",
					contentType:"application/json;charset=utf-8",
					type:"post",
					data:JSON.stringify(fixedCheckItem),
					dataType:"json",
