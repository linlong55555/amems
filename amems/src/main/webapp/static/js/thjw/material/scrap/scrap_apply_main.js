	 ...
	 function appendContentHtml(list){
		 ...
		 $.each(list,function(index,row){
			   ...
			   htmlContent += ["<tr bgcolor='"+(index % 2 == 0 ? "#f9f9f9" : "#fefefe")+"' onclick='showScrapLoadingList(\""+row.id+"\",\""+row.bfdh+"\",this)' style='cursor:pointer'>",
			                   ...
			                   ((row.zt==1||row.zt==5)?"<i class='icon-edit color-blue cursor-pointer checkPermission' permissioncode='aerialmaterial:scrap:apply:edit' onClick='goToEditPage(\""+row.id+"\")' title='修改 Edit'></i>&nbsp;&nbsp;":""),
			                   (row.zt==1?"<i class='icon-trash color-blue cursor-pointer checkPermission' permissioncode='aerialmaterial:scrap:apply:scrap' onClick='scrap(\""+row.id+"\",\""+row.bfdh+"\")' title='作废 Scrap'></i>":""),
			                   ((row.zt==2||row.zt==5)?"<i class='icon-remove-sign color-blue cursor-pointer checkPermission' permissioncode='aerialmaterial:scrap:apply:close' onClick='closed(\""+row.id+"\",\""+row.bfdh+"\")' title='指定结束 Close'></i>":""),
			                   "</td>",
...
function downloadfile(id){
	...
	function goToEditPage(id){
		window.location = basePath+"/aerialmaterial/scrap/edit/"+id+"?pageParam="+encodePageParam();
	}
	 ...
	 * 作废
	 */
	function scrap(id, bfdh){
		AlertUtil.showConfirmMessage("是否确认作废报废单号为"+bfdh+"数据？",{callback:function(){
			startWait();
			AjaxUtil.ajax({
				url:basePath+"/aerialmaterial/scrap/scrap",
				type:"post",
				data:JSON.stringify({
					id : id
				}),
	...
	function endDetail(id){
		...
		AssignEndModal.show({
			...
			jsr:StringUtil.escapeStr(row.zdjsrid),//指定结束人
			jssj:row.zdjsrq,//指定结束时间
			jsyy:row.zdjsyy,//指定结束原因
