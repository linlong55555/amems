...
$(function() {
	Navigation(menuCode,"新增航材提订单","Add");//初始化导航
...
//保存
function save(){
	var url = basePath+"/aerialmaterial/reserve/addSave";
	var message = "保存成功!";
	performAction(url,message);
}
//提交
function submit(){
	var url = basePath+"/aerialmaterial/reserve/addSubmit";
	var message = "提交成功!";
	performAction(url,message);
}
...
function performAction(url,message){
	...
	AjaxUtil.ajax({
		...
		success:function(data){
			finishWait();
			AlertUtil.showMessage(message,'/aerialmaterial/reserve/manage?id='+data+'&pageParam='+pageParam);
		}
	});
}
