...
$(document).ready(function(){
	Navigation(menuCode,"其他飞行队借入申请查看","View Other Aerocade Borrow Apply");
		//数据字典
	 ...
	 $('#example27').multiselect({
		...
		//向table新增一行
		 function addRow(obj){
		 	...
		 	var tr = "";
		 		...
		 		tr +=  "<td style='text-align:center;vertical-align:middle;' >"+StringUtil.escapeStr(obj.bjh)+"</td>";
		 		...
		 		if(StringUtil.escapeStr(obj.sn) != ""){
		 			tr += "<td class='text-left'>"+StringUtil.escapeStr(obj.sn)+"</td>"; 
				}else{
					tr += "<td class='text-left'>"+StringUtil.escapeStr(obj.pch)+"</td>"; 
				}
		...
		//移除一行
		 function removeCol(e){
		 	...
		 	if(len < 1){
		 		$("#reserveTable").append("<tr><td  colspan='16'  class='text-center'>暂无数据 No data.</td></tr>");
		 	}
		 }
		 ...
		 function selectUser(){
		 	userModal.show({
		 		...
		 		callback: function(data){//回调函数
		 			...
		 			$("#ckbmid").val(formatUndefine(data.deptId));
		 		}
		 	});
		 }	
		 ...
		 function selectUser1(){
		 	userModal.show({
		 		...
		 		callback: function(data){//回调函数
		 			...
		 			$("#inventorymckmid").val(formatUndefine(data.deptId));
