...
$(function() {
	...
	$("#reserveTable input:visible").each(function(){
		var content;
		if(this.type == "text"){
			content = $(this).val();
	...
	$("#reserveTable select:visible").each(function(){
...
function onchagekw(){
	$("#reserveTable").find("tr").each(function(){
		...
		if(kwh!=''){
			$("select[name='kwh']:eq("+index+")").val(kwh);
		}
	});
...
function performAction(url,message){
	...
	if($("#kccb").val()==""||$("#kccb").val()==null){
			AlertUtil.showErrorMessage("库存成本不能为空!");
			return false;
	}
	...
	if(parseFloat(num)>parseFloat($("#rksl").val())||parseFloat(num)<parseFloat($("#rksl").val())){
		AlertUtil.showErrorMessage("输入的数量和比如等于入库数量!");
		return false;
	}
	...
	AjaxUtil.ajax({
		...
		success:function(data){
			finishWait();
			AlertUtil.showMessage(message,'/aerialmaterial/godown/main?id='+data+'&pageParam='+pageParam);
		}
	});
}
