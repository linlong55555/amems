...
$(document).ready(function(){
	Navigation(menuCode,"手动领用出库","Manual Outstock");
...
function changeOrganization(){
	var dprtcode = $.trim($("#dprtcodeNum").val());
	var planeRegOption = '<option value="" >请选择 Choice</option><option value="00000" >通用Currency</option>';
...
function selectUser1(){
	userModal.show({
		...
		callback: function(data){//回调函数
			...
			$("#inventorymckmid").val(formatUndefine(data.id));
	...
	//只能输入数字和小数
	 function go(){
		 window.location = basePath+"/aerialmaterial/outstock/main?pageParam="+pageParam+"t=" + new Date().getTime()+"#inventory";
	 }
	...
	//删除
	 ...
	 function putoutstorage(){
		 if($("#fjzch_search").val()==null||$("#fjzch_search").val()==""){
			 ...
			 return;
				...
				var fixedCheckItem = {
						...
						fjzch :$.trim($('#fjzch_search').val()),//飞机注册号
				...
				// 提交数据
				AjaxUtil.ajax({
					url : basePath+"/aerialmaterial/outstock/manualstockRemoval",
