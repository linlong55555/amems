	...
	$(function(){
		...
		$("#installation_certificate_modal").on("shown.bs.modal", function() {
	...
	var CertificateUtil = function(param_){
		var modalId = "installation_certificate_modal";
		// 证书数据
		var data = [];
		// 当前证书数据
		var current = {};
		...
		function init(){
			...
			jQueryName("common_certificate_addBtn").off("click").on("click", function(){
				modify();
			});
		...
		function appendContentHtml(list){
			...
			$.each(attachments,function(index,row){
				...
				if(row.id){
					htmlContent += "<a href='javascript:void(0);' onclick='PreViewUtil.handle(\""+row.id+"\", PreViewUtil.Table.D_011)' >"+fj+"</a>";
				}else{
					htmlContent += fj;
				}
				htmlContent += '</td>';
