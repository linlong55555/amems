	...
	function showUpdateModal(btn, date){
		...
		startWait();
		AjaxUtil.ajax({
		   url:basePath+"/produce/scheduling/aircraft/loadPlaneScheduling",
		   type: "post",
	...
	function showPlaneViewModal(btn){
		...
		AjaxUtil.ajax({
		   url:basePath+"/produce/scheduling/aircraft/queryPlaneSchedulingInPlaneView",
		   type: "post",
		   contentType:"application/json;charset=utf-8",
		   dataType:"json",
		   data:JSON.stringify({
		    	 ...
		    	 if(dayList.length == 0){
		    		 ...
		    		 if(list.length == 24){
		    			 ...
		    			 $("#planeViewBody").append([
								...
								'	</div>',
								'</td>',
								'<td title="'+StringUtil.escapeStr(dayList[0].whr.displayName)+'">'+StringUtil.escapeStr(dayList[0].whr.displayName)+'</td>',
								'<td>'+dayList[0].whsj+'</td>',
								'</tr>'].join(""));
		    		 }
		    	 }
	...
	function refreshSchedulingTable(){
		...
		startWait();
		AjaxUtil.ajax({
		   url:basePath+"/produce/scheduling/aircraft/queryPlaneScheduling",
		   type: "post",
		   contentType:"application/json;charset=utf-8",
		   dataType:"json",
		   data:JSON.stringify({
		...
		function assembleThead(){
			 $("#schedulingTable").append([
   		  	 		...
   		  	 		'		<span class="first-span">日期</span>',
   		  	 		'		<span class="second-span">飞机</span>',
   		  	 		'	</div>',
   		  	 		'</th>',
   		  	 		assembleTh(),
   		  	 		'</tr>',
	...
	function save(){
		...
		startWait();
		AjaxUtil.ajax({
		   url:basePath+"/produce/scheduling/aircraft/savePlaneScheduling",
		   type: "post",
		   contentType:"application/json;charset=utf-8",
		   dataType:"json",
	...
	function cancel(){
		startWait();
		AjaxUtil.ajax({
		   url:basePath+"/produce/scheduling/aircraft/cancelPlaneScheduling",
		   type: "post",
		   contentType:"application/json;charset=utf-8",
		   dataType:"json",
		   data:JSON.stringify({
