...
function AjaxGetDatasWithSearch(pageNumber,sortType,sequence){
	...
	AjaxUtil.ajax({
	   url:basePath+"/produce/reservation/fault/queryMonitorAllPageList",
	   type: "post",
	 ...
	 $.each(list,function(index,row){
			...
			if( row.zt == 1 || row.zt == 6){
				htmlContent += "<i class=' icon-edit color-blue cursor-pointer checkPermission' " 
					+ " permissioncode='produce:reservation:fault:monitor:02' onClick=\"FailureKeep_Update_Modal.open('"+row.id+"')\" title='修改 Edit'></i>&nbsp;&nbsp;";//修改
				htmlContent += "<i class='fa glyphicon glyphicon-trash color-blue cursor-pointer checkPermission' permissioncode='produce:reservation:fault:monitor:03' " 
	    			+ " onClick=\"invalid('"+ row.id + "','"+ row.zt +"')\" title='删除 Delete'></i>&nbsp;&nbsp;"; 
			}
			if(row.zt == 2){
				htmlContent += "<i class=' icon-check color-blue cursor-pointer checkPermission' " 
					+ " permissioncode='produce:reservation:fault:monitor:04' onClick=\"FailureKeep_Approval_Modal.open('"+row.id+"')\" title='批准 Approvel'></i>&nbsp;&nbsp;";//批准
			}
			if( row.zt == 4 ){
				if(row.zcblbs==0){
					htmlContent += "<i class='iconnew-keep color-blue cursor-pointer checkPermission' " 
						+ " permissioncode='produce:reservation:fault:monitor:05'  onClick=\"again_keep_open.open('"+row.id+"')\" title='再次保留 Again Keep'></i>&nbsp;&nbsp;";//再次保留
				}
//				htmlContent += "<i class='iconnew-create color-blue cursor-pointer checkPermission' " 
//				+ " permissioncode='produce:reservation:fault:monitor:06' onClick=\"createNrc('"+row.id+"','"+row.bs145+"')\" title='创建NRC Create NRC'></i>&nbsp;&nbsp;";//创建nrc
				htmlContent += "<i class='icon-power-off color-blue cursor-pointer checkPermission' " 
			    	...
			    	+ "gzz=''   gzrq='' " 
			    	+ "jcz=''   jcrq='' " 
					+ " permissioncode='produce:reservation:fault:monitor:07' onClick=\"shutDown('"+row.id+"',this,true)\" title='关闭 Close'></i>&nbsp;&nbsp;";//完成
			}
			if( row.zt == 2 || row.zt == 4 ){
				htmlContent += "<i class='iconnew-end color-blue cursor-pointer checkPermission' " 
					...
					+ " permissioncode='produce:reservation:fault:monitor:08' onClick=\"zdjs('"+row.id+"',this,true)\" title='指定结束 Close'></i>&nbsp;&nbsp;";//指定结束
			}
		    htmlContent += "</td>";
		    ...
		    if( row.zt !==9 && row.zt !==10 && row.isWarning == 1){ //状态为指定结束-完成才可以改版
		    	...
		    	htmlContent += "<td class='fixed-column text-center '></td>";
		    }
		    ...
		    htmlContent += "<td style='text-align:center;' style='text-align:left;vertical-align:middle;'>"+showPJInfo(row.scblqxList, 'scblqx'+row.id)+"</td>";
		    htmlContent += "<td class='text-center' style='text-align:left;vertical-align:middle;'>"+formatUndefine(row.zcSqrq).substring(0,10)+"</td>";
		    htmlContent += "<td style='text-align:center;' style='text-align:left;vertical-align:middle;'>"+showPJInfo(row.zcblqxList, 'zcblqx'+row.id)+"</td>";
...
function filseDown(id){
	FilesDownModal.show({
		 djid:id,//单据id
		 code:"produce:reservation:fault:monitor:09",
		 callback: function(data){//回调函数
			 if(null != data && data != ''){
...
function getWarningColor(bgcolor){
	...
	return "<td  class='fixed-column text-center '></td>";*/
