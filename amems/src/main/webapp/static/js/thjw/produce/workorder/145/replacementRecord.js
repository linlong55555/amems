replacementRecord = {
		...
		init:function(param){
			...
			if(this.param.isShowed){
				...
				$("#"+this.param.parent+" #"+this.id+"_list").append("<tr id='replacementRecord_nodata'><td class='text-center' colspan=\"11\">暂无数据 No data.</td></tr>");			
			}else{
			...
			if(id == null || id == undefined){
				this_.param.index++;
					...
					htmlContent +=
						...
						"</td>";	
				    htmlContent += "<td name='cxbjh' title='"+StringUtil.escapeStr(data.cxBjh)+"' style='text-align:left;vertical-align:middle;'>"+StringUtil.escapeStr(data.cxBjh)+"</td>";
				    htmlContent += "<td name='cxxlh' title='"+StringUtil.escapeStr(data.cxXlh)+"' style='text-align:left;vertical-align:middle;'>"+StringUtil.escapeStr(data.cxXlh)+"</a></td>";
				    htmlContent += "<td name='cxmc' title='"+StringUtil.escapeStr(data.cxBs == 1 ? ((data.cxBjywmc||"") + " " + (data.cxBjzwmc||"")) : "")+"' style='text-align:left;vertical-align:middle;'>"+StringUtil.escapeStr(data.cxBs == 1 ? ((data.cxBjywmc||"") + " " + (data.cxBjzwmc||"")) : "")+"</td>";
				    ...
				    htmlContent += "<td name='czwz' title='"+DicAndEnumUtil.getEnumName('installedPositionEnum', ((data.cxj==null?"":data.cxj.wz)||(data.zsj==null?"":data.zsj.wz)|| "11"))+"' style='text-align:center;vertical-align:middle;'>"+DicAndEnumUtil.getEnumName('installedPositionEnum', ((data.cxj==null?"":data.cxj.wz)||(data.zsj==null?"":data.zsj.wz)|| "11"))+"</td>";	   
