...
function AjaxGetDatasWithSearch(pageNumber,sortType,sequence){
	...
	AjaxUtil.ajax({
	   url:basePath+"/produce/reservation/item/queryAllPageList",
	   type: "post",
	   ...
	   success:function(data){
			...
			signByKeyword($("#keyword_search").val(), [3]);
				...
				$("#pagination").empty();
				$("#ProjectKeep_list").append("<tr><td class='text-center' colspan=\"21\">暂无数据 No data.</td></tr>");
 			}
	 ...
	 $.each(list,function(index,row){
			...
			if( row.zt == 1 ){
				htmlContent += "<i class=' icon-edit color-blue cursor-pointer checkPermission' " 
					+ " permissioncode='produce:reservation:item:main:02' onClick=\"Project_Update_Modal.open('"+row.id+"')\" title='修改 Edit'></i>&nbsp;&nbsp;";//修改
				htmlContent += "<i class='fa glyphicon glyphicon-trash color-blue cursor-pointer checkPermission' permissioncode='produce:reservation:item:main:03' " 
	    			+ " onClick=\"invalid('"+ row.id + "','"+ row.zt +"')\" title='删除 Delete'></i>&nbsp;&nbsp;"; 
			}
			if( row.zt == 2){
				if(row.gdblzt==1){
					htmlContent += "<i class='iconnew-execution color-blue cursor-pointer checkPermission' " 
						+ " permissioncode='produce:reservation:item:main:04' onClick=\"reExecute('"+row.id+"','"+ row.zt +"')\" title='重新执行Re-execute'></i>&nbsp;&nbsp;";//创建nrc
					htmlContent += "<i class='iconnew-end color-blue cursor-pointer checkPermission' " 
						...
						+ "gbczsj=''   gbrid='' " 
						+ " permissioncode='produce:reservation:item:main:05' onClick=\"zdjs('"+row.id+"',this,true)\" title='指定结束 Close'></i>&nbsp;&nbsp;";//指定结束
				}
				if(row.gdblzt==2){
					htmlContent += "<i class='icon-power-off color-blue cursor-pointer checkPermission' " 
						...
						+ "gzz=''   gzrq='' " 
						+ "jcz=''   jcrq='' " 
						+ " permissioncode='produce:reservation:item:main:06' onClick=\"shutDown('"+row.id+"',this,true)\" title='关闭 Close'></i>&nbsp;&nbsp;";//完成
				}
			}
		    htmlContent += "</td>";
...
function filseDown(id){
	FilesDownModal.show({
		 djid:id,//单据id
		 code:"produce:reservation:item:main:07",
...
function exportExcel(){
	...
	window.open(basePath+"/produce/reservation/item/ItemReservation.xls?dprtcode="
 				+ dprtcode 
 				+ "&fjzch="+fjzch+"&ztList="+ztList
 				+ "&sqBeginDate="+sqBeginDate+"&sqEndDate="+sqEndDate
 				+ "&blBeginDate="+blBeginDate+"&blEndDate="+blEndDate
