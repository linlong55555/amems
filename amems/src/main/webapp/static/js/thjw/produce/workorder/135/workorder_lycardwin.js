 ...
 * 135工单，来源任务号，来源任务NRC
 */
Workorder135LycardWin = {
	id:'workorder135_lycard_Modal', //窗口ID
	baseId:"workorder135_lycard", //基础ID
	tbodyId:'', //列表ID
	...
	getParams : function(){
		...
		var paramsMap = {};
		paramsMap.keyword = $("#workorder135_lycard_keyword_search").val();
		paramsMap.widType = "135"; //135工单的来源工卡
		paramsMap.fjzch = StringUtil.escapeStr($("#wo135add_fjzch").val())==""?"-":$("#wo135add_fjzch").val(); //飞机注册号
	...
	clear : function(){
		var this_ = this;
		if(this_.param.multi){
			$("#workorder135_lycard_list :checkbox[name='"+this_.id+"_list_checkbox']").attr("checked", false);
		}else{
			$("#workorder135_lycard_list :radio[name='"+this_.id+"_list_checkbox']").attr("checked", false);
		}
		this_.save();
	},
	/**
