...
var workorder145_main = {
	...
	loadFjList:function(){
		...
		AjaxUtil.ajax({
			...
			success : function(data) {
				...
				fjList += '<option value="-1" >N/A</option>';
				...
				.ajax({
					...
					success : function(data) {
						...
						if (data.rows.length > 0) {
							...
							// 标记关键字
							signByKeyword($.trim($("#wo145search_keyword").val()), [ 3,9, 15,17 ], "#workorder145_list tr td");
	...
	appendContentHtml : function(list) {
		...
		$.each(list,function(index, row) {
			...
			htmlContent += "<i class='icon-print color-blue cursor-pointer checkPermission' permissioncode='produce:nrc145:main:08' onClick=workorder145_main.print('"+ row.id +"','"+row.gdlx+ "',event) title='打印 Print'></i>&nbsp;&nbsp;";
			/*
			...
			if (row.zt != "9" && row.zt != "10") { // 9、10以外状态,可编辑
				htmlContent += "<i class='icon-edit color-blue cursor-pointer checkPermission' permissioncode='produce:nrc145:main:02' onClick=workorder145_main.openWinEdit('"+ row.id+ "',true,event) title='修改 Edit'></i>&nbsp;&nbsp;";
			}
			if (row.zt == "1") { // 状态为保存,可删除
				htmlContent += "<i class='icon-trash color-blue cursor-pointer checkPermission' permissioncode='produce:nrc145:main:03' onClick=workorder145_main.openWinDel('"+ row.id+ "',true,event) title='删除 Delete'></i>&nbsp;&nbsp;";
			}
			if (row.zt == "2" || row.zt == "7") { // 状态为提交、生效下发,可完工反馈
				htmlContent += "<i class='iconnew-feedback color-blue cursor-pointer checkPermission' permissioncode='produce:nrc145:main:04' onClick=workorder145_main.openWinFeedback('"+ row.id+ "',true,event)  title='完工反馈'></i>&nbsp;&nbsp;";
			}
			if((row.zt == "2" ||row.zt == "7") && row.xmblbs==0){ // 状态为提交、生效下发且项目保留标识=0,可指正常完成
				htmlContent += "<i class='icon-remove-sign color-blue cursor-pointer checkPermission' permissioncode='produce:nrc145:main:05' onClick=workorder145_main.openWinWGClose('"+ row.id+ "',true,event)  title='完工关闭'></i>&nbsp;&nbsp;";
			}
			if (row.zt == "2" || row.zt == "7") { // 状态为提交、生效下发,可指定结束
				htmlContent += "<i class='iconnew-end color-blue cursor-pointer checkPermission' permissioncode='produce:nrc145:main:06' onClick=workorder145_main.openWinZDClose('"+ row.id+ "',true,event)  title='指定结束'></i>&nbsp;&nbsp;";
			}
			if (row.zt == "7" && (row.fjzch !="" && row.fjzch != null && row.fjzch !="-" ) && row.xmblbs==0) { // 状态为生效,可进行,工作项目保留
				htmlContent += "<i class='iconnew-projectKeep color-blue cursor-pointer checkPermission'  permissioncode='produce:reservation:item:main:01'  onClick=workorder145_main.openWinView('"+ row.id+ "',5)  title='工作项目保留'></i>&nbsp;&nbsp;";
			}
			if (row.zt == "9" || row.zt == "10") { // 9、10状态,可编辑
				htmlContent += "<i class='fa  fa-thumb-tack color-blue cursor-pointer checkPermission' permissioncode='produce:nrc145:main:07' onClick=workorder145_main.openWinXDClose('"+ row.id+ "',true,event)  title='修订 Revised'></i>&nbsp;&nbsp;";
			}
			htmlContent += "</td>";
			htmlContent += "<td title='"+ StringUtil.escapeStr(row.lyrwh)+ "'>"+ "<a href=javascript:workorder145_main.openWinView('"+ row.lyrwid+ "','"+ row.gdlx+ "'); class='text-left ' style='margin-right:5px;' >"+ StringUtil.escapeStr(row.lyrwh) + "</a>"+ "</td>";
			...
			htmlContent += "<td title='"+ StringUtil.escapeStr(row.gdbh)+ "'>"+ "<a href=javascript:workorder145_main.openWinView('"+ row.id+ "',3); class='text-left ' style='margin-right:5px;' >"+ StringUtil.escapeStr(row.gdbh) + "</a>"+ "</td>";
			...
			htmlContent += "<td title='"+ StringUtil.escapeStr(row.gkbh)+ "'>"+ "<a href=javascript:workorder145_main.openWinView('"+ row.gkid+ "',7); class='text-left ' style='margin-right:5px;' >"+ StringUtil.escapeStr(row.gkbh) + "</a>"+ "</td>";
			...
			htmlContent += "<td title='"+ StringUtil.escapeStr(row.paramsMap.gbbh)+ "'>"+ "<a href=javascript:workorder145_main.openWinView('"+ row.gbid+ "',4); class='text-left ' style='margin-right:5px;' >"+ StringUtil.escapeStr(row.paramsMap.gbbh)+ "</a>" + "</td>";
	...
	getWorkOrderButtonList : function(obj) {// 获取操作信息
		...
		if (zt == "7") { // 状态为生效,可进行,工作项目保留
			...
			str += "<a href='javascript:void(0);' onClick=workorder145_main.openWinView('"+ id + "',5)><i class='iconnew-projectKeep color-blue cursor-pointer'></i>&nbsp;工作项目保留</a>";
			str += "</p>";
		}
		str += "</div>";
		return str;
