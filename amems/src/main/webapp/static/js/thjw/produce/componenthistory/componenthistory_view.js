$(function(){
	Navigation(menuCode, '查看部件履历', 'View', 'SC-1-2 ', '孙霁', '2017-09-25', '孙霁', '2017-09-25');//加载导航栏
...
var componenthistory={
		...
		AjaxGetDatasWithSearch:function(pageNumber,sortType,sequence){
			 ...
			 AjaxUtil.ajax({
				 url:basePath+"/produce/componenthistory/queryAllBj",
				   type: "post",
		...
		appendContentHtml:function(list){
			...
			$.each(list,function(index,row){
			    ...
			    htmlContent += "<td style='text-align:center;vertical-align:middle;' title='"+StringUtil.escapeStr(row.paramsMap.czrq)+"'>"+StringUtil.escapeStr(row.paramsMap.czrq)+"</td>"; 
			    htmlContent += "<td style='text-align:left;vertical-align:middle;' title='"+DicAndEnumUtil.getEnumName('storageTypeEnum',row.paramsMap.czlx)+"'>"+DicAndEnumUtil.getEnumName('storageTypeEnum',row.paramsMap.czlx)+"</td>"; 
...
function viewywdj(id,czlx){
	if(czlx == 1 || czlx == 2){//1装上、2拆下
		...
		window.open(basePath+"/aerialmaterial/godown/view?id="+id);
	}else if(czlx == 20 || czlx == 21 || czlx == 22 || czlx == 23 || czlx == 24){//20出库 21领用出库 22送修出库 23归还出库 24借调出库
...
function viewsjbj(str,dprtcode){
	if(str){
		var obj = str.split(" ");
		window.open(basePath+"/produce/componenthistory/view?pageParam=" + encodePageParam(obj[0],obj[1],dprtcode));
	}
}
