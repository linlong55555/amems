	...
	var flb = {
		...
		initFlighRecord : function(){
			...
			$("#bottom_hidden_content [name='rydw']>select").replaceWith('<input class="form-control input-sm" disabled="disabled" type="text">');
			...
			if(data.total > 0){
				$.each(data.rows,function(index,row){
					...
					if(row.zt == 1 || row.zt == 2 || row.zt == 12){
						htmlContent +="<i class='icon-edit color-blue cursor-pointer checkPermission' permissioncode='produce:flb2:main:02,produce:flb2:main:03,produce:flb2:main:04,produce:flb2:main:05' onclick='flb_detail.show(\""+row.id+"\")' title='修改 Edit'></i>";
					}
//					if(row.zt == 1){
...
//					}
		...
		// 查看飞行记录单
		view : function(id){
			window.open(basePath + "/produce/flb2/view?id=" + id);
		},
