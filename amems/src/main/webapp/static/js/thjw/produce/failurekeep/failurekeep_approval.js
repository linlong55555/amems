FailureKeep_Approval_Modal = {
	...
	initInfo : function(param){
		...
		$(".div-hide").show();   			        			//隐藏标识
		FailureKeep_Open_Modal.button(4);						//初始化按钮权限
	...
	setDate : function(data){
		...
		approval_process_info.show({    
		 	status:2,			  				  //：1保存、2提交、4已批准、6审批驳回、9关闭、10完成
	 ...
	 */
	passed : function(){
		var url="/produce/reservation/fault/passed";//批准通过
		var str="批准通过";//审核通过
	 ...
	 */
	turnDown : function(){
		var url="/produce/reservation/fault/turnDown";//批准驳回
		var str="批准驳回";
		this.subjectFrom(str,url);
	},
	/**
	...
	subjectFrom : function(str,url){
		...
		obj.id=$("#id").val();
		obj.scPzyj=$.trim(data.shenpi);
		obj.fjzch=$.trim($("#FailureKeep_Open_Modal_fjzch").val()); //状态
		obj.scPzrq=$.trim(data.approvedDate);
		obj.dprtcode = $("#dprtcode").val(); //组织机构
		obj.bldh=$.trim($("#bldh").val());//保留单号
