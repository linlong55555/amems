...
Workorder145AddWin = {
	...
	show : function(param){
		...
		$("input:radio[name='wo145add_gdlx']").change(function(){
			...
			this_.lyrwbt='';
			this_.gkbt='';
			var gdlx = $("input:radio[name='wo145add_gdlx']:checked").val();
			if(gdlx == '4' || gdlx == '5'){
...
//		}
	...
	openWinLyrwh : function(){
		...
		var chkLy = $('input[name="wo145add_gdlx"]:checked').val(); //来源:   1RTN（维修项目）、2EO、3NRC、9FLB
		var title=chkLy==1?"维修项目":chkLy==2?"EO":chkLy==4?"MO/MCC":chkLy==5?"其它指令":"";
		var titleENG=chkLy==1?"Maintenance Item":chkLy==2?"EO":chkLy==4?"MO/MCC":chkLy==5?"Other":"";
		Workorder145NrcWin.show({
			...
			callback : function(data) {//回调函数
				if(null != data ){
					...
					$("#wo145add_gdbt").val(data.gdbt);
					this_.calRwmsCount();
					this_.lyrwbt=data.gdbt;
				}else{
					$("#wo145add_lyrwid").val("");
					$("#wo145add_lyrwh").val("");
					$("#wo145add_gdbt").val("");
				...
				$("#wo145add_gkbh").val("");//清空来源工卡编号
				$("#wo145add_gkid").val("");//清空来源工卡id
				$('#workorder145_form').data('bootstrapValidator').updateStatus('wo145add_gdbt','NOT_VALIDATED', null).validateField('wo145add_gdbt');
			}
		});
		...
		Workorder145LycardWin.show({
			...
			callback : function(data) {//回调函数
				if(null != data){
					...
					$("#wo145add_gdbt").val(this_.lyrwbt);
				}
				$('#workorder145_form').data('bootstrapValidator').updateStatus('wo145add_gdbt','NOT_VALIDATED', null).validateField('wo145add_gdbt');
			}
		});
		...
		Workpackage145Win.show({
			...
			callback : function(data) {//回调函数
				if(null != data){
					...
					$("#wo145add_gbjx").val(data.fjjx);//机型
				...
				$('#workorder145_form').data('bootstrapValidator').updateStatus('wo145add_gbbh','NOT_VALIDATED', null).validateField('wo145add_gbbh');
