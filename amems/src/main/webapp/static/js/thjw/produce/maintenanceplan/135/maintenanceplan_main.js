$(function(){
			Navigation(menuCode, '', '', 'SC6-1', '刘兵', '2017-10-19', '刘兵', '2017-10-19');//初始化导航
...
var maintencePlanList={
	...
	appendWPHtml: function(list){
		...
		var keyword = $.trim($("#keyword_wp_search", $("#"+this.id)).val());
		var htmlContent = '';
		if(fjzch != '' && fjzch != '-' && keyword == ''){
			htmlContent += "<tr id='9' >";
			htmlContent += "<td class='text-center' colspan='2'>";
			htmlContent += "<a href='javascript:void(0);' onclick="+this_.id+".initWorkOrder("+9+")>航线排故</a>";
			htmlContent += "</td>";
			htmlContent += "<td></td>";
			htmlContent += "</tr>"; 
		}
		$.each(list,function(index,row){
			...
			var ztValue = row.zt==7?" 已下发":" 未下发";
			var gbbhTitle = StringUtil.escapeStr(row.gbbh) + ztValue;
			var gbbh = "<a href='javascript:void(0);' onclick="+this_.id+".viewWorkpackage('"+row.id+"')>"+StringUtil.escapeStr(row.gbbh)+"</a>" + ztValue;
			var fjzch = "A/C : " + StringUtil.escapeStr(row.fjzch);
			...
			htmlContent += "</td>";
			htmlContent += "<td>";
			htmlContent += "<p title='"+fjzch+"' class='margin-bottom-0' style='height:18px;line-height:18px;'>";
			htmlContent += fjzch;
			htmlContent += "</p>";
			htmlContent += "<p title='"+gbbhTitle+"' class='margin-bottom-0' style='height:18px;line-height:18px;'>";
			htmlContent += gbbh;
			htmlContent += "</p>";
			htmlContent += "<p title='"+StringUtil.escapeStr(row.gbmc)+"' class='tag-set margin-bottom-0' style='height:18px;line-height:18px;'>"+StringUtil.escapeStr(row.gbmc)+"</p>";
			htmlContent += "</td>";
			
			htmlContent += "<td class='text-center'>";
			
			htmlContent += " <button type='button' qtid='"+row.id+"' zt='"+row.zt+"' class='btn btn-xs btn-primary attachment-gb-view'  data-toggle='dropdown'>操作";
			htmlContent += " <span class='caret'></span>";
			htmlContent += " </button>";
			/*htmlContent += '<i  class="attachment-gb-view color-blue cursor-pointer "  style="font-size:15px">操作</i>';*/
	...
	setWPNoData : function(){
		var this_ = this;
		$("#gb_list_tbody", $("#"+this_.id)).empty();
		$("#gb_list_tbody", $("#"+this_.id)).append("<tr><td colspan=\"3\" class='text-center'>暂无数据 No data.</td></tr>");
	},
	initWebuiPopoverGb : function(){//初始化
		var this_ = this;
		WebuiPopoverUtil.initWebuiPopover('attachment-gb-view',"body",function(obj){
			return this_.getPackageButtonList(obj);
		},80);
		$("#gb_table_top_div").scroll(function(){
			$('.attachment-gb-view').webuiPopover('hide');
		});
	},
	getPackageButtonList : function(obj){//获取历史附件列表
		var this_ = this;
		var id = $(obj).attr('qtid');
		var zt = $(obj).attr('zt');
		var str = "<div class='button-group-new'>";
		if(zt == 2){
			str += "<p class='margin-bottom-0 checkPermission' permissioncode='produce:workpackage:main:04'>";
			str += "<a href='javascript:void(0);' onclick="+this_.id+".issued('"+id+"')>下发</a>";
			str += "</p>";
			str += "<p class='margin-bottom-0 checkPermission' permissioncode='produce:workpackage:main:07'>";
			str += "<a href='javascript:void(0);' onclick="+this_.id+".endClose('"+id+"')>指定结束</a>";
			str += "</p>";
		}
		if(zt == 7){
			str += "<p class='margin-bottom-0 checkPermission' permissioncode='produce:workpackage:main:05'>";
			str += "<a href='javascript:void(0);' onclick="+this_.id+".doWgfk('"+id+"')>完工反馈</a>";
			str += "</p>";
			str += "<p class='margin-bottom-0 checkPermission' permissioncode='produce:workpackage:main:06'>";
			str += "<a href='javascript:void(0);' onclick="+this_.id+".closeData('"+id+"')>完工关闭</a>";
			str += "</p>";
			str += "<p class='margin-bottom-0 checkPermission' permissioncode='produce:workpackage:main:07'>";
			str += "<a href='javascript:void(0);' onclick="+this_.id+".endClose('"+id+"')>指定结束</a>";
			str += "</p>";
	...
	issued : function(id){
		...
		/*if(row.zt == 2){
		htmlContent += "<i class='iconnew-issued color-blue cursor-pointer checkPermission' permissioncode='project2:bulletin:02' onClick=\""+this_.id+".issued('"
			+ row.id + "','" + row.zt + "','" + row.dprtcode + "')\" title='下发 Issued'></i>&nbsp;&nbsp;<i class='iconnew-end color-blue cursor-pointer checkPermission' permissioncode='project2:bulletin:04' onClick=\""+this_.id+".endClose('"
				+ row.id + "')\" title='指定结束 End'></i>&nbsp;&nbsp;";
	}
	if(row.zt == 7){
		htmlContent += "<i class='iconnew-feedback color-blue cursor-pointer checkPermission' permissioncode='project2:bulletin:02' onClick=\""+this_.id+".doWgfk('"
		+ row.id + "','" + row.zt + "','" + row.dprtcode + "')\" title='完工反馈 Feedback'></i>&nbsp;&nbsp;";
		htmlContent += "<i class='icon-remove-sign color-blue cursor-pointer checkPermission' permissioncode='project2:bulletin:04' onClick=\""+this_.id+".closeData('"
				+ row.id + "')\" title='完工关闭  Close'></i>&nbsp;&nbsp;<i class='iconnew-end color-blue cursor-pointer checkPermission' permissioncode='project2:bulletin:04' onClick=\""+this_.id+".endClose('"
				+ row.id + "')\" title='指定结束 End'></i>&nbsp;&nbsp;";
	}*/
	},
	/**
		...
		this_.getDataById(id,function(obj){
			...
			workpackage_end_modal.init({
				...
				callback:function(data){
					this_.doRequest(data,"指定结束成功!",basePath + "/produce/workpackage/endClose","workpackage_end_modal");
				}
			});
		});
	},
	...
	closeData : function(id){
		...
		this_.getDataById(id,function(obj){
			...
			workpackage_close_modal.init({
				...
				operation:true,		
				callback:function(data){
					this_.doRequest(data,"完工关闭成功!",basePath + "/produce/workpackage/close","workpackage_close_modal");
				}
			});
		});
	},
	...
	initWorkOrder : function(id){
		...
		var gbbh = $("#" + id, $("#gb_list_tbody")).attr("gbbh");
		this.searchGds(gbbh);
		this.searchGd();
	},
	/**
...
//				$("#gb_sm", $("#"+this_.gddiv)).html("");
				if(data.total >0){
					...
					// 标记关键字
					signByKeyword($("#keyword_gd_search", $("#"+this_.gddiv)).val(),[3,4,9,17],"#maintenanceplan_list tr td");
				} else {
					this_.setNoData();
				}
				new Sticky({tableId:'maintenanceplan_table'});
	...
	getGdParams : function(){//将搜索条件封装 
		...
		var wxlx = $.trim($("#wxlx_search", $("#"+this.gddiv)).val());
		searchParam.dprtcode = userJgdm;
		searchParam.fjzch = fjzch;
		paramsMap.keyword = keyword;
		paramsMap.userId = userId;
	...
	appendContentHtml:function(list){
		...
		$.each(list,function(index,row){
			...
			var haveZgd = false;
			gbbh = paramsMap.gbbh;
			htmlContent += "<tr class='cursor-pointer tr-bold' jksjid='"+StringUtil.escapeStr(row.jksjid)+"' gdid='"+row.id+"' fjzch='"+row.fjzch+"' gdbh='"+StringUtil.escapeStr(row.gdbh)+"' id='"+row.id+"' onclick='maintencePlanList.showHiddenContent(this)' >";
			...
			if(row.zt == 2){
				htmlContent += "<i class='spacing iconnew-end color-blue cursor-pointer checkPermission' permissioncode='produce:workorder:main:06' onclick="+this_.id+".openWinZDClose('"+ row.id + "') title='指定结束 End'></i>";
			}
			if(row.zt == 7){
				...
				htmlContent += "<i class='spacing icon-remove-sign color-blue cursor-pointer checkPermission' permissioncode='produce:workorder:main:05' onclick="+this_.id+".openWinWGClose('"+ row.id + "') title='完工关闭  Close'></i>";
				htmlContent += "<i class='spacing iconnew-end color-blue cursor-pointer checkPermission' permissioncode='produce:workorder:main:06' onclick="+this_.id+".openWinZDClose('"+ row.id + "') title='指定结束 End'></i>";
			}
			if(row.zt == 9 && row.gdlx != 9){
				htmlContent += "<i class='fa  fa-thumb-tack color-blue cursor-pointer checkPermission' permissioncode='project2:bulletin:08' onclick="+this_.id+".openWinRevision9('"+ row.id + "') title='修订 Revision'></i>";
			}
			if(row.zt == 10 && row.gdlx != 9){
				htmlContent += "<i class='fa  fa-thumb-tack color-blue cursor-pointer checkPermission' permissioncode='project2:bulletin:08' onclick="+this_.id+".openWinRevision10('"+ row.id + "') title='修订 Revision'></i>";
			}
...
//			}
		...
		var gb_sm = gbbh+"工包：工单总数"+list.length+",已反馈"+yfkCount+",已关闭"+ygbCount;
//		$("#gb_sm", $("#"+this_.gddiv)).html(gb_sm);
	 
	 	$("#maintenanceplan_list", $("#"+this_.gddiv)).empty();
		$("#maintenanceplan_list", $("#"+this_.gddiv)).html(htmlContent);
			...
			Workorder135ZDCloseWin.show({
				...
				callback : function(data) {//回调函数
					if (data != null) {
						...
						this_.performAction(url, data, message, "workorder135_zdclose_Modal", function(obj){
							if(obj){
			...
			Workorder135WGCloseWin.show({
				...
				callback : function(data) {//回调函数
					if (data != null) {
						var message = '提交成功!';
						var url = basePath+"/produce/workorder/doWGRevision";
						this_.performAction(url,data,message,"workorder135_wgclose_Modal", function(obj){
							if(obj){
								this_.goPage(1,"auto","desc");
								refreshPermission();
							}
...
//		htmlContent += "<p title='"+StringUtil.escapeStr(paramsMap.ckh)+"' class='tag-set margin-bottom-0'>"+StringUtil.escapeStr(paramsMap.ckh)+"</p>";
	...
	setNoData : function(){
		...
		$("#maintenanceplan_list", $("#"+this_.gddiv)).append("<tr><td colspan=\"23\" class='text-center'>暂无数据 No data.</td></tr>");
	},
	initGkWebuiPopover : function(){//初始化工卡
		...
		$("#maintenanceplane_workorder_div").scroll(function(){
	...
	showHiddenContent:function(this_){
		...
		if($(".displayContent").is(":visible")){
	 	...
	 	$("#maintenanceplane_workorder_div").css("display","none");
		$("#maintenanceplane_workorder_div").css("display","block");
		this.selectTR = id;
		$("#"+id).addClass('bg_tr').siblings().removeClass('bg_tr');
		App.resizeHeight();
		if(!isBottomShown){
			$("#maintenanceplane_workorder_div").animate({scrollTop:this_.offsetTop-43},"slow");
//			TableUtil.makeTargetRowVisible($(this_), $("#maintenanceplane_workorder_div"), -20);
		}
	},
	/*显示和隐藏*/
	toggleMaintenancePlan:function(obj){
		...
		if(i.hasClass("icon-caret-left")){
			...
			$("#left_div").hide();
			$("#right_div").removeClass("col-sm-new-78 col-xs-12").addClass("col-sm-12 col-xs-12");
		}else{
			i.removeClass("icon-caret-right").addClass("icon-caret-left");
			$("#left_div").show();
			$("#right_div").removeClass("col-sm-12 col-xs-12").addClass("col-sm-new-78 col-xs-12");
		}
		App.resizeHeight();
	...
	initfjzch : function(){//飞机注册号
		...
		if(planeDatas != null && planeDatas.length >0){
			...
			planeRegOption += "<option value='-' msn='' >"+"N/A"+"</option>";
		}else{
	...
	searchreset : function(){
		...
		//关闭状态
		$("#gbzt_search option:first", $("#"+this.gddiv)).prop("selected", 'selected');  
		...
		//维修类别
		$("#wxlx_search option:first", $("#"+this.gddiv)).prop("selected", 'selected');  
		//关键搜索框
	...
	openMaterialToolDetail : function(){
		var this_ = this;
		if($("#maintenanceplan_list",$("#"+this_.gddiv)).find("tr input:checked").length > 0){
			this_.setParam("maintenanceplan_list", this_.gddiv, "3");
		...
		if($(obj).hasClass("fa-angle-down")){
			...
			this_.queryByGdsbid(gdsbid, function(gdList){
				...
				$.each(gdList,function(index,row){
					...
					if(row.zt == 9 && row.gdlx != 9){
						htmlContent += "<i class='fa  fa-thumb-tack color-blue cursor-pointer checkPermission' permissioncode='produce:workorder:main:07' onclick="+this_.id+".openWinRevision9('"+ row.id + "') title='修订 Revision'></i>";
					}
					if(row.zt == 10 && row.gdlx != 9){
						htmlContent += "<i class='fa  fa-thumb-tack color-blue cursor-pointer checkPermission' permissioncode='produce:workorder:main:07' onclick="+this_.id+".openWinRevision10('"+ row.id + "') title='修订 Revision'></i>";
					}
...
function customResizeHeight(bodyHeight, tableHeight){
    ...
    if($(".displayContent").is(":hidden")){
    	...
    	if($table.length > 0){
		 ...
		 var maintenancePlaneWorkOrder=$("#maintenanceplane_workorder_div").outerHeight()||0;
...
function workorder2feddback(){
	...
	$("#workorder_feedback_Modal").modal("show");
}
