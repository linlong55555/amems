...
$(function(){
	...
	logModal.init({code:'B_S2_012'});//初始化日志功能
	
	//执行待办
    if(todo_ywid){
    	if(todo_jd == 1 || todo_jd == 5 || todo_jd == 6){
    		Defectkeep_Update_Modal.open(todo_ywid);
    	}else if(todo_jd == 2){
    		Defectkeep_Approval_Modal.open(todo_ywid);
    	}else{
    		Defectkeep_Add_Modal.open();
    	}
    	todo_ywid = null;
    }
    else if(todo_lyid) {
...
function decodePageParam(){
	TableUtil.resetTableSorting("defrctkeepTable");
	try{
...
function gatherSearchParam (){
	...
	//保留日期
	paramsMap.xfBeginDate=xfBeginDate;
	paramsMap.xfEndDate=xfEndDate;
	
	paramsMap.sqr=$.trim($("#sqr1").val());				//申请人
...
function AjaxGetDatasWithSearch(pageNumber,sortType,sequence){
	...
	AjaxUtil.ajax({
	   url:basePath+"/produce/reservation/defect/queryAllPageList",
	   type: "post",
	   ...
	   success:function(data){
 			...
 			// 标记关键字
			signByKeyword($("#keyword_search").val(), [3,6]);
 			}else{
	 			$("#defrctkeep_list").empty();
				$("#pagination").empty();
				$("#defrctkeep_list").append("<tr><td class='text-center' colspan=\"20\">暂无数据 No data.</td></tr>");
 			}
 			new Sticky({tableId:'defrctkeepTable'}); //初始化表头浮动
	   }
  }); 	
}
	 ...
	 $.each(list,function(index,row){
			...
			if( row.zt == 1 || row.zt == 6){
				htmlContent += "<i class=' icon-edit color-blue cursor-pointer checkPermission' " 
					+ " permissioncode='produce:reservation:defect:main:02' onClick=\"Defectkeep_Update_Modal.open('"+row.id+"')\" title='修改 Edit'></i>&nbsp;&nbsp;";//修改
			...
			if(row.zt == 2){
				htmlContent += "<i class=' icon-check color-blue cursor-pointer checkPermission' " 
					+ " permissioncode='produce:reservation:defect:main:04' onClick=\"Defectkeep_Approval_Modal.open('"+row.id+"')\" title='批准 Approvel'></i>&nbsp;&nbsp;";//批准
			}
			if( row.zt == 4 ){
				if(row.gdid==null){
					htmlContent += "<i class='iconnew-create color-blue cursor-pointer checkPermission' " 
						+ " permissioncode='produce:reservation:defect:main:05' onClick=\"createNrc('"+row.id+"','"+row.bs145+"',135)\" title='创建NRC Create NRC'></i>&nbsp;&nbsp;";//创建nrc
					htmlContent += "<i class='iconnew-create color-blue cursor-pointer checkPermission' " 
						+ " permissioncode='produce:reservation:defect:main:051' onClick=\"createNrc('"+row.id+"','"+row.bs145+"',145)\" title='创建NRC Create NRC'></i>&nbsp;&nbsp;";//创建nrc
				}
				htmlContent += "<i class='icon-power-off color-blue cursor-pointer checkPermission' " 
			    	...
			    	+ "gzz=''   gzrq='' " 
					+ " permissioncode='produce:reservation:defect:main:07' onClick=\"shutDown('"+row.id+"',this,true)\" title='关闭 Close'></i>&nbsp;&nbsp;";//完成
			}
			if( row.zt == 2 || row.zt == 4 ){
				htmlContent += "<i class='iconnew-end color-blue cursor-pointer checkPermission' " 
					+ "bldh='"+StringUtil.escapeStr(row.bldh)+"'   zt='"+row.zt+"'    gbyy=''" 
					+ "gbczsj=''   gbrid='' " 
					+ " permissioncode='produce:reservation:defect:main:06' onClick=\"zdjs('"+row.id+"',this,true)\" title='指定结束 Close'></i>&nbsp;&nbsp;";//指定结束
			}
		    htmlContent += "</td>";
		    ...
		    if(row.paramsMap?row.paramsMap.lygdbh:''!=''){
		    	htmlContent += "<a href=\"javascript:findOrderView('"+row.lygdid+"','"+row.bs145+"')\"  >"+StringUtil.escapeStr(row.paramsMap?row.paramsMap.lygdbh:'')+"</a>【"+StringUtil.escapeStr(DicAndEnumUtil.getEnumName('workorderStatusEnum',row.paramsMap?row.paramsMap.lygdzt:''))+"】";
		    }
		    htmlContent +="</td>"; 
		    htmlContent += "<td class='text-left' >"+StringUtil.escapeStr(row.qxms)+"</td>";
		    htmlContent +="</td>"; 
		    ...
		    htmlContent += "<td class='text-center' >"+formatUndefine(row.xfqx).substring(0,10)+"</td>";
		    htmlContent += "<td class='text-left' >"+StringUtil.escapeStr(row.pzyj)+"</td>";
		    htmlContent += "<td class='text-left' >"+StringUtil.escapeStr(row.yxxz)+"</td>";
		    htmlContent += "<td class='text-left' >";
		    ...
		    htmlContent += "<td class='text-left' >"+StringUtil.escapeStr(row.gzz)+"</td>"; 
		    htmlContent += "<td class='text-center' >"+formatUndefine(row.gzrq).substring(0,10)+"</td>";
		    ...
		    $("#defrctkeep_list").empty();
		    $("#defrctkeep_list").html(htmlContent);
		    refreshPermission();			//权限初始化
		    TableUtil.addTitle("#defrctkeep_list tr td"); //加载td title
	 });
}
...
function invalid(id,zt){
	...
	AlertUtil.showConfirmMessage("确定要删除吗？", {callback: function(){
		startWait();
		AjaxUtil.ajax({
			url:basePath + "/produce/reservation/defect/delete",
			contentType:"application/json;charset=utf-8",
			type:"post",
			async: false,
...
function filseDown(id){
	FilesDownModal.show({
		 djid:id,//单据id
		 code:"produce:reservation:defect:main:09",
...
function filesDownSub(obj){
	 startWait($("#FilesDownModal"));
   	 AjaxUtil.ajax({
 		url:basePath+"/produce/reservation/defect/filesDownSub",
 		contentType:"application/json;charset=utf-8",
 		type:"post",
 		async: false,
 		data:JSON.stringify(obj),
...
function gbSubjectFrom(obj){
	 startWait($("#ShutDownModal"));
   	 AjaxUtil.ajax({
 		url:basePath+"/produce/reservation/defect/endModal",
 		contentType:"application/json;charset=utf-8",
 		type:"post",
 		async: false,
 		data:JSON.stringify(obj),
...
function gConfirm(obj){
	 startWait($("#ConfirmModal"));
   	 AjaxUtil.ajax({
 		url:basePath+"/produce/reservation/defect/gConfirm",
 		contentType:"application/json;charset=utf-8",
 		type:"post",
 		async: false,
 		data:JSON.stringify(obj),
...
function findView(id){
	window.open(basePath+"/produce/reservation/defect/find/"+id);
}
