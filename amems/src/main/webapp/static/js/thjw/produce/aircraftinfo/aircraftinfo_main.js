...
var fjzch_0 = '';
$(function(){
	Navigation(menuCode, '', '', 'SC-1-1', '孙霁', '2017-09-25', '孙霁', '2017-09-25');
...
function bindEvent(){
	...
	$(".fxzsyz2").keyup(function(){
		backspace2($(this), cycleReg);
	});
	
}
function backspace2(obj, reg){
	 var value = obj.val();
	 ...
	 if(lastChar=="."&&value.indexOf(lastChar)==0){
		 value = value.substr(0, value.length - 1);
		 obj.val(value);  
		 return;
	 }
	 ...
	 if(value.indexOf(".")!=-1){
		 var str=value.substr(value.lastIndexOf(".")+1);
		 if(str.length>3){
			 value = value.substr(0, value.length - 1);
			 obj.val(value);
		 }
...
//	if (!value) {
//		$('#form').data('bootstrapValidator').updateStatus('jsfxxs',
//				'NOT_VALIDATED', null).validateField('jsfxxs');
//	}
}
...
var aircraftinfoMain={
		...
		AjaxGetDatasWithSearch:function(pageNumber,sortType,sequence){
			 ...
			 startWait();
			 AjaxUtil.ajax({
				 url:basePath+"/aircraftinfo/queryAll",
				   type: "post",
				   contentType:"application/json;charset=utf-8",
				   dataType:"json",
		...
		appendContentHtml:function(list){
			...
			$.each(list,function(index,row){
				 //剩余时间    
			    var sysj = "";
				var ars = [row.paramsMap.syts1?row.paramsMap.syts1:"",row.paramsMap.syts2?row.paramsMap.syts2:"",row.paramsMap.syts3?row.paramsMap.syts3:""];
				sysj = Math.min.apply(null, ars);
				htmlContent += "<tr >";
			    ...
			    if(row.zt==1){
			    	...
			    	"permissioncode='aircraftinfo:main:02' onClick=\"edit('"+row.fjzch+"','"+row.dprtcode+"')\" title='修改 Edit'></i>&nbsp;&nbsp;";
			    	htmlContent += "<i fjjx='"+StringUtil.escapeStr(row.fjjx)+"' zt='"+row.zt+"' dprtcode='"+row.dprtcode+"' fjzch='"+StringUtil.escapeStr(row.fjzch)+"'  class='icon-lock color-blue cursor-pointer checkPermission' permissioncode='aircraftinfo:main:03' " +
			    			"onClick=\"startOrcancel(this)\" title='注销  Write Off'></i>&nbsp;&nbsp;";
			    }else{
			    	htmlContent += "<i fjjx='"+StringUtil.escapeStr(row.fjjx)+"' zt='"+row.zt+"' dprtcode='"+row.dprtcode+"' fjzch='"+StringUtil.escapeStr(row.fjzch)+"' class='icon-unlock-alt color-blue cursor-pointer checkPermission' permissioncode='aircraftinfo:main:04' " +
			    			"onClick=\"startOrcancel(this)\" title='启用 Enabled'></i>&nbsp;&nbsp;";
			    }
			    htmlContent += "</td>";
//			    var yj ="";
//			    var lxMessage = '';
...
//			    }
			    ...
			    var zt=row.zt;
			    yj=getWarningColor(lxMessage,sysj,zt);
			    htmlContent += yj; 
			    
			    htmlContent += "<td style='text-align:left;vertical-align:middle;'  title='"+StringUtil.escapeStr(row.fjjx)+"'>"+StringUtil.escapeStr(row.fjjx)+"</td>";
			    htmlContent += "<td style='text-align:center;vertical-align:middle;'  title='"+StringUtil.escapeStr(row.fjzch)+"'><a href=\"javascript:view('"+row.fjzch+"','"+row.dprtcode+"')\">"+StringUtil.escapeStr(row.fjzch)+"</a></td>";
			    ...
			    htmlContent += "<td style='text-align:left;vertical-align:middle;'  title='"+StringUtil.escapeStr(row.bzm)+"'>"+StringUtil.escapeStr(row.bzm)+"</td>";
			    var gs = '';
			    if(row.customer){
			    ...
			    htmlContent += "<td style='text-align:left;vertical-align:middle;'  title='"+StringUtil.escapeStr(gs)+"'>"+StringUtil.escapeStr(gs)+"</td>"; 
			    htmlContent += "<td style='text-align:left;vertical-align:middle;'  title='"+StringUtil.escapeStr(row.gjdjzh)+" "+(row.gjdjzyxq?row.gjdjzyxq.substring(0,10):'')+"'>"+StringUtil.escapeStr(row.gjdjzh)+" "+(row.gjdjzyxq?row.gjdjzyxq.substring(0,10):'')+"</td>"; 
			    htmlContent += "<td style='text-align:left;vertical-align:middle;'  title='"+StringUtil.escapeStr(row.shzh)+" "+(row.shzzjkrq?row.shzzjkrq.substring(0,10):'')+"'>"+StringUtil.escapeStr(row.shzh)+" "+(row.shzzjkrq?row.shzzjkrq.substring(0,10):'')+"</td>"; 
			    htmlContent += "<td style='text-align:left;vertical-align:middle;'  title='"+StringUtil.escapeStr(row.wxdtxkzh)+" "+(row.dtzzjkrq?row.dtzzjkrq.substring(0,10):'')+"'>"+StringUtil.escapeStr(row.wxdtxkzh)+" "+(row.dtzzjkrq?row.dtzzjkrq.substring(0,10):'')+"</td>"; 
				...
				htmlContent +=	"</td>";
				htmlContent += "<td style='text-align:left;vertical-align:middle;' title='"+StringUtil.escapeStr(row.base?row.base.dprtname:'')+"'>"+StringUtil.escapeStr(row.base?row.base.dprtname:'')+"</td>";
			    htmlContent += "<td style='text-align:center;vertical-align:middle;' title='"+(row.scrq?row.scrq.substring(0,10):'')+"'>"+(row.scrq?row.scrq.substring(0,10):'')+"</td>"; 
			    htmlContent += "<td style='text-align:center;vertical-align:middle;' title='"+(row.ccrq?row.ccrq.substring(0,10):'')+"'>"+(row.ccrq?row.ccrq.substring(0,10):'')+"</td>"; 
			    htmlContent += "<td style='text-align:center;vertical-align:middle;' title='"+(row.gmrq?row.gmrq.substring(0,10):'')+"'>"+(row.gmrq?row.gmrq.substring(0,10):'')+"</td>"; 
			    //有效期
			    var htmls="";
			    if(row.gjdjzyxq){
			    ...
			    htmlContent+="<td style='text-align:center;vertical-align:middle;'>"+htmls+"</td>";
			    htmlContent+="<td style='text-align:center;vertical-align:middle;' title='"+sysj+"'>"+sysj+"</td>";
			    htmlContent += "<td style='text-align:center;vertical-align:middle;' title='"+(row.zt==1?'有效':'无效')+"'>"+(row.zt==1?'有效':'无效')+"</td>"; 
			    htmlContent += "<td style='text-align:left;vertical-align:middle;' title='"+StringUtil.escapeStr(row.zdr?row.zdr.displayName:'')+"'>"+StringUtil.escapeStr(row.zdr?row.zdr.displayName:'')+"</td>"; 
			    ...
			    htmlContent += "<td style='text-align:left;vertical-align:middle;' title='"+StringUtil.escapeStr(row.jg_dprt.dprtname)+"'>"+StringUtil.escapeStr(row.jg_dprt.dprtname)+"</td>"; 
			    htmlContent += "</tr>" ;
		   $("#list").empty();
...
//获取预警颜色
function getWarningColor(lxMessage,syts,zt){
	var htmlappend = "<td class='text-center'></td>";
	//获取最小值
	if(syts<=yjtsJb1){
		bgcolor = warningColor.level1;//红色
		htmlappend =  "<td class='text-center' style='text-align:center;vertical-align:middle;' title='剩余"+lxMessage+"&lt;="+yjtsJb1+"天"+"' ><i class='iconnew-lightbulbnew' style='text-vertical:middle;font-size:16px;color:" + bgcolor+ "' /></td>";
	} else if (yjtsJb1<syts && syts<=yjtsJb2) {
		bgcolor = warningColor.level2;// 黄色
		htmlappend = "<td style='text-align:center;vertical-align:middle;' class='text-center' title='"
				+ yjtsJb1
				+ "&lt;剩余"
				+ lxMessage
				+ "&lt;="
...
function edit(fjzch, dprtcode){
	...
	selectById(aircraftinfo,function(obj){
			aircraftinfoModal.show({
				...
				if (data != null) {
					var message = '保存成功!';
					var url = basePath+"/aircraftinfo/update";
					performAction(url, data, message);
				}
			}
		});
...
function performAction(url, param, message){
	...
	AjaxUtil.ajax({
		...
		success:function(data){
			...
			aircraftinfoMain.goPage(aircraftinfoMain.pagination.page, aircraftinfoMain.pagination.sort, aircraftinfoMain.pagination.order);
		}
	});
}
//搜索条件重置
