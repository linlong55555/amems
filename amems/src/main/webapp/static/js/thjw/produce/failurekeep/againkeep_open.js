again_keep_open = {
	...
	setDate : function(data){
		...
		/*$("#zcFhInput").val(TimeUtil.convertToHour(data.fxsj, TimeUtil.Separator.COLON)); //飞行时间
		$("#zcFcInput").val(data.fxxh); //飞行循环*/	
	},
	/**
	 * 获取数据
	 */
...
/*		 var oDate1 = new Date($("#"+this_.id+"_blqx").val()); //保留日期
	...
	save : function(){
		...
		if(obj==false){
		 ...
		 startWait($("#Againkeep_Open_Modal"));
	   	 AjaxUtil.ajax({
	 		url:basePath+"/produce/reservation/fault/saveagainkeep",
	 		contentType:"application/json;charset=utf-8",
	 		type:"post",
	 		async: false,
	 		data:JSON.stringify(obj),
