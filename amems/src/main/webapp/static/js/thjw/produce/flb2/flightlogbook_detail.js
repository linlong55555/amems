	...
	var flb_detail = {
		...
		getData : function(){
			...
			obj.gjbs = $("#"+this_.id+" input[name='work_gjbs']").is(":checked") ? 1 : 0;
		...
		del : function(){
			...
			AlertUtil.showConfirmMessage("是否确定删除？", {callback: function(){
				startWait();
				AjaxUtil.ajax({
					url:basePath+"/produce/flb2/delete",
					contentType:"application/json;charset=utf-8",
					type:"post",
					data:JSON.stringify({
						id : id
		...
		save : function(){
			...
			function doSave(){
				startWait($("#"+this_.id));
				AjaxUtil.ajax({
					url:basePath+"/produce/flb2/save",
					contentType:"application/json;charset=utf-8",
					type:"post",
		...
		submit : function(){
			...
			function doSubmit(){
				startWait($("#"+this_.id));
				AjaxUtil.ajax({
					url:basePath+"/produce/flb2/submit",
					contentType:"application/json;charset=utf-8",
					type:"post",
					data:JSON.stringify(obj),
					modal : $("#"+this_.id),
		...
		revision:function(){
			...
			function doRevise(){
				startWait($("#"+this_.id));
				AjaxUtil.ajax({
					url:basePath+"/produce/flb2/revise",
					contentType:"application/json;charset=utf-8",
					type:"post",
					data:JSON.stringify(obj),
					modal : $("#"+this_.id),
		...
		discard:function(){
			...
			AlertUtil.showConfirmMessage("确定要作废该记录?", {callback: function(){
				startWait($("#"+this_.id));
				AjaxUtil.ajax({
					url:basePath+"/produce/flb2/discard/"+id,
					contentType:"application/json;charset=utf-8",
					type:"post",
					modal : $("#"+this_.id),
					dataType:"json",
		...
		fillData : function(data){
			...
			if(data.gjbs == 1){
				$("#"+this_.id+" input[name='work_gjbs']").attr("checked", true);
			}else{
				$("#"+this_.id+" input[name='work_gjbs']").attr("checked", false);
			}
