...
function AjaxGetDatasWithSearch(pageNumber, sortType, sequence) {
	...
	startWait();
	AjaxUtil.ajax({
				url : basePath + "/produce/maintenanceplan145/getWorkpackage145List",
				type : "post",
				...
				success : function(data) {
					...
					if (data.total > 0) {
						...
						// 标记关键字
						signByKeyword($("#keyword_search").val(), [ 3, 7,15 ],"#packaginglist tr td");
					} else {
						$("#packaginglist").empty();
...
function appendContentHtml(list) {
	...
	$.each(list,function(index, row) {
		...
		htmlContent += "</td>";
		htmlContent += "<td class='text-center fixed-column'>";	
		if(row.zt==7 && row.wgbs==0){
			htmlContent += "<i class='iconnew-feedback color-blue cursor-pointer checkPermission' permissioncode='produce:maintenanceplan145:main:01' onClick=\"doWgfk('"
			+ row.id + "','" + row.zt + "','" + row.dprtcode + "')\" title='完工反馈 Feedback'></i>&nbsp;&nbsp;";
		}
		if(row.zt == 7){
			htmlContent += "<i class='icon-remove-sign color-blue cursor-pointer checkPermission' permissioncode='produce:maintenanceplan145:main:02' onClick=\"closeData('"
					+ row.id + "')\" title='关闭  Close'></i>&nbsp;&nbsp;<i class='iconnew-end  color-blue cursor-pointer checkPermission' permissioncode='produce:maintenanceplan145:main:03' onClick=\"endClose('"
					+ row.id + "')\" title='指定结束 End'></i>&nbsp;&nbsp;";
		}
		if(row.zt == "9" || row.zt == "10"){ //9、10状态,可编辑
			htmlContent += "<i class='fa  fa-thumb-tack color-blue cursor-pointer checkPermission' permissioncode='produce:maintenanceplan145:main:03' onClick=openWinXDClose('"+ row.id + "','" + row.zt + "')  title='修订 Revised'></i>&nbsp;&nbsp;";
		}
		htmlContent += "</td>";
	...
	getDataById(id,function(obj){
		...
		if(obj.zt==9){
			workpackage_end_modal.init({
				...
				callback:function(data){
					data.zt=obj.zt;
					doRequest(data,"保存成功!",basePath + "/produce/maintenanceplan145/doXDClose","workpackage_end_modal");
				}
			});
		}else{
			workpackage_close_modal.init({
				...
				callback:function(data){
					data.zt=obj.zt;
					doRequest(data,"保存成功!",basePath + "/produce/maintenanceplan145/doXDClose","workpackage_close_modal");
				}
			});
		}
	})
