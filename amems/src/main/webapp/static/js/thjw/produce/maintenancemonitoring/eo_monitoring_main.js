		...
		TimeUtil.getCurrentDate(function (data){
			...
			AjaxUtil.ajax({
				...
				success:function(data){
					...
					$("#e_total_count", $("#"+this_.id)).html(data.total);
//					new Sticky({tableId:this_.tableId});
	...
	appendContentHtml: function(list){
		...
		$.each(list,function(index,row){
			var eo = row.engineeringOrder?row.engineeringOrder:{};
			...
			if(paramsMap.checkedId == null && paramsMap.gdbh == null){
				htmlContent += "<td index="+index+" onclick='"+this_.id+".checkRow(this,2)' class='' style='text-align:center;vertical-align:middle;cursor:pointer;'><input index="+index+" type='checkbox' name='eo_m_row' onclick='"+this_.id+".checkRow(this,2)' /></td>";
			}else{
				htmlContent += "<td index="+index+" onclick='"+this_.id+".checkRow(this,2)' class='' style='text-align:center;vertical-align:middle;cursor:pointer;'><input index="+index+" hide='hide' type='checkbox' name='eo_m_row' style='display:none;' /></td>";
			}
	...
	setNoData : function(){
		...
		$("#"+this_.tbodyId, $("#"+this_.tableDivId)).append("<tr><td colspan=\"28\" class='text-center'>暂无数据 No data.</td></tr>");
	},
	...
	showHiddenContent:function(this_){
		if(this.selectRowType == 1){
			...
			this.param.parentObj.loadTool(id, 2);
