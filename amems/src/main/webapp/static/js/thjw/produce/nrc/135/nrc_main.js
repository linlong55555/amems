...
var workorder135_main = {
	...
	AjaxGetDatasWithSearch:function(pageNumber,sortType,sequence){
		...
		startWait();
		AjaxUtil.ajax({
			url:basePath+"/produce/nrc135/queryAllPageList",
			type: "post",
			...
			success:function(data){
				...
				if(data.rows.length > 0){
					...
					// 标记关键字
					signByKeyword($.trim($("#wo135search_keyword").val()),[3,9,14,16],"#workorder135_list tr td");
	...
	getParams : function(){
		...
		searchParam.gzlb = $("#gzlb_search").val(); //工作类别
	...
	appendContentHtml:function(list){
		...
		$.each(list,function(index,row){
			...
			htmlContent += "<i class='icon-print color-blue cursor-pointer checkPermission' permissioncode='produce:nrc135:main:08' onClick=workorder135_main.print('"+ row.paramsMap.gdid +"','"+row.gdlx+ "',event) title='打印 Print'></i>&nbsp;&nbsp;";
			/*操作
			...
			if(row.zt != "9" && row.zt != "10"){ //9、10以外状态,可编辑
				htmlContent += "<i class='icon-edit color-blue cursor-pointer checkPermission' permissioncode='produce:nrc135:main:02' onClick=workorder135_main.openWinEdit('"+ row.paramsMap.gdid + "',true,event) title='修改 Edit'></i>&nbsp;&nbsp;";
			}
			if(row.zt == "1"){ //状态为保存,可删除
				htmlContent += "<i class='icon-trash color-blue cursor-pointer checkPermission' permissioncode='produce:nrc135:main:03' onClick=workorder135_main.openWinDel('"+ row.paramsMap.gdid + "',true,event) title='删除 Delete'></i>&nbsp;&nbsp;";
			}
			if(row.zt == "2" || row.zt == "7"){ //状态为提交、生效下发,可完工反馈
				htmlContent += "<i class='iconnew-feedback color-blue cursor-pointer checkPermission' permissioncode='produce:nrc135:main:04' onClick=workorder135_main.openWinFeedback('"+ row.paramsMap.gdid + "',true,event)  title='完工反馈'></i>&nbsp;&nbsp;";
			}
			if((row.zt == "2" ||row.zt == "7") && row.xmblbs==0){ //状态为提交或生效下发且项目保留标识=0,可指正常完成
				htmlContent += "<i class='icon-remove-sign color-blue cursor-pointer checkPermission' permissioncode='produce:nrc135:main:05' onClick=workorder135_main.openWinWGClose('"+ row.paramsMap.gdid + "',true,event)  title='完工关闭'></i>&nbsp;&nbsp;";
			}
			if(row.zt == "2" || row.zt == "7"){ //状态为提交、生效下发,可指定结束
				htmlContent += "<i class='iconnew-end color-blue cursor-pointer checkPermission' permissioncode='produce:nrc135:main:06' onClick=workorder135_main.openWinZDClose('"+ row.paramsMap.gdid + "',true,event)  title='指定结束'></i>&nbsp;&nbsp;";
			}
			if((row.zt == "2" || row.zt == "7" )&& (row.fjzch !="" && row.fjzch != null && row.fjzch !="-" ) && row.xmblbs==0){ //状态为生效,可进行,工作项目保留
				htmlContent += "<i class='iconnew-projectKeep color-blue cursor-pointer checkPermission' permissioncode='produce:reservation:item:main:01' onClick=workorder135_main.openWinView('"+ row.paramsMap.gdid + "',5)  title='工作项目保留'></i>&nbsp;&nbsp;";
			}
			if(row.zt == "9" && row.gdlx != 9){ //状态为指定结束,可进行,修订
				htmlContent += "<i class='fa fa-thumb-tack color-blue cursor-pointer checkPermission' permissioncode='produce:nrc135:main:07' onClick=workorder135_main.openWinRevision9('"+ row.paramsMap.gdid + "',5)  title='修订'></i>&nbsp;&nbsp;";
			}
			if(row.zt == "10" && row.gdlx != 9){ //状态为指定结束,可进行,修订
				htmlContent += "<i class='fa fa-thumb-tack color-blue cursor-pointer checkPermission' permissioncode='produce:nrc135:main:07' onClick=workorder135_main.openWinRevision10('"+ row.paramsMap.gdid + "',5)  title='修订'></i>&nbsp;&nbsp;";
			}
			htmlContent += "</td>";
			htmlContent += "<td title='"+StringUtil.escapeStr(row.lyrwh)+"'>"+
				"<a href=javascript:workorder135_main.openWinView('"+ row.lyrwid+"','"+row.gdlx + "'); class='text-left ' style='margin-right:5px;' >"+StringUtil.escapeStr(row.lyrwh)+"</a>"+
				"</td>";
			...
			htmlContent += "<td title='"+StringUtil.escapeStr(row.gdbh)+"'>"+
								"<a href=javascript:workorder135_main.openWinView('"+ row.paramsMap.gdid + "',3); class='text-left ' style='margin-right:5px;' >"+StringUtil.escapeStr(row.gdbh)+"</a>"+
						   "</td>";
			...
			htmlContent += "<td title='"+lyfx+"'>"+lyfx+"</td>";
			...
			htmlContent += "<td title='"+StringUtil.escapeStr(row.gkbh)+"'>"+
			                     "<a href=javascript:workorder135_main.openWinView('"+ row.gkid + "',7); class='text-left ' style='margin-right:5px;' >"+StringUtil.escapeStr(row.gkbh)+"</a>"+
			               "</td>";
			...
			htmlContent += "<td title='"+StringUtil.escapeStr(row.paramsMap.gbbh)+"'>"+
			 					 "<a href=javascript:workorder135_main.openWinView('"+ row.gbid + "',4); class='text-left ' style='margin-right:5px;' >"+StringUtil.escapeStr(row.paramsMap.gbbh)+"</a>"+
			               "</td>";
...
//		this_.initWebuiPopoverWorkOrder();
		...
		this_.selectById(id,function(obj){
			...
			Workorder135ZDCloseWin.show({
				...
				callback : function(data) {//回调函数
					if (data != null) {
						var message = '提交成功!';
						var url = basePath+"/produce/nrc135/doWGRevision";
	...
	getWorkOrderButtonList : function(obj){//获取操作信息
		...
		if(zt == "7"){ //状态为生效,可进行,工作项目保留
			...
			str += "<a href='javascript:void(0);' onClick=workorder135_main.openWinView('"+ gdid + "',5)><i class='iconnew-projectKeep color-blue cursor-pointer'></i>&nbsp;工作项目保留</a>";
			str += "</p>";
		}
		str += "</div>";
		return str;
		...
		this_.selectById(id,function(obj){
			...
			Workorder135FeedbackWin.show({
				...
				callback : function(data) {//回调函数
					if (data != null) {
						var message = '反馈成功!';
						var url = basePath+"/produce/nrc135/doFeedback";
		...
		this_.selectById(id,function(obj){
			...
			Workorder135ZDCloseWin.show({
				...
				callback : function(data) {//回调函数
					if (data != null) {
						var message = '指定结束成功!';
						var url = basePath+"/produce/nrc135/doZDClose";
		...
		this_.selectById(id,function(obj){
			...
			Workorder135WGCloseWin.show({
				...
				callback : function(data) {//回调函数
					if (data != null) {
						var message = '关闭成功!';
						var url = basePath+"/produce/nrc135/doWGClose";
		...
		Workorder135AddWin.show({
			...
			callback : function(data) {//回调函数
				if (data != null) {
					...
					var url = basePath+"/produce/nrc135/save";
			...
			Workorder135AddWin.show({
				...
				callback : function(data) {//回调函数
					if (data != null) {
						...
						var url = basePath+"/produce/nrc135/update";
						this_.performAction(url,data,message,"workorder135_add_Modal",isRefresh);
					}
				}
			});
	...
	openWinDel : function(id,isRefresh,o){
		...
		this_.selectById(id,function(obj){
			...
			AlertUtil.showConfirmMessage("确定要删除吗？", {callback: function(){
				AjaxUtil.ajax({
					type:"post",
					url:basePath+"/produce/nrc135/doDelete",
					dataType:"json",
					async: false,
					data:{'woId':id},
					success:function(data) {
	...
	openWinView  : function(id,pageSwitch){
		...
		if(pageSwitch == 1){
			...
			window.open(basePath+"/produce/nrc135/woView?gdid="+id);
		}
	...
	selectById : function(id,obj){//通过id获取数据
		AjaxUtil.ajax({
			async: false,
			url:basePath+"/produce/nrc135/selectWOById",
			type:"post",
			data:{gdid:id},
			dataType:"json",
			success:function(data){
