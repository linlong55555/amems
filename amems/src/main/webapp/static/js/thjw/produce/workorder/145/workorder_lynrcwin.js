 ...
 */
Workorder145NrcWin = {
	id:'workorder145_lynrc_Modal', //窗口ID
	baseId:"workorder145_lynrc", //基础ID
	tbodyId:'', //列表ID
	...
	clearData : function(){
		$("#workorder145_lynrc_keyword").val("");
	},
	/**初始数据*/
	initInfo : function(){
		//初始窗口标题信息
		...
		AjaxUtil.ajax({
			...
			success:function(data){
				...
				if(data.rows.length > 0){
					...
					// 标记关键字
					signByKeyword($("#workorder145_lynrc_keyword").val(),[2,3],"#"+this_.baseId+"_list tr td");
				}else{
	...
	getParams : function(){
		...
		var paramsMap = {};
		paramsMap.keyword = $("#workorder145_lynrc_keyword").val();
		paramsMap.gdlx = this_.param.gdlx;
		paramsMap.gdzt = this_.param.gdzt;
		paramsMap.gbid = this_.param.gbid;
		...
		$.each(list,function(index,row){
		   ...
		   if(this_.param.multi){
			   htmlContent += "<input type='checkbox' name='"+this_.id+"_list_checkbox' value='"+index+"'>";  
				...
				if(null != row.paramsMap && this_.param.selected == row.id){
