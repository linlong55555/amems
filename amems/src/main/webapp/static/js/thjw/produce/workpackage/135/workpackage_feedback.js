workpackage_feedback_modal = {
	id:"workpackage_feedback",
	data:[],
	param: {
		obj:{},
		dprtcode:userJgdm,
	...
	init:function(param){
		...
		var zxdw = document.getElementsByName(this.id+"_sjzxdw_radio");
		for(var i=0;i<zxdw.length;i++){
			if(zxdw[i].value=='0'){
				zxdw[i].checked='checked';
				$("#"+this.id+"_sjzxdw_gysdiv").hide();
			}
		}
		if(param){
		...
		if(this.param.operation){
			$("#"+this.id+"_doDiv  input[type='text']").attr("disabled",false);
			$("#"+this.id+"_doDiv  textarea").attr("disabled",false);
			$("#"+this.id+"_doDiv  input[type='radio']").attr("disabled",false);
			...
			$("#"+this.id+"_doDiv  input[type='text']").attr("disabled",true);
			$("#"+this.id+"_doDiv  input[type='radio']").attr("disabled",true);
			$("#"+this.id+"_doDiv  textarea").attr("disabled",true);
		...
		if(this.param.workpackageId !=''){
			...
			this.checkRadio(0)
			$("#workpackage_feedback_sjzxdw_bmdiv").show();
			$("#workpackage_feedback_sjzxdw_gysdiv").hide();
		}
		$('.date-picker').datepicker('update');
		$('input:radio[name="workpackage_feedback_sjzxdw_radio"]').click( function(){
			zxdw = $(this).val();
				if(zxdw==0){
					$("#workpackage_feedback_sjzxdw_bmdiv").show();
					$("#workpackage_feedback_sjzxdw_gysdiv").hide();
					$("#workpackage_feedback_gys").val("");
					$("#workpackage_feedback_gysid").val("");
				}else{
					$("#workpackage_feedback_sjzxdw_bmdiv").hide();
					$("#workpackage_feedback_sjzxdw_gysdiv").show();
					$("#workpackage_feedback_bm").val("");
					$("#workpackage_feedback_bmid").val("");
				}
		});
		$("#workpackage_feedback_modal").on("hidden.bs.modal", function() {
		$("#workpackage_feedback_form").data('bootstrapValidator').destroy();
		$('#workpackage_feedback_form').data('bootstrapValidator', null);
		workpackage_feedback_modal.validation();
		});
	...
	checkRadio:function(obj){
		var this_=this;
		var xfdw = document.getElementsByName(this_.id+"_sjzxdw_radio");
	...
	fillData:function(data){
		...
		if(wwbs!=null || wwbs != undefined){
			...
			if(wwbs==0){				
				...
				$("#"+this_.id+"_sjzxdw_bmdiv").show();
				$("#"+this_.id+"_sjzxdw_gysdiv").hide();
			}else{
				$("#"+this_.id+"_gys").val(data.sjZxdw);
				$("#"+this_.id+"_gysid").val(data.sjZxdwid);
				$("#"+this_.id+"_sjzxdw_bmdiv").hide();
				$("#"+this_.id+"_sjzxdw_gysdiv").show();
			}
		}else{
			wwbs=data.jhWwbs;
			this_.checkRadio(wwbs);
	...
	openzrdw : function(obj){
		var this_ = this;
		var dprtname = $("#"+this_.id+"_bm").val();
		var dprtcode = $("#"+this_.id+"_bmid").val();
...
$('#workpackage_feedback_sjwcrq').datepicker({
	autoclose : true,
	clearBtn : true
}).on('hide',function(e) {
	$('#workpackage_feedback_form').data('bootstrapValidator').updateStatus('workpackage_feedback_sjwcrq',
				'NOT_VALIDATED', null).validateField('workpackage_feedback_sjwcrq');
});
