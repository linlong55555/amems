...
var po_monitoring_main = {
	...
	load : function(pageNumber, sortColumn, orderType){
		...
		TimeUtil.getCurrentDate(function (data){
			...
			startWait();
			AjaxUtil.ajax({
				url:basePath+"/produce/maintenance/initialization/queryAllPagePOList",
				type: "post",
				...
				success:function(data){
					...
					if(data.total >0){
					   ...
					   new Pagination({
						...
						// 标记关键字
						signByKeyword($("#keyword_search", $("#"+this_.id)).val(),[2,4,16],"#"+this_.tbodyId+" tr td");
					} else {
						this_.setNoData();
					}
					new Sticky({tableId:this_.tableId});
	...
	appendContentHtml: function(list){
		...
		$.each(list,function(index,row){
			...
			htmlContent += "<td class='fixed-column' title='"+StringUtil.escapeStr(paramsMap.zlbh)+"' style='text-align:left;vertical-align:middle;'>";
			...
			htmlContent += this_.formatMonitorData(paramsMap.jhsjsj, paramsMap.hdwz);
	...
	setNoData : function(){
		...
		$("#"+this_.tbodyId, $("#"+this_.tableDivId)).append("<tr><td colspan=\"19\" class='text-center'>暂无数据 No data.</td></tr>");
	},
	...
	viewHistory : function(id, dprtcode){
		window.open(basePath+"/produce/maintenance/initialization/po/history?id=" + id + "&fjzch=" + this.param.fjzch + "&dprtcode="+ dprtcode);
	},
	/**
	 * 打开EO监控弹出框
	 */
