 ...
 * 145工单完工反馈
 */
Workorder145FeedbackWin = {
	id:'workorder145_feedback_Modal', //窗口ID
	tbodyId:'wo145feedback_list', //列表ID
	...
	show : function(param){
		...
		$('#workorder145_feedback_Modal .date-picker').datepicker('update');
	},
	/**初始数据*/
	clearDatas : function(){
		var this_ = this;
	...
	doOption:function(){
		...
		if(this_.param.colOptionhead){
			...
			$("#wo145feedback_sjgzz_btn").show();
			$("#wo145feedback_sjjcz_btn").show();
			$("#wo145feedback_sjgzz").addClass("readonly-style");
		}else{
			...
			$("#wo145feedback_sjgzz_btn").hide();
			$("#wo145feedback_sjjcz_btn").hide();
			$("#wo145feedback_sjgzz").removeClass("readonly-style");
		}
	},
	/**初始数据*/
	initInfo : function(){
		...
		$("#"+this_.id).on("hidden.bs.modal", function() {
			 $("#wo145feedback_from").data('bootstrapValidator').destroy();
		     $('#wo145feedback_from').data('bootstrapValidator', null);
		     this_.formValidator();
		});
		//实际完成日期
		$('#wo145feedback_sjJsrq').datepicker({
			autoclose : true,
			clearBtn : true
		}).on('hide',function(e) {
			$('#wo145feedback_from').data('bootstrapValidator').updateStatus('wo145feedback_sjJsrq','NOT_VALIDATED', null).validateField('wo145feedback_sjJsrq');
		});
		...
		//显示拆换件列表
		console.info(this.param.viewObj.fjzch);
		...
		if(this.param.isShowReplacementRecord){
			$("#wo145feedback_replacementRecord_div").show();
			this.showReplacementRecord();
		}else{
			$("#wo145feedback_replacementRecord_div").hide();
		}
		//显示附件列表
	...
	loadeBaseData : function(){
		...
		if(this_.param.viewObj.gdlx == 3){
			$("#workorder_feedback_title").html("缺陷描述");
			$("#workorder_feedback_Etitle").html("Desc");
		}else{
			$("#workorder_feedback_title").html("工单标题");
			$("#workorder_feedback_Etitle").html("WO Title");
		}
		var total = 0;
		...
		$("#wo145feedback_list").html(htmlContent);
		
		$("#wo145feedback_sjJsrq").val(StringUtil.escapeStr(this_.param.viewObj.sjJsrq).substring(0, 10)); //实际完成日期
		$("#wo145feedback_sjJssj").val(StringUtil.escapeStr(this_.param.viewObj.sjJsrq).substring(11, 16));//实际完成时间
//		$("#wo145feedback_sjgzz").val(this_.param.viewObj.sjGzz);//实际工作者
//		$("#wo145feedback_sjjcz").val(this_.param.viewObj.sjJcz);//实际检查者
		...
		$("#wo145feedback_sjgzz").val(gzzList);
		$("#wo145feedback_sjgzzid").val(gzzidList);
		$("#wo145feedback_sjjcz").val(this_.param.viewObj.sjJcz);
		$("#wo145feedback_sjjczid").val(this_.param.viewObj.sjJczid);
		this_.disableUser("wo145feedback_sjjcz");
		
		$("#wo145feedback_sjgs").val(this_.param.viewObj.sjGs);//实际工时
		$("#wo145feedback_sjzd").val(this_.param.viewObj.sjZd);//工作站点
		$("#wo145feedback_sjksrq").val(StringUtil.escapeStr(this_.param.viewObj.sjKsrq).substring(0, 10));//实际开始日期
		$("#wo145feedback_gzxx").val(this_.param.viewObj.gzxx);//故障信息
		$("#wo145feedback_clcs").val(this_.param.viewObj.clcs);//处理措施
		//拆换件暂缺......
	},
	/**初始化对话框头部*/
	...
	showReplacementRecord : function(){
		replacementRecord.init({
			...
			defaultCzrq:function(){
				var sjjsrq = $.trim($("#wo145feedback_sjJsrq").val());
				var sjjssj = $.trim($("#wo145feedback_sjJssj").val());
				var czrq = "";
				if(sjjsrq){
	...
	showAttachments : function(){
		var this_ = this;
		var attachmentsObj = attachmentsUtil.getAttachmentsComponents('wo145feedback_attachments_list_workorderedit');
		attachmentsObj.show({
	...
	getData : function(){
		...
		var sjJsrq = $("#wo145feedback_sjJsrq").val() + " "+$("#wo145feedback_sjJssj").val();
		param.sjJsrq = new Date(Date.parse(sjJsrq.replace(/-/g, "/"))); ;//实际完成日期
//		param.sjGzz = $("#wo145feedback_sjgzz").val();//实际工作者
//		param.sjJcz = $("#wo145feedback_sjjcz").val();//实际检查者
		
		var userList = [];
		var gzzidList = $("#wo145feedback_sjgzzid").val();
		var gzzList = $("#wo145feedback_sjgzz").val();
		...
		param.sjJcz = $("#wo145feedback_sjjcz").val();
		param.sjJczid = $("#wo145feedback_sjjczid").val();
		
		param.sjGs = $("#wo145feedback_sjgs").val();//实际工时
		param.sjZd = $("#wo145feedback_sjzd").val();//工作站点
		param.sjKsrq = $("#wo145feedback_sjksrq").val();//实际开始日期
		param.gzxx = $("#wo145feedback_gzxx").val();//故障信息
		param.clcs = $("#wo145feedback_clcs").val();//处理措施
	...
	submitFeedback :  function(){
		...
		$('#wo145feedback_from').data('bootstrapValidator').validate();
		if(!$('#wo145feedback_from').data('bootstrapValidator').isValid()){
			AlertUtil.showModalFooterMessage("请根据提示输入正确的信息!");
			$("#"+this_.id+" .modal-body").prop('scrollTop','0');
			return false;
		}
		var sjjssj=$("#wo145feedback_sjJssj").val();
		if(sjjssj != ''){
		...
		if($("#wo145feedback_sjksrq").val() != '' && $("#wo145feedback_sjJsrq").val() != '' && $("#wo145feedback_sjksrq").val()>$("#wo145feedback_sjJsrq").val()){
			AlertUtil.showModalFooterMessage("实际开始日期不能晚于实际完成日期!");
			return false;
		}
		var sjgs = $("#wo145feedback_sjgs").val();//实际工时
	...
	formValidator : function(){//加载验证
		validatorForm =  $('#wo145feedback_from').bootstrapValidator({
	        message: '数据不合法',
	        feedbackIcons: {
	...
	openGzzWin : function(){
		...
		Personel_Tree_Multi_Modal.show({
			...
			callback: function(data){//回调函数
				...
				$("#wo145feedback_sjgzz").val(displayName);
				$("#wo145feedback_sjgzzid").val(mjsrid);
			}
		});
		AlertUtil.hideModalFooterMessage();
	},
	...
	openJczWin : function(){
		var this_ = this;
		Personel_Tree_Multi_Modal.show({
			checkUserList:[{id:$("#wo145feedback_sjjczid").val(),displayName:$("#wo145feedback_sjjcz").val()}],//原值，在弹窗中回显
			dprtcode:this_.param.dprtcode,
			multi:false,
			callback: function(data){//回调函数
				var user = $.isArray(data)?data[0]:{id:'',displayName:''};
				$("#wo145feedback_sjjczid").val(user.id);
				$("#wo145feedback_sjjcz").val(user.displayName);
				this_.disableUser("wo145feedback_sjjcz");
			}
		});
		AlertUtil.hideModalFooterMessage();
