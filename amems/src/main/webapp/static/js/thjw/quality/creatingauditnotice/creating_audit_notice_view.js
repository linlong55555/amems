$(function(){
	Navigation(menuCode, '查看审核通知单', 'View', 'SC-1-2 ', '孙霁', '2017-09-25', '孙霁', '2017-09-25');//加载导航栏
	var auditnoticeId = $("#auditnoticeId").val();
	selectById(auditnoticeId);
	shcyappend(auditnoticeId);
...
function viewObj(obj){
	var this_ = this;
	$("#jcdbh").val(StringUtil.escapeStr(obj.jcdbh));
	$("#jcdbh").attr("disabled",true);
	$("#jhShrq").val(StringUtil.escapeStr(obj.jhShrq?obj.jhShrq.substring(0,10):''));
	$("input:radio[name='lx'][value="+obj.lx+"]").attr('checked', 'checked');
	$("input:radio[name='shlb'][value="+obj.shlb+"]").attr('checked', 'checked');
	$("#jcnr").val(StringUtil.escapeStr(obj.jcnr));
	...
	$("#zt").val(StringUtil.escapeStr(DicAndEnumUtil.getEnumName('auditnoticeStatusEnum',obj.zt)));
