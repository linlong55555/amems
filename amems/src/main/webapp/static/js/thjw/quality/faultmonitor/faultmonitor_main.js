...
function AjaxGetDatasWithSearch(pageNumber,sortType,sequence){
	...
	AjaxUtil.ajax({
	   url:basePath+"/quality/faultmonitoring/queryMonitorAllPageList",
	   type: "post",
			...
			if( row.zt == 4 ){
				if(row.zcblbs==0){
					htmlContent += "<i class='iconnew-keep color-blue cursor-pointer checkPermission' " 
						+ " permissioncode='quality:faultmonitoring:main:01'  onClick=\"again_keep_open.open('"+row.id+"')\" title='再次保留 Again Keep'></i>&nbsp;&nbsp;";//再次保留
				}
...
//				+ " permissioncode='produce:reservation:fault:monitor:06' onClick=\"createNrc('"+row.id+"','"+row.bs145+"')\" title='创建NRC Create NRC'></i>&nbsp;&nbsp;";//创建nrc
				htmlContent += "<i class='icon-power-off color-blue cursor-pointer checkPermission' " 
			    	...
			    	+ "gzz=''   gzrq='' " 
			    	+ "jcz=''   jcrq='' " 
					+ " permissioncode='quality:faultmonitoring:main:02' onClick=\"shutDown('"+row.id+"',this,true)\" title='关闭 Close'></i>&nbsp;&nbsp;";//完成
			}
			if( row.zt == 2 || row.zt == 4 ){
				htmlContent += "<i class='iconnew-end color-blue cursor-pointer checkPermission' " 
					...
					+ " permissioncode='quality:faultmonitoring:main:03' onClick=\"zdjs('"+row.id+"',this,true)\" title='指定结束 Close'></i>&nbsp;&nbsp;";//指定结束
			}
		    htmlContent += "</td>";
		    ...
		    htmlContent += "<td style='text-align:center;' >"+showPJInfo(row.syList, 'syz'+row.id)+"</td>";
		    ...
		    htmlContent += "<td style='text-align:center;' >"+showPJInfo(row.scblqxList, 'scblqx'+row.id)+"</td>";
		    htmlContent += "<td class='text-center' >"+formatUndefine(row.zcSqrq).substring(0,10)+"</td>";
		    htmlContent += "<td style='text-align:center;' >"+showPJInfo(row.zcblqxList, 'zcblqx'+row.id)+"</td>";
...
function filesDownSub(obj){
	 startWait($("#FilesDownModal"));
   	 AjaxUtil.ajax({
 		url:basePath+"/quality/faultmonitoring/filesDownSub",
 		contentType:"application/json;charset=utf-8",
 		type:"post",
 		async: false,
 		data:JSON.stringify(obj),
...
function gbSubjectFrom(obj){
	 startWait($("#ShutDownModal"));
   	 AjaxUtil.ajax({
 		url:basePath+"/quality/faultmonitoring/endModal",
 		contentType:"application/json;charset=utf-8",
 		type:"post",
 		async: false,
 		data:JSON.stringify(obj),
...
function gConfirm(obj){
	 startWait($("#ConfirmModal"));
   	 AjaxUtil.ajax({
 		url:basePath+"/quality/faultmonitoring/gConfirm",
 		contentType:"application/json;charset=utf-8",
 		type:"post",
 		async: false,
 		data:JSON.stringify(obj),
