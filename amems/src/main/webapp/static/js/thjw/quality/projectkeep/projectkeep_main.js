	 ...
	 $.each(list,function(index,row){
			...
			if( row.zt == 1 ){
				htmlContent += "<i class=' icon-edit color-blue cursor-pointer checkPermission' " 
					+ " permissioncode='produce:reservation:fault:main:02' onClick=\"Project_Update_Modal.open('"+row.id+"')\" title='修改 Edit'></i>&nbsp;&nbsp;";//修改
			...
			if( row.zt == 2){
				if(row.gdblzt==1){
					htmlContent += "<i class='iconnew-execution color-blue cursor-pointer checkPermission' " 
						+ " permissioncode='produce:reservation:fault:main:06' onClick=\"reExecute('"+row.id+"','"+ row.zt +"')\" title='重新执行Re-execute'></i>&nbsp;&nbsp;";//创建nrc
