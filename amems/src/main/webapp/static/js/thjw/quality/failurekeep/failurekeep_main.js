	...
	AjaxUtil.ajax({
	   ...
	   success:function(data){
			...
			signByKeyword($("#keyword_search").val(), [3,6, 15,16, 18]);
				...
				$("#pagination").empty();
				$("#airworthiness_list").append("<tr><td class='text-center' colspan=\"23\">暂无数据 No data.</td></tr>");
 			}
...
function appendContentHtml(list){
	 ...
	 $.each(list,function(index,row){
			...
			if( row.zt == 4 ){
				...
				if(row.gdid==null){
					htmlContent += "<i class='iconnew-create color-blue cursor-pointer checkPermission' " 
						+ " permissioncode='produce:reservation:fault:main:06' onClick=\"createNrc('"+row.id+"','"+row.bs145+"')\" title='创建NRC Create NRC'></i>&nbsp;&nbsp;";//创建nrc
				}
		    ...
		    htmlContent += "<td class='text-left' >"+StringUtil.escapeStr(row.fixChapter?row.fixChapter.zjh:'')+"</td>"; 
		    htmlContent += "<td class='text-left' >"+StringUtil.escapeStr(row.hz)+"</td>";
		    htmlContent += "<td class='text-left' >"+StringUtil.escapeStr(DicAndEnumUtil.getEnumName('failureKeepEnum',row.blly))+"</td>";
		    htmlContent += "<td class='text-left' >"+StringUtil.escapeStr(DicAndEnumUtil.getEnumName('failureKeepTypeEnum',row.bllx))+"</td>";
		    htmlContent += "<td class='text-left' >"+StringUtil.escapeStr(row.blyj)+"</td>"; 
		    htmlContent += "<td class='text-left' >"+StringUtil.escapeStr(row.gzms)+"</td>"; 
		    htmlContent += "<td class='text-left' >"+StringUtil.escapeStr(row.blyy)+"</td>"; 
		    htmlContent += "<td class='text-left' >"+StringUtil.escapeStr(row.lscs)+"</td>"; 
			...
			htmlContent +=	"</td>";
		    htmlContent += "<td class='text-center' >"+formatUndefine(row.scSqrq).substring(0,10)+"</td>";
		    htmlContent += "<td class='text-center' >"+formatUndefine(row.scBlqx).substring(0,10)+"</td>";
		    htmlContent += "<td class='text-center' >"+formatUndefine(row.zcSqrq).substring(0,10)+"</td>";
		    htmlContent += "<td class='text-center' >"+formatUndefine(row.zcBlqx).substring(0,10)+"</td>";
		    htmlContent += "<td class='text-left' >"+StringUtil.escapeStr(row.zcBlyy)+"</td>"; 
		    htmlContent += "<td class='text-left' >"; 
		    if(row.zt==9){
...
function filseDown(id){
	FilesDownModal.show({
		 djid:id,//单据id
		 code:"quality:defectmonitoring:main:01",
		 callback: function(data){//回调函数
			 if(null != data && data != ''){
