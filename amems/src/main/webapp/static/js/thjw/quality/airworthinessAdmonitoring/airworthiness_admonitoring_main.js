...
$(function(){
	...
	// 带条件搜索的翻页
	 function AjaxGetDatasWithSearch(pageNumber,sortType,sequence){
		  ...
		  startWait();
		  AjaxUtil.ajax({
				url : basePath + "/quality/airworthiness/admonitoring/queryAllAd",
				type : "post",
	...
	//关闭技术文件监控
	 function closeMonitoring(sid,zt){
	 	...
	 	AlertUtil.showConfirmMessage(messageStater, {callback: function(){
	 		startWait();
	 			 AjaxUtil.ajax({
	 				 url:basePath+"/quality/airworthiness/admonitoring/closeMonitoring",
	 				 type:"post",
	 				 async: false,
	 				 data:{
	 					 'id' : sid,
	...
	//关闭适用性监控(确认)
	 function adClose(){
		 ...
		 obj.paramsMap = paramsMap;
		  startWait();//开始Loading
		  AjaxUtil.ajax({
			  url:basePath+"/quality/airworthiness/admonitoring/closeApplyMonitoring",
				type: "post",
				contentType:"application/json;charset=utf-8",
...
function exportExcel(){
	...
	window.open(basePath+"/quality/airworthiness/admonitoring/ADMonitor.xls?params="+JSON.stringify(searchParams));
}
