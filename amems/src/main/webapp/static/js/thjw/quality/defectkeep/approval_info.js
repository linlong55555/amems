...
approval_process_info = {
	...
	show: function(param){
		...
		if(status==1){
			...
			//只有批准可以编辑
			$("#"+this.id+"_shenpi").attr("disabled",true);
			$("#"+this.id+"_xfqx").attr("disabled",true);
			$("#"+this.id+"_yxxz").attr("disabled",true);
			$("#approvedDate").attr("disabled",true);
	...
	getData:function(){
		...
		data.yxxz=$("#"+this.id+"_yxxz").val();		//运行限制
		data.xfqx=$("#approvedDate").val(); //修复期限
		return data;
	}
};