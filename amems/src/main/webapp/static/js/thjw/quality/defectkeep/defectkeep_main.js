			...
			if( row.zt == 4 ){
				if(row.gdid==null){
					htmlContent += "<i class='iconnew-create color-blue cursor-pointer checkPermission' " 
						+ " permissioncode='produce:reservation:defect:main:05' onClick=\"createNrc('"+row.id+"','"+row.bs145+"')\" title='创建NRC Create NRC'></i>&nbsp;&nbsp;";//创建nrc
				}
 ...
 * 下发nrc
 */
function createNrc(id,bs145){
	if(bs145==1){
		window.open(basePath+"/produce/reservation/defect/openOrder145/"+id);
	}else{
		window.open(basePath+"/produce/reservation/defect/openOrder135/"+id);
	}
}
