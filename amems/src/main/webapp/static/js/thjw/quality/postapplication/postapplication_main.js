...
$(function(){
    ...
    if(todo_ywid){
    	if(todo_jd == 5){
    		Post_Update_Modal.open(todo_ywid);
    	}
    	todo_ywid = null;
    }
...
function AjaxGetDatasWithSearch(pageNumber,sortType,sequence){
	...
	AjaxUtil.ajax({
	   ...
	   success:function(data){
 			...
 			// 标记关键字
			signByKeyword($("#keyword_search").val(), [2,4]);
 			}else{
	 			$("#postapplication_list").empty();
				$("#pagination").empty();
				$("#postapplication_list").append("<tr><td class='text-center' colspan=\"10\">暂无数据 No data.</td></tr>");
 			}
	 ...
	 $.each(list,function(index,row){
			...
			if( row.zt == 1 ||row.zt == 5){
				htmlContent += "<i class=' icon-edit color-blue cursor-pointer checkPermission' " 
					+ " permissioncode='quality:post:application:main:02' onClick=\"Post_Update_Modal.open('"+row.id+"')\" title='修改 Edit'></i>&nbsp;&nbsp;";//修改
				htmlContent += "<i class='fa glyphicon glyphicon-trash color-blue cursor-pointer checkPermission' permissioncode='quality:post:application:main:03' " 
	    			+ " onClick=\"invalid('"+ row.id + "','"+ row.zt +"')\" title='删除 Delete'></i>&nbsp;&nbsp;"; //删除
			}
			if( row.zt == 2){
					htmlContent += "<i class='iconnew-auditor color-blue cursor-pointer checkPermission' " 
						+ " permissioncode='quality:post:application:main:04' onClick=\"Post_Update_Shr_Modal.open('"+row.id+"')\" title='修改审核人Update Leader'></i>&nbsp;&nbsp;";//修改审核人
			}
			if( row.zt == 2 || row.zt == 3 || row.zt == 4){
				htmlContent += "<i class='iconnew-end color-blue cursor-pointer checkPermission' " 
					+ "sqsqdh='"+StringUtil.escapeStr(row.sqsqdh)+"'   zt='"+row.zt+"'    gbyy=''" 
					+ "gbczsj=''   gbrid='' " 
					+ " permissioncode='quality:post:application:main:05' onClick=\"zdjs('"+row.id+"',this,true)\" title='指定结束 Close'></i>&nbsp;&nbsp;";//指定结束
			}
		    htmlContent += "</td>";
