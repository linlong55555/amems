...
//	logModal.init({code:'B_Z_00101'});
	authorizationmonitor.init();
	refreshPermission();
});

var authorizationmonitor = {
		id:'authorizationmonitor',
		monitorsettings: {},
		data: {},
		init: function() {
//			$('.date-picker').daterangepicker().prev().on("click",
//					function() {
//						$(this).next().focus();
//			});
		...
		search: function() {
			this.load(1);
			TableUtil.resetTableSorting("authorizationmonitor_Table");
		},
		load : 	function(pageNumber, sortColumn, orderType){
			...
			var this_ = this;
			AjaxUtil.ajax({
				url: basePath+"/quality/authorizationmonitor/page",
				type: "post",
...
function exportExcel(){
	var keyword = $.trim($("#authorizationmonitor_keyword_search").val());
	...
	window.open(basePath+"/quality/authorizationmonitor/LicenseValidMonitor.xls?dprtcode="
 				+ dprtcode 
 				+ "&dqrqStart="+encodeURIComponent(dqrqStart)
 				+ "&dqrqEnd="+encodeURIComponent(dqrqEnd)
 				+ "&xxlx="+encodeURIComponent(21)
