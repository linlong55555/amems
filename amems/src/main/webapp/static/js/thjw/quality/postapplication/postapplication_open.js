...
Post_Open_Modal = {
	...
	EmptiedData : function(){
		...
		if($.inArray(code.toLowerCase(), buttonPermissions) == -1){
			$("#xm").removeClass("readonly-style");  	 //去掉文本框为白的的样式
			$("#xm").attr("ondblclick","");	
		}else{
			$("#xm").addClass("readonly-style");  //去掉文本框为白的的样式
			$("#xm").attr("ondblclick","Post_Open_Modal.openPersonelWin();");	
		}
	},
	/**
	 * 展开或收缩人员信息
	...
	openProjectWin: function(){
		...
		param.callback = function(data){
			$("#sqgwid").val(data.id);
			$("#sqgwms").val(StringUtil.escapeStr(data.dgbh));
			$("#dgmc").val(StringUtil.escapeStr(data.dgmc));
			
			var fjjx=plane_model_list_edit.getFjjxList();
			Post_Open_Modal_Trainingcourse.setData($("#wxrydaid").val(),$("#sqgwid").val(),data.dprtcode,fjjx);  //课程培训经历
	...
	fillTableData: function(data){
		...
		if($('#form1').data('bootstrapValidator')!=null){
			$('#form1').data('bootstrapValidator')  
		...
		startWait();
	   	 AjaxUtil.ajax({
		   	    ...
		   	    success:function(data){	   	    
		   	    	finishWait();
					// 回显表格数据
					this_.fillTableDatatojx(data);
		        }
	        }); 
	},
	//初始化数据
	fillTableDatatojx: function(data){
		...
		Post_Open_Modal_Trainingcourse.setDataNofjjx($("#wxrydaid").val(),$("#sqgwid").val(),data.dprtcode,$("#id").val());  //课程培训经历
		Post_Open_Modal_MeTraining.setData(data.trainings);  				//全部培训经历
