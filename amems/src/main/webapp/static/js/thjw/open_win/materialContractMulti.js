MaterialContractModal = {
	alertId:'MaterialContractModal',
	tbodyId:'materialContractList',
	paginationId:'materialContractPagination',
	param: {
		existsIdList:null,
		htlx : 1,
		callback:function(){},
	...
	AjaxGetDatasWithSearch: function(pageNumber,sortType,sequence){
		var this_ = this;
		var url = basePath+"/aerialmaterial/reserve/queryReserveDetailPageList";
		...
		AjaxUtil.ajax({
		   ...
		   success:function(data){
			   ...
			   if(data.total >0){
				   ...
				   $("#"+this_.tbodyId, $("#"+this_.alertId)).append("<tr><td colspan=\"10\" class='text-center'>暂无数据 No data.</td></tr>");
			   }
	     }
	   }); 
	},
	gatherSearchParam : function(){
		 ...
		 if(null != yqqx && "" != yqqx){
			 var yqqxBegin = yqqx.substring(0,10)+" 00:00:00";
			 var yqqxEnd = yqqx.substring(13,23)+" 23:59:59";
	...
	appendContentHtml: function(list){
		...
		$.each(list,function(index,row){
			...
			if(this_.param.htlx == 1){
				htmlContent += "<td style='text-align:right;vertical-align:middle;' >"+formatUndefine(row.SL - row.YCGSL)+"</td>";
			}else{
			...
			htmlContent += "<td title='"+StringUtil.escapeStr(formatUndefine(row.USERNAME)+" "+formatUndefine(row.REALNAME))+"' style='text-align:left;vertical-align:middle;' >"+StringUtil.escapeStr(row.USERNAME)+" "+StringUtil.escapeStr(row.REALNAME)+"</td>"; 
