...
outfield_main = {
		...
		AjaxUtil.ajax({
			...
			success:function(data){
				...
				if(data.total >0){
					...
					// 标记关键字
					signByKeyword($("#materialAndOutfield_keyword_search").val(),[2,3,5,6,8], "#"+this_.id+"_list tr td");
				} else {
	...
	getParams : function(){
		...
		var qczt = $.trim($("#materialAndOutfield_qczt_search").val());
	...
	appendContentHtml: function(list){
		...
		$.each(list,function(index,row){
			...
			htmlContent += "<td style='text-align:center;vertical-align:middle;'>"+StringUtil.escapeStr(row.qczt)+"</td>";  
		   	htmlContent += "</tr>";
	   });
