   ...
   var PDFViewerApplication = {
    ...
    setTitleUsingUrl: function pdfViewSetTitleUsingUrl(url) {
     ...
     try {
    	 ...
    	 if(url.indexOf("fileName=") != -1){
    		 var fileName = url.substring(url.lastIndexOf("fileName=") + 9, url.length);
    		 this.setTitle(decodeURIComponent(fileName) || url);
    	 }else{
    ...
    progress: function pdfViewProgress(level) {
     ...
     if (percent > this.loadingBar.percent || isNaN(percent)) {
      ...
      document.getElementById("loadingBar_percent").innerHTML = percent+"%";
    ...
    bindWindowEvents: function pdfViewBindWindowEvents() {
     ...
     window.addEventListener('afterprint', function windowAfterPrint() {
      ...
      var file = parseQueryString(document.location.search.substring(1));
      if(file.afterprint){
    	  try{
    		  doRequest4AddPrintCount(file.id,file.dprtcode,file.ywlx,file.basepath);
          }catch(e){
          }
      }
     });	
...
function doRequest4AddPrintCount(id,dprtcode,ywlx,basePath){
	var param={};
	param.ywid=id;
	param.dprtcode=dprtcode;
	param.ywlx=ywlx;
	$.ajax({
		url: basePath+"/sys/print/add",
