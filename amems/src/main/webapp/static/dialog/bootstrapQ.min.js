...
bootstrapQ.alert = function(options, func) {
    ...
    qiao.on("#bsmodal", "hidden.bs.modal",
    function() {
        $modal.remove()
    });
    $modal.modal("show")
...
bootstrapQ.dialog = function(options, func) {
    var opt = $.extend({},
    bootstrapQ.modaloptions, options);
    opt.big = true;
    $("body").append(bootstrapQ.modalstr(opt));
    var html = qiao.ajax({
        url: options.url,
...
bootstrapQ.tree.sub = function(tree, opt) {
    var res = "";
    if (tree) {
        var res = "<li>" + '<a href="javascript:void(0);" data="id:' + tree.id + ";url:" + tree.url + ';">' + '<span class="glyphicon glyphicon-minus"></span>';
        if (opt.checkbox) {
        ...
        if (opt.edit) {
            res += '&nbsp;&nbsp;<span class="label label-primary bstreeadd">\u6dfb\u52a0\u5b50\u83dc\u5355</span>' + '&nbsp;&nbsp;<span class="label label-primary bstreeedit">\u4fee\u6539</span>' + '&nbsp;&nbsp;<span class="label label-danger  bstreedel">\u5220\u9664</span>'
        }
        res += "</a>";
...
bootstrapQ.tree.del = function() {
    var res = qiao.ajax({
        url: bootstrapQ.tree.url + "/del/" + $(this).parent().qdata().id
    });
    bootstrapQ.msg(res)
};
...
bootstrapQ.bstroinit = function(selector, options, step) {
    if (selector) {
        ...
        if ($element.length > 0) {
            var opt = $.extend({},
            bootstrapQ.bstrooptions, options);
            if (typeof options == "string") {
                opt.content = options
            } else {
                $.extend(opt, options)
...
bootstrapQ.bsdate = function(selector, options) {
    if (selector) {
        ...
        if ($element.length > 0) {
            var opt = $.extend({},
            bootstrapQ.bsdateoptions, options);
            $element.each(function() {
                $(this).datepicker(opt)
            })
        }
