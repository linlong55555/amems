...
var App = function () {
    ...
    var handleSidebarToggler = function () {
        ...
        $('.sidebar-toggler-wrapper, .header').on('click', '.sidebar-toggler', function (e) {
...
/*        	
    ...
    return {
        ...
        blockUI: function (options) {
            ...
            var html = '';
            if (options.iconOnly) {
                html = '<div class="loading-message ' + (options.boxed ? 'loading-message-boxed' : '')+'"><img style="" src="/pbs/static/assets/img/loading-spinner-grey.gif" align=""></div>';
            } else if (options.textOnly) {
                html = '<div class="loading-message ' + (options.boxed ? 'loading-message-boxed' : '')+'"><span>&nbsp;&nbsp;' + (options.message ? options.message : 'LOADING...') + '</span></div>';
            } else {    
                html = '<div class="loading-message ' + (options.boxed ? 'loading-message-boxed' : '')+'"><img style="" src="/pbs/static/assets/img/loading-spinner-grey.gif" align=""><span>&nbsp;&nbsp;' + (options.message ? options.message : 'LOADING...') + '</span></div>';
            }
        ...
        resizeHeight:function(){
            ...
            var headerHeight = $('.header').outerHeight();
            // 头部菜单高度
            var headerDownHeight = $('.header-down').outerHeight();//因为头部菜单上移到了LOGO行，计算时不需要再带入头部菜单高度
            // 屏幕高度
            ...
            // 菜单高度
            var menuHeight = windowHeight - headerHeight- footerHeight - headerDownHeight - copyrightHeight;
            //设定菜单高度
            $("#mCSB_1").attr("style",'height:' + menuHeight + "px");
            //设定右侧面板高度和自动Y轴滚动条
            $(".page-content .panel-body:first").height(bodyHeight-3);
            $(".page-content .panel-body:first").css("overflow-y","auto");
            
            //设定表格高度
            var tableHeight = bodyHeight - rowHeight - paginationHeight - searchHeight - adjustHeight - 5;
            // chrome浏览器
            if(navigator.userAgent.indexOf("Chrome") > -1){
            	tableHeight -= 10;
            }
            $(".table-h").attr('style', 'height:' + tableHeight + 'px !important;overflow-x: auto;');
            /*alert($(".displayContent").is(":hidden"));*/
            //多列表高度设定
            if($(".displayContent").is(":hidden")){
            	...
            	if($(".displayContent").length==0){
                	...
                	if($table.length > 0){
                		...
                		var bottom_panel_heading;
                		if($(".displayContent>ul").length>0 ){
                			//下方的格式是Tab
                			bottom_panel_heading=$(".displayContent>ul").eq(0).outerHeight()||0;     
                			var botttom_panel_body=bottom_height-bottom_panel_heading-5;
                    		$(".displayContent .tab-content .tab-pane:visible .panel-body ").attr('style', 'height:' + botttom_panel_body+ 'px !important;');
                    		//下方内容中是否存在搜索条件，若存在则需要在搜索条件的div中加row-height class
                    		var displayContentTabRowHeight=$(".displayContent .tab-content .row-height:visible").outerHeight()||0;
                    		var tableTabheight=botttom_panel_body-displayContentTabRowHeight-3;
                    		//tab中表格需要设置样式，表格中添加tab-table-content样式
                    		 $(".displayContent .tab-content .tab-pane:visible .tab-table-content").attr('style', 'height:' + tableTabheight+ 'px !important;overflow: auto;')
                		}else{
                			bottom_panel_heading=$(".displayContent .panel-heading").eq(0).outerHeight()||0;
                			var botttom_panel_body=bottom_height-bottom_panel_heading-5;
            ...
            /*显示Tab*/
          /* 1. page-content 中加Class为tabcontent-main
           2.给第一个UL上面的div加class为first-tab-style，上下分屏c-height必须得写。c-height:50%
           3.隐藏的Div class为displayTabContent*/
            ...
            // 浮动日志div高度
            $("#floatDiv").height($(".page-content:first").height()-2);
            try{
            	$("#logTableDiv").height($("#log_pagination").offset().top-$("#logTableDiv").offset().top - 5);
            }catch(e){}
            //各页面自定义
            try{
            	customResizeHeight(bodyHeight, tableHeight);
            }catch(e){}
        }
    };
