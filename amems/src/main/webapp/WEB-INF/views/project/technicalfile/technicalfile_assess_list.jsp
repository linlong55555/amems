...
<body class="page-header-fixed">
						...
						<div class="col-lg-8 col-sm-8 col-xs-8 padding-left-8 padding-right-0">
							<select id="jxgcs" class="form-control " name="jxgcs">
								...
								<c:forEach items="${users}" var="row">
									<option value="${row.id}"
										<c:if test="${user.id eq row.id }">selected='selected'</c:if>>
										${erayFns:escapeStr(row.displayName)}</option>
								</c:forEach>
							</select>
						</div>
				</div>
	...
	<script type="text/javascript"
		src="${ctx }/static/js/thjw/project/technicalfile/technicalfile_assess_list.js"></script>
</body>
</html>