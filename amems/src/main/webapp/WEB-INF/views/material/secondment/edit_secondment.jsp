...
<div class="page-content ">
		...
		<input type="hidden"  id="jddxbh" name="jddxbh" value="${secondment.jddxbh}">
		<!-------导航Start--->
		 ...
		 <div class="panel-heading" id="NavigationBar"></div>
			<div class="panel-body padding-bottom-0">
			 <div class="panel panel-default">
				...
				<input type="hidden" id="id" value="${secondment.id}">
					...
					<div class="col-lg-3 col-sm-6 col-xs-12 padding-left-0 padding-right-0"  >
						...
						<div class=" col-xs-8 padding-left-8 padding-right-0 form-group">
							 <select  id="jddxlx" name="jddxlx" class="form-control" onchange="jddxlxChange()">
								  <c:forEach items="${secondmentTypeEnum}" var="item">
									<option value="${item.id}" <c:if test="${secondment.jddxlx == item.id }">selected="true"</c:if> >${item.name}</option>
								  </c:forEach>
						       </select>
						</div>
					</div> 
									...
									<option value="">请选择</option>
										<c:choose>
											...
											<c:forEach items="${departments}" var="type">
												<option value="${type.id}" <c:if test="${secondment.jddxbh == item.id }">selected="true"</c:if>>${type.dprtname}
												</option>
											</c:forEach>
											</c:when>
											<c:otherwise>
			 ...
			 <div class="col-lg-12 col-sm-12 col-xs-12  padding-right-0" >
			    <div class=" col-lg-12 col-sm-12 col-xs-12  padding-left-0 padding-right-0" >
						...
						<div class="col-lg-11  col-sm-10 col-xs-8 padding-left-8  padding-right-0 form-group">
							<input type="text" class="form-control "  id="jddxms" name="jddxms" value="${erayFns:escapeStr(secondment.jddxms)}" maxlength="150">
						</div>
				</div>
			  </div>
		       <div class="col-lg-12 col-sm-12 col-xs-12  padding-right-0" >
				    <div class=" col-lg-12 col-sm-12 col-xs-12  padding-left-0 padding-right-0" >
							...
							<div class="col-lg-11  col-sm-10 col-xs-8 padding-left-8  padding-right-0 form-group">
								<textarea class="form-control"  id="bz" name="bz" maxlength="300" placeholder='最大长度不超过300'>${erayFns:escapeStr(secondment.bz)}</textarea>
							</div>
					</div>
			  </div>
			  </form>	
...
<script type="text/javascript" src="${ctx}/static/js/thjw/material/secondment/edit_secondment.js"></script>
</body>
</html>
