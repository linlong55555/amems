 ...
 * @CreateBy 孙霁	
 */
@Service("stationExcelImporter")
public class StationExcelImporter extends AbstractExcelImporter<ZoneStation>{
		...
		for (Integer sheetIndex : datasMap.keySet()) {
			...
			for (int i = 0; i < datas.size(); i++) {
				...
				if(zoneSztation.getJx() != null && b){
					addErrorMessage(zoneSztation.getJx()+"机型不存在");
				}
			}
			...
			for (int i = 0; i < datas.size()-1; i++) {
				...
				if(b){
					addErrorMessage("第"+(i+2)+"行，出现重复数据，请确保机型和飞机站位数据唯一");
				}
			}
		...
		// 结果集
				...
				for (Integer sheetIndex : totalMapList.keySet()) {
					...
					for (int i = 0; i < mapList.size(); i++) {
						...
						zoneStation.setSz(bean.get(1));	// 区域/站位
						zoneStation.setMs(bean.get(2));	// 描述
						zoneStation.setLx(3);	// 飞机站位
