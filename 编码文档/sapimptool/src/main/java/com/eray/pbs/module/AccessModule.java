...
import com.eray.pbs.thread.ReadAccessThread;
import com.eray.pbs.util.Module;
import com.eray.util.framework.ThreadPoolManager;

public class AccessModule implements Module<Object>
{
    private Logger logger = LoggerFactory.getLogger(AccessModule.class);

    @Override
    public void init(DbServer<Object> server) throws Exception
    {
        logger.info("{} is starting", this.getClass().getSimpleName());
        ThreadPoolManager.getThreadPoolExecutor().execute(new ReadAccessThread());
