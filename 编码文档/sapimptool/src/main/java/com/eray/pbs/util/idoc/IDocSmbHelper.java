...
public class IDocSmbHelper
{
    ...
    public static IDoc parseTxt(SmbFile iDocFile) throws IOException
    {
        if (iDocFile == null)
        {
            return null;
        }
        BufferedReader in = new BufferedReader(new UnicodeReader(new SmbFileInputStream(iDocFile), Charset
                .defaultCharset().name()));
        int start = 0;
        String line;
        IDoc iDoc = new IDoc();
                ...
                if (segment.startsWith("E2ORTOB_PARTNR_ADR"))
                {
                    ...
                    if (sdata.length() >= 486)
                    {
                        ...
                        e2ORTOB_PARTNR_ADR.setTEL_NUMBER(sdata.substring(346,376).trim());
                         e2ORTOB_PARTNR_ADR.setTEL_EXTENS(sdata.substring(376,
                         386).trim());
                         e2ORTOB_PARTNR_ADR.setSTR_SUPPL1(sdata.substring(386,
                         426).trim());
                         e2ORTOB_PARTNR_ADR.setSTR_SUPPL2(sdata.substring(426,
                         466).trim());
                         e2ORTOB_PARTNR_ADR.setSORT1(sdata.substring(466,
                                 486).trim());
